...
Hosted state persistence is pluggable. The default remains the local private app-data JSON file for desktop and local worker runs. Production deployments can use the PostgreSQL snapshot adapter by providing an injected SQL query function and setting `GIDEON_STORE_PROVIDER=postgres_snapshot`, `GIDEON_DATABASE_URL`, and optionally `GIDEON_POSTGRES_SNAPSHOT_TABLE`; `pnpm worker:hosted:check` validates this configuration before deploy.
