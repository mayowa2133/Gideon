...
For containerized local smoke testing, use `docker-compose.hosted-worker.yml`; production deployments can build `Dockerfile.hosted-worker` and scale that worker service horizontally against a managed Redis/BullMQ backend. Keep the worker on a private network with no public ingress, mount durable `/data` only when using local store/artifact paths, and prefer private object storage for production artifacts.
