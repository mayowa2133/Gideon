...
GitHub Actions also exposes an opt-in `workflow_dispatch` live promotion path on the macOS workflow. Set `run_live_promotion=true` only after staging secrets, base64 fixture secrets, staging vars, the scratch hosted MCP project/session, metric probe configuration, and Apple signing credentials are configured; the workflow validates and materializes those private fixtures with `pnpm production:fixtures:materialize`, then uploads `Gideon-production-promotion-evidence` as the safe promotion artifact.
