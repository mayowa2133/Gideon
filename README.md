...
Live promotion writes a safe JSON evidence report to `tmp/production-promotion-evidence.json` by default, or to `GIDEON_PRODUCTION_PROMOTION_EVIDENCE_PATH` when set. The report records step names, commands, safe env overrides, timings, exit codes, and the failed step if any; it does not record cookies, API keys, signed URLs, provider payloads, transcripts, prompts, or media paths.
