...
export interface FlowExecutionDto { id: string; captureRunId: string; flowId: string; flowRevision: number; status: "queued" | "dry_running" | "recording" | "normalizing" | "verified" | "failed" | "blocked" | "canceled"; attempt: number; blockerCode: string | null; normalizedClipArtifactId: string | null; quality?: null | { status: "ready" | "warning" | "failed"; checks: Array<{ code: string; status: "pass" | "warning" | "fail" }> }; updatedAt: string }
export interface CoverageDimensionDto { key: string; denominator: number | "unknown"; denominatorSource?: string; coveredIds: string[]; uncoveredIds: string[]; excluded: Array<{ id: string; reason: string }>; blocked: Array<{ id: string; code: string }> }
export interface CoverageSnapshotDto { id: string; projectId: string; environmentVersionId: string; calculationVersion: string; dimensions: CoverageDimensionDto[]; createdAt: string }
...
export class CaptureApi {
  ...
  setFlowApproval(projectId: string, flowId: string, approval: "approve" | "reject") { return this.request<{ flow: ProductFlowDto }>("POST", projectPath(projectId, `/product-flows/${encodeURIComponent(flowId)}/${approval}`), {}).then((data) => data.flow); }
