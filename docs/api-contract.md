...
- **Queue handoff:** When `GIDEON_HOSTED_QUEUE_URL`/`GIDEON_WORKER_QUEUE_URL` and matching queue secret are configured, hosted dependencies enqueue analysis jobs by POSTing `{ kind, projectId, jobId }` to the worker endpoint with `X-Gideon-Queue-Timestamp` and HMAC `X-Gideon-Queue-Signature`. Worker intake must verify the timestamp tolerance, HMAC, JSON shape, and allowed `analysis|render` kind before dispatching to the worker executor. A configured intake lease coordinator claims the job lease before dispatch, heartbeats after dispatch, marks dispatch failures against the owning worker lease, and recovers expired leases before accepting new work. The hosted worker runtime adapter can detach accepted jobs for background execution, maintain heartbeats while the executor runs, and report executor failures through the same lease coordinator. Hosted dependencies may also enqueue into a broker interface that exposes enqueue/subscribe/cancel/stats; `GIDEON_HOSTED_QUEUE_PROVIDER=memory` or `GIDEON_WORKER_QUEUE_PROVIDER=memory` selects the in-memory broker for local composition, while `GIDEON_HOSTED_QUEUE_PROVIDER=bullmq` or `GIDEON_WORKER_QUEUE_PROVIDER=bullmq` with `GIDEON_REDIS_URL` or `REDIS_URL` selects the Redis-backed BullMQ broker. `GIDEON_BULLMQ_QUEUE_NAME` and `GIDEON_BULLMQ_PREFIX` customize BullMQ naming. Worker process bootstrap uses the same broker plus store-backed lease coordinator and executor hooks, with `GIDEON_WORKER_ID`, `GIDEON_WORKER_LEASE_SECONDS`, and `GIDEON_WORKER_HEARTBEAT_INTERVAL_MS` controlling lease identity and cadence. The hosted worker process emits structured JSON lifecycle/job metrics and persisted job observability snapshots for queue depth, queue age, expired leases, recovered lease failures, and terminal failure rate.
...
- **Response 200:** Ordered moments with evidence-frame thumbnail signed URLs, transcript excerpts, bounded typed OCR UI elements where available, and validated `sourceEvidenceIds` that point to transcript segments, frame OCR evidence, or fallback moments from the analysis evidence catalog.
...
- **Response 200:** Script resources including hook, voiceover, CTA, caption cues, visual beats, validation warnings, revision, and sanitized EDL projections with `templateId`, `templateVersion`, `brandKitId`, quick-cut transitions, callouts, and cursor emphasis cues.
- **Errors:** 404 project.
- **Rate limit:** 120/min.
