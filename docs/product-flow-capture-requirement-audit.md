...
| Persistence and idempotency | Migrations, workspace-scoped repositories, atomic run/job creation, canonical JSONB immutability, concurrent race convergence, transaction rollback, and workspace-keyed immutable usage deduplication pass on disposable PostgreSQL 16 | Apply to staging PostgreSQL and run deployment load/failover tests |
| Queue, cancellation, retry, quotas, usage | Real disposable Redis 8/BullMQ tests cover duplicate suppression, bounded retry after worker failure, durable recovery after broker-client interruption, pending cancellation, lease recovery/claim/heartbeat/failure recording, and queue drain; cooperative capture cancellation and quota/usage hooks are implemented | Connect the production entitlement ledger and distributed fairness/rate-limit scheduler; exercise full Redis process outage/failover in staging |
...
| Self-service UI | Hosted Next.js project launcher and connect/discover/edit/exact-revision approve/run/results/framing preview/quality/repair-review/retry-state/coverage/assembly screens implemented; same-origin proxy and real-browser E2E are tested; capture controls fail closed unless all dependencies are configured | Deploy with the existing auth shell, run accessibility/usability review, and validate with design partners |
...
- `pnpm test:infrastructure`: provisions disposable PostgreSQL/Redis, applies all migrations, runs 26 real-service/storage lifecycle checks, writes a redacted report, and verifies teardown.
