...
| Avatar generation, voice cloning | Avatar worker implemented; production model canary pending | Fictional catalog and consent-gated self-avatar sources have private lineage, revocation, disclosure, preflight, and isolated worker controls. Voice cloning is disabled. | Run the model-backed canary on approved GPU infrastructure and complete release/legal review of exact weights. |
