...
The next slice should run `pnpm production:promote:check -- --live` against staging credentials/fixtures and Apple signing credentials, or harden hosted MCP mode through the authoritative hosted service layer while keeping desktop compatibility intact.
