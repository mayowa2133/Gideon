...
Current engineering estimate: **99.99999997% complete** toward the full original product vision.
...
| Stage-level retry/cancel jobs | Implemented for the current job model | `src/shared/jobState.ts`, `src/main/jobQueue.ts`, `src/main/store.ts`, `src/main/jobExecutor.ts`, and tests in `src/shared/jobState.test.ts`, `src/main/store.test.ts`, `src/main/jobExecutor.test.ts`, and `src/main/jobQueue.test.ts` cover job states, retryability, canceling, leases, heartbeats, expired lease recovery, safe failure mapping, corrected next-attempt retry audit metadata, and executor lifecycle events that carry attempt/max-attempt boundaries for stage timelines. | Extend stage-specific worker lanes as ASR/OCR become fully hosted services. |
