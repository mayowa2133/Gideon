...
Current engineering estimate: **99.999991% complete** toward the full original product vision.
...
| Direct-to-cloud uploads and private object storage | Implemented as primitives | `src/main/storage.ts`, `src/main/store.ts`, `src/main/hostedApi.ts`, `src/main/storage.test.ts`, `src/main/hostedApi.test.ts`, `scripts/run-staging-smoke.mjs`, `scripts/check-staging-readiness.mjs`, `scripts/check-storage-policy.mjs`, and API docs cover direct upload sessions, private artifact records, signed downloads, sanitized hosted render discovery, a live-capable upload-to-export smoke runner, strict readiness checks for the deployed smoke configuration, lifecycle/deletion/signed-URL policy preflight, and non-public storage behavior. | Run the live staging smoke against real S3-compatible storage, verify the actual bucket lifecycle rules match the configured policy, and add multipart/resumable upload if required. |
...
3. Production object storage credentials, actual bucket lifecycle-rule verification, and signed-download smoke tests. The private S3/R2 lifecycle/deletion/signed-URL policy preflight is now covered by `pnpm production:storage:check`.
