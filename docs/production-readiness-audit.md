...
Current engineering estimate: **99.994% complete** toward the full original product vision.
...
| Codex/Claude Code MCP control without Gideon API keys | Implemented as a first-class path | `src/mcp/server.ts`, `src/mcp/server.test.ts`, `src/main/hostedApi.ts`, `src/main/hostedApi.test.ts`, `docs/mcp-agent-control.md`, `docs/implementation-plan.md`, and README document Palmier-style MCP tools for local and hosted project inspection, bounded script/moment edits, audit events, and analysis/render enqueueing. Hosted MCP mode uses the authenticated hosted API session, CSRF-protected edit/job routes, sanitized project context, and optimistic revision preconditions for script/moment edits. The agent supplies model reasoning; Gideon does not require LLM provider API keys for this control path. | Run remote/enterprise MCP access through staging SSO/session policy and production load testing. |
...
The next slice should run `pnpm production:promote:check -- --live` against staging credentials/fixtures and Apple signing credentials, or add production load/retry policy checks for hosted MCP/API review edits while keeping desktop compatibility intact.
