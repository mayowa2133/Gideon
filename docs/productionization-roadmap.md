...
3. Run the manual GitHub Actions live promotion workflow with staging upload-to-export, hosted MCP smoke, deployed metric export, signed/notarized release credentials, self-verified promotion evidence, and `pnpm production:github-evidence:check -- --run-id <github-run-id>` archived release evidence before removing snapshot reads entirely.
