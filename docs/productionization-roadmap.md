...
3. Run `pnpm production:github-settings:check -- --repo mayowa2133/Gideon`, then `pnpm production:github-promote:run -- --confirm-live` with staging upload-to-export, hosted MCP smoke, deployed metric export, signed/notarized release credentials, self-verified promotion evidence, and archived release evidence plus a `pnpm production:github-receipt:check -- --path tmp/github-production-promotion-evidence/verification-receipt.json` validation of its safe verification receipt before removing snapshot reads entirely.
