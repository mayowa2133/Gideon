...
- Native/extension recording, autonomous browser control, unauthorized third-party/public-figure avatar generation, voice cloning, social posting, analytics, or a general multitrack editor. Custom avatar generation is limited to private source artifacts with explicit self/authorized-likeness consent and revocation controls.
...
- Action locators wait for asynchronous application rendering only within the configured bounded action timeout, then still require exactly one visible match. Missing, hidden, and ambiguous controls fail closed before interaction.
- The committed hostile fixture exercises complex UI and adversarial text against the same compiler/executor. Explicit 500–30,000 ms action timeouts bound locator waits; hidden geometry candidates are rejected before scroll alignment. Dangerous controls are denied by inferred minimum risk plus the approved risk allowlist, and the fixture's server-side trap counters must remain zero.
- Locator resolution ranks labels, stable-link destinations, exact accessible roles, test IDs, named-landmark structure, placeholders, and text. The executor counts visible matches before acting and fails closed for zero, hidden-only, or ambiguous matches. Provider-guided discovery and repair enforce runtime schemas, evidence grounding, timeout/attempt/candidate budgets, duplicate rejection, and cooling circuits. Repair compares safe page fingerprints, permits only uniquely evidenced locator/wait changes on failed steps, and routes material changes to a new human-reviewed revision.
- `capture-masking-v1` is installed through the browser context before page creation and is manifest-hash bound. Protected form fields, detectable visible sensitive text, configured selectors, and canvases receive direct visual obscuring plus fixed overlays that track DOM/scroll/resize/modal changes. Per-frame audits run before screenshots/actions and completion; only policy hashes and numeric counts leave the worker. Sensitive assertion text is removed from receipts after evaluation, and remote receipts are privacy-revalidated.
- Synthetic fixture values cross the isolated-client boundary through a scoped opaque fixture grant rather than the declarative manifest. Only the grant ID and bounded fixture keys are hash-bound; the client must revoke the staged grant in terminal success and failure paths.
...
Frame ranking records proof score, inferred visual role, focus region, interaction hints, and a deterministic `beforeAfterPairId` when an early before-state can be paired with a later payoff. The before/after template consumes that pair when choosing its problem, payoff, and CTA beats. The paired evidence is available to review and creator templates without treating OCR text as instructions.
...
The deterministic script compiler also applies the selected creator tone to its opening spoken line: direct, casual, founder, educational, bold, or professional. Each variant retains short sentences and source-backed visual beats rather than changing the underlying evidence plan.
...
`docker-compose.avatar-musetalk.yml` defines the production-oriented MuseTalk 1.5 worker at pinned source revision `0a89dec45a0192b824e3cf4daf96c239440c5ed8`. It uses the same request/consent/receipt protocol and isolation controls, while mounting the operator-reviewed MuseTalk, VAE, Whisper, DWPose, and face-parsing model tree read-only. The image never downloads weights at build or runtime. Release approval covers each component model separately; the MuseTalk repository's MIT code license and commercial model statement do not override inherited component terms.
...
Each avatar artifact also records the source script ID, exact script revision timestamp, and source voiceover artifact when available. The render executor selects only a private local clip whose lineage and model receipt match the current script and selected fictional avatar. FFmpeg loops and crops that clip into the configured presenter position before applying transparent captions and disclosure overlays; stale, missing, or mismatched clips fall back to the deterministic catalog presenter.
