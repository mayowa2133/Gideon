...
- Run `pnpm lint`, `pnpm production:check`, `pnpm package:mac`, `pnpm release:mac:check`, `hdiutil verify release/Gideon-0.1.0-arm64.dmg`, and `pnpm staging:check`; production-shaped staging candidates must also pass `pnpm staging:check -- --strict`, `pnpm package:mac:signed`, and `GIDEON_RELEASE_CHANNEL=production pnpm release:mac:check`.
