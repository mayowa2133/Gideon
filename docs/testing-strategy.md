...
- Run `pnpm lint`, `pnpm hosted:review:check`, `pnpm production:check`, `pnpm production:promote:check -- --dry-run`, `pnpm package:mac`, `pnpm release:mac:check`, `hdiutil verify release/Gideon-0.1.0-arm64.dmg`, `pnpm staging:check`, and `pnpm staging:smoke -- --dry-run`; production-shaped staging candidates must also pass `pnpm production:promote:check -- --live`, `pnpm staging:check -- --strict`, `pnpm staging:smoke -- --live`, `pnpm package:mac:signed`, and `GIDEON_RELEASE_CHANNEL=production pnpm release:mac:check`.
