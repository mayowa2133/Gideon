...
7. Results show each clip’s verified/failed/blocked status, automated video-quality status, safe non-pass check codes, and the coverage denominator. Failed-quality clips are not previewable or selectable for assembly; warning clips remain available for explicit human review. Never label unknown product areas as uncovered zero or claim complete-product coverage.
...
- Framing: per-beat source in/out plus normalized focus point and zoom intensity; no arbitrary keyframe editor in MVP.
