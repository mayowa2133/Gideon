...
const root=path.resolve(path.dirname(fileURLToPath(import.meta.url)),".."),output=path.join(root,"tmp","solomon-creator-story-v19-performance"),master=process.env.SOLOMON_V19_MASTER??path.join(output,"final","solomon-creator-story-v19-performance-master-NOT-FOR-PUBLICATION.mp4"),baseline=JSON.parse(await fs.readFile(path.join(output,"baseline","v18-authoritative-baseline.json"),"utf8")),quality=JSON.parse(await fs.readFile(path.join(output,"reports","media-quality-report.json"),"utf8"));
const current=await measureV19Motion(master,[{id:"opening",fromSeconds:0,toSeconds:4.4},{id:"mechanism",fromSeconds:14,toSeconds:24.5},{id:"payoff",fromSeconds:27.5,toSeconds:32},{id:"cta",fromSeconds:32,toSeconds:36}]),v18=baseline.v18.motion,referenceAverage=average(baseline.referenceMeasurements.map(({motion})=>motion.averageFrameChange));
const report={schemaVersion:"10.1",methodVersion:"common-decoded-luma-v19.1",v18:{path:baseline.v18.path,sha256:baseline.v18.sha256,motion:v18},v19:{path:master,sha256:quality.masterSha256,motion:current},references:baseline.referenceMeasurements.map(({path,sha256,motion})=>({path,sha256,motion})),improvements:{hookComprehension:"Cause-and-effect is shown with Applied, a cursor action, and Interviewing in the opening proof chain.",contactRevealTimingSeconds:1.8,productOccupancy:"Large isolated evidence objects replace desktop-page framing.",requiredTextReadability:quality.ocr.requiredCoverage,semanticMotion:"Every manifest scene has a semantic event; decoded movement is reported separately so decorative drift cannot satisfy the contract alone.",transitionTypes:quality.transitions.boundaries,mascotFaces:quality.mascot.renderedFaceStateCount,mascotGestures:quality.mascot.renderedGestureStateCount,payoff:"One combined state replaces V18’s four-screen carousel.",cta:"Mascot looks, holds, taps, confirms, and returns gaze around one truthful save action.",audio:quality.decoded.loudness,privacy:{bannedPassed:quality.ocr.banned.passed,privateMatches:quality.ocr.privateMatches,disclosurePassed:quality.ocr.singleDisclosurePassed}},targets:{medianBand:[1.2,8],continuousBand:[1.2,5.5],lowMotionAtMostSeconds:4,shotCountBand:[16,21]},referenceAverageFrameChange:referenceAverage,automatedParityClaimed:false,humanReferenceQualityRequired:true};

// Regression floors. Every metric that passed in the accepted parent version
// becomes a floor for this one: V5→V18 repeatedly fixed the criticized axis
// while silently regressing another, and only a mechanical check stops that.
const floorsPath=path.join(root,"fixtures","creator-story","v18-accepted-metrics.json");
...
const scorecardRows=Object.entries(floors.metrics).map(([metric,floor])=>{
  ...
  if(accepted&&status==="failed"&&(accepted.min===undefined||value>=accepted.min)&&(accepted.max===undefined||value<=accepted.max)){
    return{metric,v18:floor.value,v19:value,requirement:`accepted >= ${accepted.min ?? "n/a"} (${accepted.reason})`,status:"accepted"};
  }
  return{metric,v18:floor.value,v19:value,requirement,status};
});
...
await fs.writeFile(path.join(output,"reports","v19-scorecard.json"),`${JSON.stringify(scorecardReport,null,2)}\n`);
await fs.writeFile(path.join(output,"reports","v19-scorecard.md"),scorecardMarkdown(scorecardReport));
if(!scorecardReport.passed){process.stdout.write(`${JSON.stringify({regressions,failedGates},null,2)}\n`);process.exitCode=1;}

await fs.writeFile(path.join(output,"reports","v18-v19-reference-comparison.json"),`${JSON.stringify(report,null,2)}\n`);await fs.writeFile(path.join(output,"reports","v18-v19-reference-comparison.md"),`# V18 → V19 → reference comparison\n\n| Metric | V18 | V19 | Target / context |\n|---|---:|---:|---:|\n| Median movement | ${v18.medianFrameChange.toFixed(3)} | ${current.medianFrameChange.toFixed(3)} | in band |\n| Continuous movement | ${v18.continuousMovementExcludingCuts.toFixed(3)} | ${current.continuousMovementExcludingCuts.toFixed(3)} | in band |\n| Near-static frames | ${v18.nearStaticFramePercent.toFixed(2)}% | ${current.nearStaticFramePercent.toFixed(2)}% | lower is better |\n| Longest low-motion interval | ${v18.longestLowMotionSeconds.toFixed(2)} s | ${current.longestLowMotionSeconds.toFixed(2)} s | ≤ 4.0 s |\n| Average frame change | ${v18.averageFrameChange.toFixed(3)} | ${current.averageFrameChange.toFixed(3)} | references avg ${referenceAverage.toFixed(3)} |\n\nV19 improves proof size, cause-and-effect, one-state payoff, presenter acting, and CTA performance. Automated metrics do not establish reference parity; the human checklist remains blocked.\n`);process.stdout.write(`${JSON.stringify(report,null,2)}\n`);
function average(values){return values.length?values.reduce((sum,value)=>sum+value,0)/values.length:0;}

function scorecardMarkdown(report){
  const metrics=report.metrics.map(({metric,v18,v19,requirement,status})=>`| ${metric} | ${format(v18)} | ${format(v19)} | ${requirement} | ${status} |`).join("\n");
  const gates=report.gates.map(({gate,status})=>`| ${gate} | ${status} |`).join("\n");
  return `# V19 scorecard\n\nParent: V${report.parentVersion} (\`${report.parentMasterSha256}\`)\nMaster: \`${report.masterSha256}\`\nRegression-free: ${report.passed?"yes":"no"}\n\n## Metrics vs parent floors\n\n| Metric | V18 | V19 | Requirement | Status |\n| --- | --- | --- | --- | --- |\n${metrics}\n\n## Render gates\n\n| Gate | Status |\n| --- | --- |\n${gates}\n`;
}
