...
const {V21_MOTION_BANDS,V21_SHOT_BANDS}=require("../dist/main/shared/creatorStoryV21Quality.js");

const root=path.resolve(path.dirname(fileURLToPath(import.meta.url)),".."),output=path.join(root,"tmp","solomon-creator-story-v21-performance"),master=process.env.SOLOMON_V21_MASTER??path.join(output,"final","solomon-creator-story-v21-performance-master-NOT-FOR-PUBLICATION.mp4"),baseline=JSON.parse(await fs.readFile(path.join(output,"baseline","v20-authoritative-baseline.json"),"utf8")),quality=JSON.parse(await fs.readFile(path.join(output,"reports","media-quality-report.json"),"utf8"));
const current=await measureV21Motion(master,[{id:"opening",fromSeconds:0,toSeconds:4.4},{id:"mechanism",fromSeconds:14,toSeconds:24.5},{id:"payoff",fromSeconds:27.5,toSeconds:32},{id:"cta",fromSeconds:32,toSeconds:36}]),v20=baseline.v20.motion,referenceAverage=average(baseline.referenceMeasurements.map(({motion})=>motion.averageFrameChange));
const report={schemaVersion:"10.1",methodVersion:"common-decoded-luma-v21.1",v20:{path:baseline.v20.path,sha256:baseline.v20.sha256,motion:v20},v21:{path:master,sha256:quality.masterSha256,motion:current},references:baseline.referenceMeasurements.map(({path,sha256,motion})=>({path,sha256,motion})),improvements:{hookComprehension:"Cause-and-effect is shown with Applied, a cursor action, and Interviewing in the opening proof chain.",contactRevealTimingSeconds:1.8,productOccupancy:"Large isolated evidence objects replace desktop-page framing.",requiredTextReadability:quality.ocr.requiredCoverage,semanticMotion:"Every manifest scene has a semantic event; decoded movement is reported separately so decorative drift cannot satisfy the contract alone.",transitionTypes:quality.transitions.boundaries,mascotFaces:quality.mascot.renderedFaceStateCount,mascotGestures:quality.mascot.renderedGestureStateCount,payoff:"One combined state replaces V19’s four-screen carousel.",cta:"Mascot looks, holds, taps, confirms, and returns gaze around one truthful save action.",audio:quality.decoded.loudness,privacy:{bannedPassed:quality.ocr.banned.passed,privateMatches:quality.ocr.privateMatches,disclosurePassed:quality.ocr.singleDisclosurePassed}},targets:{medianBand:V21_MOTION_BANDS.medianFrameChange,continuousBand:V21_MOTION_BANDS.continuousMovementExcludingCuts,lowMotionAtMostSeconds:V21_MOTION_BANDS.longestLowMotionSeconds[1],shotCountBand:V21_SHOT_BANDS.decodedShotCount},referenceAverageFrameChange:referenceAverage,automatedParityClaimed:false,humanReferenceQualityRequired:true};
...
const floorsPath=path.join(root,"fixtures","creator-story","v20-accepted-metrics.json");
...
const scorecardRows=Object.entries(floors.metrics).map(([metric,floor])=>{
  ...
  if(accepted&&status==="failed"&&(accepted.min===undefined||value>=accepted.min)&&(accepted.max===undefined||value<=accepted.max)){
    return{metric,v20:floor.value,v21:value,requirement:`accepted >= ${accepted.min ?? "n/a"} (${accepted.reason})`,status:"accepted"};
  }
  return{metric,v20:floor.value,v21:value,requirement,status};
});
...
await fs.writeFile(path.join(output,"reports","v21-scorecard.json"),`${JSON.stringify(scorecardReport,null,2)}\n`);
await fs.writeFile(path.join(output,"reports","v21-scorecard.md"),scorecardMarkdown(scorecardReport));
if(!scorecardReport.passed){process.stdout.write(`${JSON.stringify({regressions,failedGates},null,2)}\n`);process.exitCode=1;}

await fs.writeFile(path.join(output,"reports","v20-v21-reference-comparison.json"),`${JSON.stringify(report,null,2)}\n`);await fs.writeFile(path.join(output,"reports","v20-v21-reference-comparison.md"),`# V19 → V21 → reference comparison\n\n| Metric | V19 | V21 | Target / context |\n|---|---:|---:|---:|\n| Median movement | ${v20.medianFrameChange.toFixed(3)} | ${current.medianFrameChange.toFixed(3)} | in band |\n| Continuous movement | ${v20.continuousMovementExcludingCuts.toFixed(3)} | ${current.continuousMovementExcludingCuts.toFixed(3)} | in band |\n| Near-static frames | ${v20.nearStaticFramePercent.toFixed(2)}% | ${current.nearStaticFramePercent.toFixed(2)}% | lower is better |\n| Longest low-motion interval | ${v20.longestLowMotionSeconds.toFixed(2)} s | ${current.longestLowMotionSeconds.toFixed(2)} s | ≤ 4.0 s |\n| Average frame change | ${v20.averageFrameChange.toFixed(3)} | ${current.averageFrameChange.toFixed(3)} | references avg ${referenceAverage.toFixed(3)} |\n\nV21 improves proof size, cause-and-effect, one-state payoff, presenter acting, and CTA performance. Automated metrics do not establish reference parity; the human checklist remains blocked.\n`);process.stdout.write(`${JSON.stringify(report,null,2)}\n`);
function average(values){return values.length?values.reduce((sum,value)=>sum+value,0)/values.length:0;}

function scorecardMarkdown(report){
  const metrics=report.metrics.map(({metric,v20,v21,requirement,status})=>`| ${metric} | ${format(v20)} | ${format(v21)} | ${requirement} | ${status} |`).join("\n");
  const gates=report.gates.map(({gate,status})=>`| ${gate} | ${status} |`).join("\n");
  return `# V21 scorecard\n\nParent: V${report.parentVersion} (\`${report.parentMasterSha256}\`)\nMaster: \`${report.masterSha256}\`\nRegression-free: ${report.passed?"yes":"no"}\n\n## Metrics vs parent floors\n\n| Metric | V19 | V21 | Requirement | Status |\n| --- | --- | --- | --- | --- |\n${metrics}\n\n## Render gates\n\n| Gate | Status |\n| --- | --- |\n${gates}\n`;
}
