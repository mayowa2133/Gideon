...
const { compileSolomonV10DemoContent } = require("../dist/main/shared/solomonDemoContentV10.js");
const fixture = compileSolomonV10DemoContent();
const output = path.join(root, "tmp", "solomon-creator-story-v10-performance", "capture");
const productRoot = process.env.SOLOMON_V10_PRODUCT_ROOT ?? path.join(root, "tmp", "solomon-v10-demo-product");
...
if (!productEngineer) throw new Error("V10 capture requires the Product Engineer / Northstar Labs fixture.");
...
if (captures.some(({ failure, bannedPrimaryMatches, disclosureCount }) => failure || bannedPrimaryMatches.length > 0 || disclosureCount !== 1)) {
  throw new Error(`V10 capture failed closed: ${JSON.stringify(receipt, null, 2)}`);
}
