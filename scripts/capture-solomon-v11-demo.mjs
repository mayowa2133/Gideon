...
const { compileSolomonV11DemoContent } = require("../dist/main/shared/solomonDemoContentV11.js");
const fixture = compileSolomonV11DemoContent();
const output = path.join(root, "tmp", "solomon-creator-story-v11-performance", "capture");
const productRoot = process.env.SOLOMON_V11_PRODUCT_ROOT ?? path.join(root, "tmp", "solomon-v11-demo-product");
...
if (!productEngineer) throw new Error("V11 capture requires the Product Engineer / Northstar Labs fixture.");
...
const receipt = {
  schemaVersion: "10.1",
  environment: "fail_closed_fictional_solomon_v11",
  product: "Solomon",
  sourceRevision,
...
if (captures.some(({ failure, bannedPrimaryMatches, disclosureCount }) => failure || bannedPrimaryMatches.length > 0 || disclosureCount !== 1)) {
  throw new Error(`V11 capture failed closed: ${JSON.stringify(receipt, null, 2)}`);
}
