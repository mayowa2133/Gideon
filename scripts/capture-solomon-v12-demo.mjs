...
const { compileSolomonV12DemoContent } = require("../dist/main/shared/solomonDemoContentV12.js");
const fixture = compileSolomonV12DemoContent();
const output = path.join(root, "tmp", "solomon-creator-story-v12-performance", "capture");
const productRoot = process.env.SOLOMON_V12_PRODUCT_ROOT ?? path.join(root, "tmp", "solomon-v12-demo-product");
...
if (!productEngineer) throw new Error("V12 capture requires the Product Engineer / Northstar Labs fixture.");
...
const receipt = {
  schemaVersion: "10.1",
  environment: "fail_closed_fictional_solomon_v12",
  product: "Solomon",
  sourceRevision,
...
if (captures.some(({ failure, bannedPrimaryMatches, disclosureCount }) => failure || bannedPrimaryMatches.length > 0 || disclosureCount !== 1)) {
  throw new Error(`V12 capture failed closed: ${JSON.stringify(receipt, null, 2)}`);
}
