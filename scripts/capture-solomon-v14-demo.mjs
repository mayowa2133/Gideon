...
const { compileSolomonV14DemoContent } = require("../dist/main/shared/solomonDemoContentV14.js");
const fixture = compileSolomonV14DemoContent();
const output = path.join(root, "tmp", "solomon-creator-story-v14-performance", "capture");
const productRoot = process.env.SOLOMON_V14_PRODUCT_ROOT ?? path.join(root, "tmp", "solomon-v14-demo-product");
...
if (!productEngineer) throw new Error("V14 capture requires the Product Engineer / Northstar Labs fixture.");
...
const receipt = {
  schemaVersion: "10.1",
  environment: "fail_closed_fictional_solomon_v14",
  product: "Solomon",
  sourceRevision,
...
if (captures.some(({ failure, bannedPrimaryMatches, disclosureCount }) => failure || bannedPrimaryMatches.length > 0 || disclosureCount !== 1)) {
  throw new Error(`V14 capture failed closed: ${JSON.stringify(receipt, null, 2)}`);
}
