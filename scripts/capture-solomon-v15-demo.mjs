...
const fixture = compileSolomonV15DemoContent();
const output = path.join(root, "tmp", "solomon-creator-story-v15-performance", "capture");
const productRoot = process.env.SOLOMON_V15_PRODUCT_ROOT ?? path.join(root, "tmp", "solomon-v15-demo-product");
...
if (!productEngineer) throw new Error("V15 capture requires the Product Engineer / Northstar Labs fixture.");
...
const receipt = {
  schemaVersion: "10.1",
  environment: "fail_closed_fictional_solomon_v15",
  product: "Solomon",
  sourceRevision,
...
if (captures.some(({ failure, bannedPrimaryMatches, disclosureCount }) => failure || bannedPrimaryMatches.length > 0 || disclosureCount !== 1)) {
  throw new Error(`V15 capture failed closed: ${JSON.stringify(receipt, null, 2)}`);
}
