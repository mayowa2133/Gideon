...
const fixture = compileSolomonV16DemoContent();
const output = path.join(root, "tmp", "solomon-creator-story-v16-performance", "capture");
const productRoot = process.env.SOLOMON_V16_PRODUCT_ROOT ?? path.join(root, "tmp", "solomon-v16-demo-product");
...
if (!productEngineer) throw new Error("V16 capture requires the Product Engineer / Northstar Labs fixture.");
...
const receipt = {
  schemaVersion: "10.1",
  environment: "fail_closed_fictional_solomon_v16",
  product: "Solomon",
  sourceRevision,
...
if (captures.some(({ failure, bannedPrimaryMatches, disclosureCount }) => failure || bannedPrimaryMatches.length > 0 || disclosureCount !== 1)) {
  throw new Error(`V16 capture failed closed: ${JSON.stringify(receipt, null, 2)}`);
}
