...
const fixture = compileSolomonV17DemoContent();
const output = path.join(root, "tmp", "solomon-creator-story-v17-performance", "capture");
const productRoot = process.env.SOLOMON_V17_PRODUCT_ROOT ?? path.join(root, "tmp", "solomon-v17-demo-product");
...
if (!productEngineer) throw new Error("V17 capture requires the Product Engineer / Northstar Labs fixture.");
...
const receipt = {
  schemaVersion: "10.1",
  environment: "fail_closed_fictional_solomon_v17",
  product: "Solomon",
  sourceRevision,
...
if (captures.some(({ failure, bannedPrimaryMatches, disclosureCount }) => failure || bannedPrimaryMatches.length > 0 || disclosureCount !== 1)) {
  throw new Error(`V17 capture failed closed: ${JSON.stringify(receipt, null, 2)}`);
}
