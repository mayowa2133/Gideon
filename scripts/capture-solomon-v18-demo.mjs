...
const fixture = compileSolomonV18DemoContent();
const output = path.join(root, "tmp", "solomon-creator-story-v18-performance", "capture");
const productRoot = process.env.SOLOMON_V18_PRODUCT_ROOT ?? path.join(root, "tmp", "solomon-v18-demo-product");
...
if (!productEngineer) throw new Error("V18 capture requires the Product Engineer / Northstar Labs fixture.");
...
const receipt = {
  schemaVersion: "10.1",
  environment: "fail_closed_fictional_solomon_v18",
  product: "Solomon",
  sourceRevision,
...
if (captures.some(({ failure, bannedPrimaryMatches, disclosureCount }) => failure || bannedPrimaryMatches.length > 0 || disclosureCount !== 1)) {
  throw new Error(`V18 capture failed closed: ${JSON.stringify(receipt, null, 2)}`);
}
