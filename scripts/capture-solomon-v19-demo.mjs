...
const { compileSolomonV19DemoContent } = require("../dist/main/shared/solomonDemoContentV19.js");
const fixture = compileSolomonV19DemoContent();
const output = path.join(root, "tmp", "solomon-creator-story-v19-performance", "capture");
const productRoot = process.env.SOLOMON_V19_PRODUCT_ROOT ?? path.join(root, "tmp", "solomon-v19-demo-product");
...
if (!productEngineer) throw new Error("V19 capture requires the Product Engineer / Northstar Labs fixture.");
...
const receipt = {
  schemaVersion: "10.1",
  environment: "fail_closed_fictional_solomon_v19",
  product: "Solomon",
  sourceRevision,
...
if (captures.some(({ failure, bannedPrimaryMatches, disclosureCount }) => failure || bannedPrimaryMatches.length > 0 || disclosureCount !== 1)) {
  throw new Error(`V19 capture failed closed: ${JSON.stringify(receipt, null, 2)}`);
}
