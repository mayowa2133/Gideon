...
const { compileSolomonV20DemoContent } = require("../dist/main/shared/solomonDemoContentV20.js");
const fixture = compileSolomonV20DemoContent();
const output = path.join(root, "tmp", "solomon-creator-story-v20-performance", "capture");
const productRoot = process.env.SOLOMON_V20_PRODUCT_ROOT ?? path.join(root, "tmp", "solomon-v20-demo-product");
...
if (!productEngineer) throw new Error("V20 capture requires the Product Engineer / Northstar Labs fixture.");
...
const receipt = {
  schemaVersion: "10.1",
  environment: "fail_closed_fictional_solomon_v20",
  product: "Solomon",
  sourceRevision,
...
if (captures.some(({ failure, bannedPrimaryMatches, disclosureCount }) => failure || bannedPrimaryMatches.length > 0 || disclosureCount !== 1)) {
  throw new Error(`V20 capture failed closed: ${JSON.stringify(receipt, null, 2)}`);
}
