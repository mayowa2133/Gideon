...
const fixture = compileSolomonV21DemoContent();
const output = path.join(root, "tmp", "solomon-creator-story-v21-performance", "capture");
const productRoot = process.env.SOLOMON_V21_PRODUCT_ROOT ?? path.join(root, "tmp", "solomon-v21-demo-product");
...
if (!productEngineer) throw new Error("V21 capture requires the Product Engineer / Northstar Labs fixture.");
...
const receipt = {
  schemaVersion: "10.1",
  environment: "fail_closed_fictional_solomon_v21",
  product: "Solomon",
  sourceRevision,
...
if (captures.some(({ failure, bannedPrimaryMatches, disclosureCount }) => failure || bannedPrimaryMatches.length > 0 || disclosureCount !== 1)) {
  throw new Error(`V21 capture failed closed: ${JSON.stringify(receipt, null, 2)}`);
}
