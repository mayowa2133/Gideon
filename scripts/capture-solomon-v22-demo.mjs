...
const { compileSolomonV22DemoContent } = require("../dist/main/shared/solomonDemoContentV22.js");
const fixture = compileSolomonV22DemoContent();
const output = path.join(root, "tmp", "solomon-creator-story-v22-performance", "capture");
const productRoot = process.env.SOLOMON_V22_PRODUCT_ROOT ?? path.join(root, "tmp", "solomon-v22-demo-product");
...
if (!productEngineer) throw new Error("V22 capture requires the Product Engineer / Northstar Labs fixture.");
...
const receipt = {
  schemaVersion: "10.1",
  environment: "fail_closed_fictional_solomon_v22",
  product: "Solomon",
  sourceRevision,
...
if (captures.some(({ failure, bannedPrimaryMatches, disclosureCount }) => failure || bannedPrimaryMatches.length > 0 || disclosureCount !== 1)) {
  throw new Error(`V22 capture failed closed: ${JSON.stringify(receipt, null, 2)}`);
}
