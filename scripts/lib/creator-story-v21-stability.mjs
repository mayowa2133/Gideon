...
// entire point of it. Its sibling, creator-story-v21-motion.mjs, decodes at
// 180x320 and 5 fps — a 36x spatial and 6x temporal decimation — which is
// exactly why no gate through V20 could see the defect V21 exists to fix. The
...
export async function measureV21HeldStability(target, { width = 1080, height = 1920, windows = [], regions = [], windowFrames = DEFAULT_WINDOW_FRAMES } = {}) {
  const samples = [];
  for (const window of windows) {
    ...
    if (frames.length < 2) throw new Error(`V21 stability decode returned ${frames.length} frames for window ${window.id}`);
    samples.push({
      id: window.id,
      fromFrame: from,
      toFrame: to,
...
// first V21 measurement rejected that: the `five` scene scored 44.3 on the
...
// reject V20 (24 distinct wrapper transforms across 24 frames, against V21's 1).
...
export const V21_STABILITY = { minIdenticalPairFraction: .6, controlChurnCeiling: .25 };

export function evaluateV21HeldStability(report, bands = V21_STABILITY) {
  const checks = [];
