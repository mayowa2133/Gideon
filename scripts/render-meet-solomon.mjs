...
async function main() {
  ...
  const passages = [];
  for (let i = 0; i < story.scenes.length; i += 6) passages.push({ id: `passage-${passages.length + 1}`, approvedText: story.scenes.slice(i, i + 6).map(s => s.vo).join(" "), startMs: i * 3000, endMs: (i + 6) * 3000, energy: "high" });
  const narration = await provider.synthesize({ outputDir: voiceDir, beats: passages, language: "en", voice: { mode: "model_default" }, seed: 83026 });
  const trims = narration.beats.map((_, i) => `[${i}:a]silenceremove=start_periods=1:start_threshold=-45dB:start_silence=0.04,areverse,silenceremove=start_periods=1:start_threshold=-45dB:start_silence=0.16,areverse${i < narration.beats.length - 1 ? ",apad=pad_dur=0.25" : ""}[a${i}]`);
  ...
  if (!Number.isFinite(seconds) || seconds < (realInternships || categoriesV2 ? 30 : 35) || seconds > (realInternships ? 40 : categoriesV2 ? 33 : 45)) throw new Error(realInternships ? "Real internship duration must be 30–40 seconds." : categoriesV2 ? "Category V2 duration must be 30–33 seconds." : "Meet pilot duration must be 35–45 seconds.");
  ...
  await ffmpeg(["-i", clean, "-af", `atempo=${tempo},apad,atrim=duration=${seconds},afade=t=out:st=${seconds - .15}:d=0.15`, "-ar", "48000", "-ac", "2", spoken]);
  ...
  const scenes = story.scenes.map((scene, i) => {
    ...
    let actionFrame = 0;
    if ((realInternships || v2 || nontech || internships || categories) && scene.actionCue) {
