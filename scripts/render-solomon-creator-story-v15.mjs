...
import { measureV15Motion } from "./lib/creator-story-v15-motion.mjs";

const require=createRequire(import.meta.url);
const {ChatterboxNarrationProvider}=require("../dist/main/main/chatterboxNarrationProvider.js");
const {SOLOMON_CREATOR_STORY_V15_SCRIPT,SOLOMON_CREATOR_STORY_V15_TTS_BEATS,SOLOMON_CREATOR_STORY_V15_NUMERAL_ANCHORS,assertSolomonCreatorStoryV15Manifest,auditSolomonCreatorStoryV15,createSolomonCreatorStoryV15Manifest}=require("../dist/main/shared/solomonCreatorStoryV15.js");
const {auditV15BannedStrings,auditV15Layout,auditV15PhoneScale,auditV15RenderedBounds,evaluateV15MotionBands}=require("../dist/main/shared/creatorStoryV15Quality.js");
const root=path.resolve(path.dirname(fileURLToPath(import.meta.url)),".."),output=path.join(root,"tmp","solomon-creator-story-v15-performance"),captureDir=process.env.SOLOMON_V15_CAPTURE_DIR??path.join(output,"capture"),publicDir=path.join(output,"remotion-public"),finalDir=path.join(output,"final"),reviewDir=path.join(output,"review"),reportsDir=path.join(output,"reports"),candidatesDir=path.join(output,"candidates"),narrationDir=path.join(output,"narration-candidates");
...
const parentMaster=path.join(root,"tmp","solomon-creator-story-v14-performance","final","solomon-creator-story-v14-performance-master-NOT-FOR-PUBLICATION.mp4"),parentHash=await sha256(parentMaster);
...
let manifest=createSolomonCreatorStoryV15Manifest(inputs,parentHash);assertSolomonCreatorStoryV15Manifest(manifest);
for(const source of manifest.sources){if(!existsSync(source.sourcePath))throw new Error(`V15 source missing: ${source.sourcePath}`);if(await sha256(source.sourcePath)!==source.sourceSha256)throw new Error(`V15 source hash mismatch: ${source.id}`);}

await extractProofs();
const narration=await generateNarration();
manifest=await hydrateMascotAudio(manifest,path.join(publicDir,"narration.wav"));assertSolomonCreatorStoryV15Manifest(manifest);
const soundDesign=await generateSoundDesign();
await writePlanning(narration,soundDesign);

const browserExecutable=resolveBrowser(),serveUrl=await bundle({entryPoint:path.join(root,"src","remotion","solomonCreatorStoryV15","index.ts"),publicDir,outDir:path.join(output,"remotion-bundle"),onProgress:(progress)=>{const value=Math.round((progress>1?progress:progress*100));if(value%25===0)process.stdout.write(`Bundle ${value}%\n`);}}),props=JSON.parse(JSON.stringify(manifest));
const composition=await selectComposition({serveUrl,id:"SolomonCreatorStoryV15Performance",inputProps:props,browserExecutable,chromeMode:"headless-shell",timeoutInMilliseconds:120000});
const intermediate=path.join(output,"solomon-creator-story-v15-remotion.mp4");
...
const master=path.join(finalDir,"solomon-creator-story-v15-performance-master-NOT-FOR-PUBLICATION.mp4"),social=path.join(finalDir,"solomon-creator-story-v15-performance-social-720x1280.mp4"),muted=path.join(finalDir,"solomon-creator-story-v15-performance-muted-review.mp4"),opening=path.join(finalDir,"solomon-creator-story-v15-performance-opening-review.mp4"),mechanism=path.join(finalDir,"solomon-creator-story-v15-performance-mechanism-review.mp4"),payoff=path.join(finalDir,"solomon-creator-story-v15-performance-payoff-cta-review.mp4");
...
await writeFinal(qa);await run("node",[path.join(root,"scripts","analyze-solomon-v15-output.mjs")],600000);
...
async function generateNarration(){
  ...
  if(!runtime)throw new Error("Chatterbox runtime is required for the V15 master.");
  ...
  // manifest instead of hand-typed millisecond windows (the V14 dead-air cause).
  const sceneById=new Map(manifest.scenes.map((scene)=>[scene.id,scene]));
  const beats=SOLOMON_CREATOR_STORY_V15_TTS_BEATS.map((item,index,list)=>{
  ...
  const scriptHash=createHash("sha256").update(SOLOMON_CREATOR_STORY_V15_SCRIPT).digest("hex").slice(0,12);
  const target=path.join(narrationDir,`v15-warm-direct-${scriptHash}.wav`);
  ...
  const report={schemaVersion:"11.1",approvedScript:SOLOMON_CREATOR_STORY_V15_SCRIPT,scriptHash,selectedCandidateId:`v15-warm-direct-${scriptHash}`,selectedTarget:target,sha256:await sha256(target),beats,realizedTimings:realized,humanVoiceApprovalRequired:true,provenance};await writeJson(path.join(reportsDir,"narration-provenance.json"),report);return report;
}
...
async function assembleNarration(result,target){
  ...
  // silently absorbed into stretched silence like V14's fixed adelay windows.
...
function layoutAudit(){const scenes=manifest.scenes.map((scene)=>({sceneId:scene.id,...auditV15Layout(scene.layout)})),collisions=scenes.flatMap((scene)=>scene.collisions.map((collision)=>({...collision,sceneId:scene.sceneId})));return{schemaVersion:"10.1",method:"Declared final-canvas rectangles plus decoded review strips use the V15 forbidden-pair matrix.",scenes,collisions,collisionCount:collisions.length,passed:scenes.every(({passed})=>passed)};}
// The mascot is a persistent layer in V15: it slides between scene anchors and
// must never blink out at a boundary the way the V14 per-scene entrance spring did.
// A giant on-screen numeral with nothing spoken near it is the V14 "5×" defect:
// the graphic survived a script rewrite that dropped the word "five".
async function numeralAnchorAudit(){
  const transcriptDir=path.join(output,"transcript-check"),master=path.join(finalDir,"solomon-creator-story-v15-performance-master-NOT-FOR-PUBLICATION.mp4");
  ...
  const anchors=SOLOMON_CREATOR_STORY_V15_NUMERAL_ANCHORS.map((anchor)=>{
...
async function mascotBoundaryContinuityAudit(){
  const master=path.join(finalDir,"solomon-creator-story-v15-performance-master-NOT-FOR-PUBLICATION.mp4"),boundaries=[];
