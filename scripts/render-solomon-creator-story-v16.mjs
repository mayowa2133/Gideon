...
const {SOLOMON_CREATOR_STORY_V16_SCRIPT,SOLOMON_CREATOR_STORY_V16_TTS_BEATS,SOLOMON_CREATOR_STORY_V16_NUMERAL_ANCHORS,assertSolomonCreatorStoryV16Manifest,auditSolomonCreatorStoryV16,createSolomonCreatorStoryV16Manifest}=require("../dist/main/shared/solomonCreatorStoryV16.js");
const {auditV16BannedStrings,auditV16Layout,auditV16PhoneScale,auditV16RenderedBounds,evaluateV16MotionBands}=require("../dist/main/shared/creatorStoryV16Quality.js");
const root=path.resolve(path.dirname(fileURLToPath(import.meta.url)),".."),output=path.join(root,"tmp","solomon-creator-story-v16-performance"),captureDir=process.env.SOLOMON_V16_CAPTURE_DIR??path.join(output,"capture"),publicDir=path.join(output,"remotion-public"),finalDir=path.join(output,"final"),reviewDir=path.join(output,"review"),reportsDir=path.join(output,"reports"),candidatesDir=path.join(output,"candidates"),narrationDir=path.join(output,"narration-candidates");
...
const parentMaster=path.join(root,"tmp","solomon-creator-story-v15-performance","final","solomon-creator-story-v15-performance-master-NOT-FOR-PUBLICATION.mp4"),parentHash=await sha256(parentMaster);
...
let manifest=createSolomonCreatorStoryV16Manifest(inputs,parentHash);assertSolomonCreatorStoryV16Manifest(manifest);
for(const source of manifest.sources){if(!existsSync(source.sourcePath))throw new Error(`V16 source missing: ${source.sourcePath}`);if(await sha256(source.sourcePath)!==source.sourceSha256)throw new Error(`V16 source hash mismatch: ${source.id}`);}

await extractProofs();
const narration=await generateNarration();
manifest=await hydrateMascotAudio(manifest,path.join(publicDir,"narration.wav"));assertSolomonCreatorStoryV16Manifest(manifest);
const soundDesign=await generateSoundDesign();
await writePlanning(narration,soundDesign);

const browserExecutable=resolveBrowser(),serveUrl=await bundle({entryPoint:path.join(root,"src","remotion","solomonCreatorStoryV16","index.ts"),publicDir,outDir:path.join(output,"remotion-bundle"),onProgress:(progress)=>{const value=Math.round((progress>1?progress:progress*100));if(value%25===0)process.stdout.write(`Bundle ${value}%\n`);}}),props=JSON.parse(JSON.stringify(manifest));
const composition=await selectComposition({serveUrl,id:"SolomonCreatorStoryV16Performance",inputProps:props,browserExecutable,chromeMode:"headless-shell",timeoutInMilliseconds:120000});
const intermediate=path.join(output,"solomon-creator-story-v16-remotion.mp4");
...
const master=path.join(finalDir,"solomon-creator-story-v16-performance-master-NOT-FOR-PUBLICATION.mp4"),social=path.join(finalDir,"solomon-creator-story-v16-performance-social-720x1280.mp4"),muted=path.join(finalDir,"solomon-creator-story-v16-performance-muted-review.mp4"),opening=path.join(finalDir,"solomon-creator-story-v16-performance-opening-review.mp4"),mechanism=path.join(finalDir,"solomon-creator-story-v16-performance-mechanism-review.mp4"),payoff=path.join(finalDir,"solomon-creator-story-v16-performance-payoff-cta-review.mp4");
...
await writeFinal(qa);await run("node",[path.join(root,"scripts","analyze-solomon-v16-output.mjs")],600000);
...
async function generateNarration(){
  ...
  if(!runtime)throw new Error("Chatterbox runtime is required for the V16 master.");
  ...
  // manifest instead of hand-typed millisecond windows (the V15 dead-air cause).
  const sceneById=new Map(manifest.scenes.map((scene)=>[scene.id,scene]));
  const beats=SOLOMON_CREATOR_STORY_V16_TTS_BEATS.map((item,index,list)=>{
  ...
  const scriptHash=createHash("sha256").update(SOLOMON_CREATOR_STORY_V16_SCRIPT).digest("hex").slice(0,12);
  const target=path.join(narrationDir,`v16-warm-direct-${scriptHash}.wav`);
  ...
  const report={schemaVersion:"11.1",approvedScript:SOLOMON_CREATOR_STORY_V16_SCRIPT,scriptHash,selectedCandidateId:`v16-warm-direct-${scriptHash}`,selectedTarget:target,sha256:await sha256(target),beats,realizedTimings:realized,humanVoiceApprovalRequired:true,provenance};await writeJson(path.join(reportsDir,"narration-provenance.json"),report);return report;
}
...
async function assembleNarration(result,target){
  ...
  // silently absorbed into stretched silence like V15's fixed adelay windows.
...
function layoutAudit(){const scenes=manifest.scenes.map((scene)=>({sceneId:scene.id,...auditV16Layout(scene.layout)})),collisions=scenes.flatMap((scene)=>scene.collisions.map((collision)=>({...collision,sceneId:scene.sceneId})));return{schemaVersion:"10.1",method:"Declared final-canvas rectangles plus decoded review strips use the V16 forbidden-pair matrix.",scenes,collisions,collisionCount:collisions.length,passed:scenes.every(({passed})=>passed)};}
// The mascot is a persistent layer in V16: it slides between scene anchors and
// must never blink out at a boundary the way the V15 per-scene entrance spring did.
// A giant on-screen numeral with nothing spoken near it is the V15 "5×" defect:
// the graphic survived a script rewrite that dropped the word "five".
async function numeralAnchorAudit(){
  const transcriptDir=path.join(output,"transcript-check"),master=path.join(finalDir,"solomon-creator-story-v16-performance-master-NOT-FOR-PUBLICATION.mp4");
  ...
  const anchors=SOLOMON_CREATOR_STORY_V16_NUMERAL_ANCHORS.map((anchor)=>{
...
async function mascotBoundaryContinuityAudit(){
  const master=path.join(finalDir,"solomon-creator-story-v16-performance-master-NOT-FOR-PUBLICATION.mp4"),boundaries=[];
