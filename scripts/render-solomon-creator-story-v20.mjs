...
const {SOLOMON_CREATOR_STORY_V20_SCRIPT,SOLOMON_CREATOR_STORY_V20_TTS_BEATS,SOLOMON_CREATOR_STORY_V20_NUMERAL_ANCHORS,assertSolomonCreatorStoryV20Manifest,auditSolomonCreatorStoryV20,createSolomonCreatorStoryV20Manifest}=require("../dist/main/shared/solomonCreatorStoryV20.js");
const {auditV20BannedStrings,auditV20Layout,auditV20PhoneScale,auditV20RenderedBounds,evaluateV20MotionBands,evaluateV20ShotBands}=require("../dist/main/shared/creatorStoryV20Quality.js");
const root=path.resolve(path.dirname(fileURLToPath(import.meta.url)),".."),output=path.join(root,"tmp","solomon-creator-story-v20-performance"),captureDir=process.env.SOLOMON_V20_CAPTURE_DIR??path.join(output,"capture"),publicDir=path.join(output,"remotion-public"),finalDir=path.join(output,"final"),reviewDir=path.join(output,"review"),reportsDir=path.join(output,"reports"),candidatesDir=path.join(output,"candidates"),narrationDir=path.join(output,"narration-candidates");
...
const parentMaster=path.join(root,"tmp","solomon-creator-story-v19-performance","final","solomon-creator-story-v19-performance-master-NOT-FOR-PUBLICATION.mp4"),parentHash=await sha256(parentMaster);
...
let manifest=createSolomonCreatorStoryV20Manifest(inputs,parentHash);assertSolomonCreatorStoryV20Manifest(manifest);
for(const source of manifest.sources){if(!existsSync(source.sourcePath))throw new Error(`V20 source missing: ${source.sourcePath}`);if(await sha256(source.sourcePath)!==source.sourceSha256)throw new Error(`V20 source hash mismatch: ${source.id}`);}

await extractProofs();
const narration=await generateNarration();
manifest=await hydrateMascotAudio(manifest,path.join(publicDir,"narration.wav"));assertSolomonCreatorStoryV20Manifest(manifest);
const soundDesign=await generateSoundDesign();
await writePlanning(narration,soundDesign);

const browserExecutable=resolveBrowser(),serveUrl=await bundle({entryPoint:path.join(root,"src","remotion","solomonCreatorStoryV20","index.ts"),publicDir,outDir:path.join(output,"remotion-bundle"),onProgress:(progress)=>{const value=Math.round((progress>1?progress:progress*100));if(value%25===0)process.stdout.write(`Bundle ${value}%\n`);}}),props=JSON.parse(JSON.stringify(manifest));
const composition=await selectComposition({serveUrl,id:"SolomonCreatorStoryV20Performance",inputProps:props,browserExecutable,chromeMode:"headless-shell",timeoutInMilliseconds:120000});
const intermediate=path.join(output,"solomon-creator-story-v20-remotion.mp4");
...
const master=path.join(finalDir,"solomon-creator-story-v20-performance-master-NOT-FOR-PUBLICATION.mp4"),social=path.join(finalDir,"solomon-creator-story-v20-performance-social-720x1280.mp4"),muted=path.join(finalDir,"solomon-creator-story-v20-performance-muted-review.mp4"),opening=path.join(finalDir,"solomon-creator-story-v20-performance-opening-review.mp4"),mechanism=path.join(finalDir,"solomon-creator-story-v20-performance-mechanism-review.mp4"),payoff=path.join(finalDir,"solomon-creator-story-v20-performance-payoff-cta-review.mp4");
...
await writeFinal(qa);await run("node",[path.join(root,"scripts","analyze-solomon-v20-output.mjs")],600000);
...
async function generateNarration(){
  ...
  if(!runtime)throw new Error("Chatterbox runtime is required for the V20 master.");
  ...
  const beats=SOLOMON_CREATOR_STORY_V20_TTS_BEATS.map((item,index,list)=>{
  ...
  const scriptHash=createHash("sha256").update(SOLOMON_CREATOR_STORY_V20_SCRIPT).digest("hex").slice(0,12);
  const target=path.join(narrationDir,`v20-warm-direct-${scriptHash}.wav`);
  ...
  const report={schemaVersion:"11.1",approvedScript:SOLOMON_CREATOR_STORY_V20_SCRIPT,scriptHash,selectedCandidateId:`v20-warm-direct-${scriptHash}`,selectedTarget:target,sha256:await sha256(target),beats,realizedTimings:realized,humanVoiceApprovalRequired:true,provenance};await writeJson(path.join(reportsDir,"narration-provenance.json"),report);return report;
}
...
function layoutAudit(){const scenes=manifest.scenes.map((scene)=>({sceneId:scene.id,...auditV20Layout(scene.layout)})),collisions=scenes.flatMap((scene)=>scene.collisions.map((collision)=>({...collision,sceneId:scene.sceneId})));return{schemaVersion:"10.1",method:"Declared final-canvas rectangles plus decoded review strips use the V20 forbidden-pair matrix.",scenes,collisions,collisionCount:collisions.length,passed:scenes.every(({passed})=>passed)};}
// The mascot is a persistent layer in V20: it slides between scene anchors and
...
async function numeralAnchorAudit(){
  const transcriptDir=path.join(output,"transcript-check"),master=path.join(finalDir,"solomon-creator-story-v20-performance-master-NOT-FOR-PUBLICATION.mp4");
  ...
  const anchors=SOLOMON_CREATOR_STORY_V20_NUMERAL_ANCHORS.map((anchor)=>{
...
async function mascotBoundaryContinuityAudit(){
  const master=path.join(finalDir,"solomon-creator-story-v20-performance-master-NOT-FOR-PUBLICATION.mp4"),boundaries=[];
