...
import { measureV21Motion } from "./lib/creator-story-v21-motion.mjs";
import { evaluateV21HeldStability, measureV21HeldStability } from "./lib/creator-story-v21-stability.mjs";

const require=createRequire(import.meta.url);
const {ChatterboxNarrationProvider}=require("../dist/main/main/chatterboxNarrationProvider.js");
const {SOLOMON_CREATOR_STORY_V21_SCRIPT,SOLOMON_CREATOR_STORY_V21_TTS_BEATS,SOLOMON_CREATOR_STORY_V21_NUMERAL_ANCHORS,assertSolomonCreatorStoryV21Manifest,auditSolomonCreatorStoryV21,createSolomonCreatorStoryV21Manifest}=require("../dist/main/shared/solomonCreatorStoryV21.js");
const {auditV21BannedStrings,auditV21Layout,auditV21PhoneScale,auditV21RenderedBounds,evaluateV21MotionBands,evaluateV21ShotBands,mascotBoxForScene}=require("../dist/main/shared/creatorStoryV21Quality.js");
...
const root=path.resolve(path.dirname(fileURLToPath(import.meta.url)),".."),output=path.join(root,"tmp","solomon-creator-story-v21-performance"),captureDir=process.env.SOLOMON_V21_CAPTURE_DIR??path.join(output,"capture"),publicDir=path.join(output,"remotion-public"),finalDir=path.join(output,"final"),reviewDir=path.join(output,"review"),reportsDir=path.join(output,"reports"),candidatesDir=path.join(output,"candidates"),narrationDir=path.join(output,"narration-candidates");
...
const parentMaster=path.join(root,"tmp","solomon-creator-story-v20-performance","final","solomon-creator-story-v20-performance-master-NOT-FOR-PUBLICATION.mp4"),parentHash=await sha256(parentMaster);
...
let manifest=createSolomonCreatorStoryV21Manifest(inputs,parentHash);assertSolomonCreatorStoryV21Manifest(manifest);
for(const source of manifest.sources){if(!existsSync(source.sourcePath))throw new Error(`V21 source missing: ${source.sourcePath}`);if(await sha256(source.sourcePath)!==source.sourceSha256)throw new Error(`V21 source hash mismatch: ${source.id}`);}

await extractProofs();
const narration=await generateNarration();
manifest=await hydrateMascotAudio(manifest,path.join(publicDir,"narration.wav"));assertSolomonCreatorStoryV21Manifest(manifest);
const soundDesign=await generateSoundDesign();
await writePlanning(narration,soundDesign);

const browserExecutable=resolveBrowser(),serveUrl=await bundle({entryPoint:path.join(root,"src","remotion","solomonCreatorStoryV21","index.ts"),publicDir,outDir:path.join(output,"remotion-bundle"),onProgress:(progress)=>{const value=Math.round((progress>1?progress:progress*100));if(value%25===0)process.stdout.write(`Bundle ${value}%\n`);}}),props=JSON.parse(JSON.stringify(manifest));
const composition=await selectComposition({serveUrl,id:"SolomonCreatorStoryV21Performance",inputProps:props,browserExecutable,chromeMode:"headless-shell",timeoutInMilliseconds:120000});
const intermediate=path.join(output,"solomon-creator-story-v21-remotion.mp4");
...
const master=path.join(finalDir,"solomon-creator-story-v21-performance-master-NOT-FOR-PUBLICATION.mp4"),social=path.join(finalDir,"solomon-creator-story-v21-performance-social-720x1280.mp4"),muted=path.join(finalDir,"solomon-creator-story-v21-performance-muted-review.mp4"),opening=path.join(finalDir,"solomon-creator-story-v21-performance-opening-review.mp4"),mechanism=path.join(finalDir,"solomon-creator-story-v21-performance-mechanism-review.mp4"),payoff=path.join(finalDir,"solomon-creator-story-v21-performance-payoff-cta-review.mp4");
...
await writeFinal(qa);await run("node",[path.join(root,"scripts","analyze-solomon-v21-output.mjs")],600000);
...
async function generateNarration(){
  ...
  if(!runtime)throw new Error("Chatterbox runtime is required for the V21 master.");
  ...
  const beats=SOLOMON_CREATOR_STORY_V21_TTS_BEATS.map((item,index,list)=>{
  ...
  const scriptHash=createHash("sha256").update(SOLOMON_CREATOR_STORY_V21_SCRIPT).digest("hex").slice(0,12);
  const target=path.join(narrationDir,`v21-warm-direct-${scriptHash}.wav`);
  ...
  const report={schemaVersion:"11.1",approvedScript:SOLOMON_CREATOR_STORY_V21_SCRIPT,scriptHash,selectedCandidateId:`v21-warm-direct-${scriptHash}`,selectedTarget:target,sha256:await sha256(target),beats,realizedTimings:realized,humanVoiceApprovalRequired:true,provenance};await writeJson(path.join(reportsDir,"narration-provenance.json"),report);return report;
}
...
// Held-element stability. This is the gate V21 exists to add, and the reason it
// is measured here rather than folded into measureV21Motion is that the motion
...
function layoutAudit(){const scenes=manifest.scenes.map((scene)=>({sceneId:scene.id,...auditV21Layout(scene.layout)})),collisions=scenes.flatMap((scene)=>scene.collisions.map((collision)=>({...collision,sceneId:scene.sceneId})));return{schemaVersion:"10.1",method:"Declared final-canvas rectangles plus decoded review strips use the V21 forbidden-pair matrix.",scenes,collisions,collisionCount:collisions.length,passed:scenes.every(({passed})=>passed)};}
// The mascot is a persistent layer in V21: it slides between scene anchors and
...
async function numeralAnchorAudit(){
  const transcriptDir=path.join(output,"transcript-check"),master=path.join(finalDir,"solomon-creator-story-v21-performance-master-NOT-FOR-PUBLICATION.mp4");
  ...
  const anchors=SOLOMON_CREATOR_STORY_V21_NUMERAL_ANCHORS.map((anchor)=>{
...
async function mascotBoundaryContinuityAudit(){
  const master=path.join(finalDir,"solomon-creator-story-v21-performance-master-NOT-FOR-PUBLICATION.mp4"),boundaries=[];
