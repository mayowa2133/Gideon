...
// Scene content starts here, so a drawn caption band must clear it.
const CAPTION_CONTENT_CEILING=275;
...
function sceneSampleFrames(){
  // Two samples per scene, because one is not enough to see a scene that changes
  // what it says. `result` displays "ONE CONNECTED NEXT STEP" in full at local
  // frame 5 and only "ONE" by local 16; a single 55% sample landed on the latter
  // and reported the phrase missing when the film shows it plainly. Both samples
  // settle past the post-cut slide and stop before the outgoing transition.
  const frames=[];
  for(const scene of manifest.scenes){
    const length=scene.to-scene.from;
    const at=(fraction,floorFrames)=>Math.min(scene.to-1,scene.from+Math.min(Math.max(floorFrames,Math.floor(length*fraction)),Math.max(1,length-3)));
    const early=at(.2,6),late=at(.65,10);
    frames.push(early);
    if(late!==early)frames.push(late);
  }
  return frames;
}
...
// Caption collisions, measured rather than asserted.
//
// This was `captionCollisionCount: 0` -- a literal handed to the gate whose job
// is to notice captions colliding with things. Every caption defect this project
// hit was found by eye or by a one-off count, and none of them could have been
// found here, because the answer was supplied.
//
// Three kinds, all checkable without a render:
//
//   declared   the layout audit already forbids caption x product and
//              caption x mascot, so its caption collisions are counted here too
//              rather than tracked separately.
//   overlap    two caption windows live at once. This happened: alignment
//              rewrote caption timings and two groups ran together, and it took
//              an explicit overlap count to see it because all 32 gates passed.
//   band       the drawn caption band crossing into scene content. Captions are
//              drawn at absolute pixels -- top 86 at 116px for emphasis, 116 at
//              80px otherwise -- so the declared rect cannot see this. Scene
//              content starts at ~275, which is the ceiling the band must clear.
function captionCollisionAudit(){
  const overlaps=[];
  const windows=[...manifest.captions].sort((left,right)=>left.from-right.from);
  for(let index=1;index<windows.length;index+=1){
    const previous=windows[index-1],current=windows[index];
    if(current.from<previous.to)overlaps.push({left:previous.id,right:current.id,frames:previous.to-current.from});
  }
  const bandBreaches=[];
  for(const caption of manifest.captions){
    for(const group of caption.wordGroups??[]){
      const top=group.emphasis?86:116,bottom=top+Math.round((group.emphasis?116:80)*1.2);
      if(bottom>CAPTION_CONTENT_CEILING)bandBreaches.push({caption:caption.id,group:group.text,bottom});
    }
  }
  const declared=manifest.scenes.flatMap((scene)=>auditV22Layout(scene.layout).collisions
    .filter(({left,right})=>left.startsWith("caption")||right.startsWith("caption"))
    .map((collision)=>({sceneId:scene.id,...collision})));
  const collisionCount=overlaps.length+bandBreaches.length+declared.length;
