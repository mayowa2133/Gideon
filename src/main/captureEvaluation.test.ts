...
describe("discovery provider evaluation", () => {
  it("gates provider rollout on recall and invalid-output thresholds", async () => {
    const bundle = createDiscoveryEvidenceBundle({ environmentVersionId: "version-1", projectId: "project-1", goals: [{ id: "goal-1", text: "Open projects", priority: 100 }], personas: [{ id: "persona-1", key: "founder", displayName: "Founder", roleDescription: "Founder role" }], renderedPages: [{ id: "page-1", url: "/projects", title: "Projects", controls: [], accessibleTreeHash: "a".repeat(64), domStructureHash: "b".repeat(64) }], allowedRisks: ["observe", "navigate"], maxCandidates: 10 });
