...
export function createCapturePreviewService(options: { repository: CapturePreviewRepository; signer: CapturePreviewSigner; expiresInSeconds?: number }) {
  ...
  return {
    async create(input: { workspaceId: string; projectId: string; executionId: string }) {
      ...
      if (!execution || execution.workspaceId !== input.workspaceId || execution.projectId !== input.projectId || execution.status !== "verified" || !execution.normalizedClipArtifactId) throw new Error("Verified capture clip was not found.");
      ...
      if (!artifact || artifact.workspaceId !== input.workspaceId || artifact.projectId !== input.projectId || artifact.kind !== "normalized_flow_clip") throw new Error("Verified capture clip was not found.");
