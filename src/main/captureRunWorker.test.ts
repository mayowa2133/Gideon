...
describe("capture run worker", () => {
  ...
  it("records, normalizes, assembles, stores lineage, and activates a verified source", async () => {
    ...
    expect(repo.executions.at(-1)).toMatchObject({ status: "verified", rawCaptureArtifactId: expect.any(String), normalizedClipArtifactId: expect.any(String) });
    expect(stored.kinds).toEqual(["verification_receipt", "raw_browser_capture", "normalized_flow_clip", "action_telemetry", "source_recording", "capture_assembly_manifest"]);
