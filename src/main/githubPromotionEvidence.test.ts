...
describe("GitHub promotion evidence artifact check", () => {
  it("prints the artifact verification plan in dry-run mode", async () => {
    ...
    expect(result.stdout).toContain("write a safe verification receipt with SHA-256 artifact digests, byte sizes");
  });
  ...
  it("writes a safe verification receipt for archived release evidence", async () => {
    ...
    const receipt = JSON.parse(await fs.readFile(receiptPath, "utf8")) as {
      repository: string;
      runId: string;
      evidence: { gitCommit: string; stepCount: number; sizeBytes: number; sha256: string };
      providerCanaryReport: { mode: string; capabilityCount: number; capabilities: string[]; sizeBytes: number; sha256: string };
      releaseReceipt: {
        product: string;
        channel: string;
        notarizationStatus: string;
        installSmokeResult: string;
        sizeBytes: number;
        sha256: string;
      };
    ...
    expect(receipt.providerCanaryReport.sizeBytes).toBeGreaterThan(0);
    ...
    expect(receipt.releaseReceipt.sizeBytes).toBeGreaterThan(0);
