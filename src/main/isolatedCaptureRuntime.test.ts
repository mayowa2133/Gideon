...
import { createIsolatedCaptureRuntime, verifyIsolatedCaptureManifest, type IsolatedCaptureManifest } from "./isolatedCaptureRuntime";
...
describe("isolated capture runtime boundary", () => {
  it("sends a hashed declarative manifest with no secret callbacks", async () => {
    ...
    expect(manifest).toMatchObject({ schemaVersion: "2", outputHandle: "capture-output:execution-1", fixtureGrantId: "fixture:grant-1", fixtureKeys: ["project_name"], runtimePolicyVersion: "capture-browser-isolation-v1", runtimePolicyHash: "b".repeat(64), capturePacing: { afterActionMs: 750 }, capturePresentation: { showPointer: true, pointerMoveMs: 300, typingDelayMs: 35 }, maskingPolicy: { categories: ["password", "token", "payment", "email", "personal_data", "canvas"] }, manifestHash: expect.stringMatching(/^[a-f0-9]{64}$/) });
  ...
  it("rejects credential-like fixture keys before remote execution", async () => {
    const plan = compiledPlan();
    const runtime = createIsolatedCaptureRuntime({ isolation: "microvm", expectedImageDigest: `sha256:${"d".repeat(64)}`, ...policyIdentity(), async execute() { throw new Error("must not call"); } });
  ...
  it("requires opaque fixture grants and revokes staged fixture data after worker failure", async () => {
    ...
    const runtime = createIsolatedCaptureRuntime({ isolation: "container", expectedImageDigest: `sha256:${"d".repeat(64)}`, ...policyIdentity(), async prepareFixtureGrant() { return { grantId: "fixture:temporary-1" }; }, async revokeFixtureGrant(input) { revoked.push(input.grantId); }, async execute() { throw new Error("worker unavailable"); } });
    ...
    const unsafe = createIsolatedCaptureRuntime({ isolation: "container", expectedImageDigest: `sha256:${"d".repeat(64)}`, ...policyIdentity(), async prepareFixtureGrant() { return { grantId: "fixture:token-secret" }; }, async revokeFixtureGrant() {}, async execute() { throw new Error("must not execute"); } });
  ...
  it("rejects a response that is not tied to the submitted manifest and pinned image", async () => {
    const plan = compiledPlan();
    const imageDigest = `sha256:${"d".repeat(64)}` as const;
    const runtime = createIsolatedCaptureRuntime({ isolation: "container", expectedImageDigest: imageDigest, ...policyIdentity(), async execute(input) { return { result: { receipt: receipt(plan), maskingReceipt: maskingReceipt(), networkReceipts: [] }, attestation: { ...attestation(input, "container", imageDigest), manifestHash: "a".repeat(64) } }; } });
  ...
  it("rejects tampered worker manifests and incomplete runtime teardown", async () => {
    const plan = compiledPlan();
    const imageDigest = `sha256:${"d".repeat(64)}` as const;
    let captured: IsolatedCaptureManifest | undefined;
    const runtime = createIsolatedCaptureRuntime({ isolation: "container", expectedImageDigest: imageDigest, ...policyIdentity(), async execute(input) {
      captured = input;
      return { result: { receipt: receipt(plan), maskingReceipt: maskingReceipt(), networkReceipts: [] }, attestation: { ...attestation(input, "container", imageDigest), cleanup: { ...attestation(input, "container", imageDigest).cleanup, runtimeInstance: "pending" as never } } };
    } });
    await expect(runtime.execute({ id: "execution-1", workspaceId: "workspace-1", plan, policy: policy(), fixtureValues: {}, outputDir: "/ignored", recordVideo: false })).rejects.toThrow("teardown is incomplete");
    expect(captured).toBeDefined();
    expect(() => verifyIsolatedCaptureManifest({ ...captured!, workspaceId: "workspace-2" })).toThrow("manifest hash is invalid");
  });
  ...
  it("rejects missing or mismatched remote masking evidence", async () => {
    const plan = compiledPlan();
    const imageDigest = `sha256:${"d".repeat(64)}` as const;
    const runtime = createIsolatedCaptureRuntime({ isolation: "container", expectedImageDigest: imageDigest, ...policyIdentity(), async execute(input) { return { result: { receipt: receipt(plan), maskingReceipt: { ...maskingReceipt(), policyHash: "a".repeat(64) }, networkReceipts: [] }, attestation: attestation(input, "container", imageDigest) }; } });
...
function policyIdentity() { return { expectedRuntimePolicyVersion: "capture-browser-isolation-v1", expectedRuntimePolicyHash: "b".repeat(64) }; }
function attestation(manifest: IsolatedCaptureManifest, isolation: "container" | "microvm", imageDigest: `sha256:${string}`) { return { schemaVersion: "2" as const, manifestHash: manifest.manifestHash, workspaceId: manifest.workspaceId, executionId: manifest.executionId, isolation, runtimeInstanceId: "worker-1", imageDigest, runtimePolicyVersion: manifest.runtimePolicyVersion, runtimePolicyHash: manifest.runtimePolicyHash, startedAt: "2026-07-14T10:00:00.000Z", completedAt: "2026-07-14T10:00:01.000Z", terminalState: "succeeded" as const, cleanup: { browserProfile: "destroyed" as const, cookies: "destroyed" as const, clipboard: "cleared" as const, cache: "destroyed" as const, scratch: "destroyed" as const, runtimeInstance: "destroyed" as const } }; }
