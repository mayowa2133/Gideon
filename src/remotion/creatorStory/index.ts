...
// noticed, because a fallback font renders perfectly happily. The creator-story bundle
...
void (async () => {
  ...
  if (missing.length > 0) {
    throw new Error(`Creator story typography: no face loaded for ${missing.join(", ")}. The film would render in the fallback serif.`);
  }
