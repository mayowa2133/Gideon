...
import { auditSolomonMascotV11, SOLOMON_MASCOT_V11_GEOMETRY, v11FaceSchema, v11GestureSchema, v11MascotPerformanceSchema, type V11Face, type V11Gesture, type V11MascotPerformance } from "../../shared/solomonMascotV11";
import { RobotMascotV11Rig } from "./RobotMascotV11Rig";
...
function plan(overrides: Partial<V11MascotPerformance> = {}): V11MascotPerformance {
  return v11MascotPerformanceSchema.parse({
    sceneId: "snapshot",
    role: "host",
...
const render = (performance: V11MascotPerformance, frame = 14) => renderToString(<RobotMascotV11Rig plan={performance} frame={frame} fps={30} enterOverride={1} />);
const withGesture = (gesture: V11Gesture, overrides: Partial<V11MascotPerformance> = {}) => plan({ left: { gesture, timing: { start: 5, peak: 11, recover: 20, wristRotation: 14 } }, ...overrides });

describe("RobotMascotV11 character golden masters", () => {
  it("renders every resting face state", () => {
    for (const face of v11FaceSchema.options.filter((option): option is V11Face => option !== "surprised")) {
  ...
  it("renders the surprised accent only inside its hold window", () => {
    ...
    expect(render(accented, 4)).toContain('data-v11-face="surprised"');
    expect(render(accented, 12)).toContain('data-v11-face="happy"');
    expect(render(accented, 4)).toMatchSnapshot("face-surprised-accent");
  });

  it("renders every gesture silhouette", () => {
    for (const gesture of v11GestureSchema.options as V11Gesture[]) {
  ...
  it("keeps mitts at cameo scale and fingers only at host scale", () => {
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).not.toContain('data-v11-hand="palm-fingers"');
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).toContain('data-v11-hand="mitt"');
    expect(render(withGesture("open_palm", { role: "host" }))).toContain('data-v11-hand="palm-fingers"');
  });

  it("draws a neck that closes the head-to-body seam", () => {
    expect(render(plan())).toContain("data-v11-neck");
    const neckTop = 482, neckBottom = 482 + 60, headBottom = 48 + SOLOMON_MASCOT_V11_GEOMETRY.headHeight, torsoTop = 720 - 195;
  ...
  it("consumes the shared geometry spec rather than private literals", () => {
    ...
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V11_GEOMETRY.headWidth}"`);
    expect(markup).toContain(`height="${SOLOMON_MASCOT_V11_GEOMETRY.headHeight}"`);
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V11_GEOMETRY.screenWidth}"`);
    expect(markup).toContain(SOLOMON_MASCOT_V11_GEOMETRY.faceColor);
    expect(auditSolomonMascotV11().passed).toBe(true);
  });
});
