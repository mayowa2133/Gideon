...
import { auditSolomonMascotV12, SOLOMON_MASCOT_V12_GEOMETRY, v12FaceSchema, v12GestureSchema, v12MascotPerformanceSchema, type V12Face, type V12Gesture, type V12MascotPerformance } from "../../shared/solomonMascotV12";
import { RobotMascotV12Rig } from "./RobotMascotV12Rig";
...
function plan(overrides: Partial<V12MascotPerformance> = {}): V12MascotPerformance {
  return v12MascotPerformanceSchema.parse({
    sceneId: "snapshot",
    role: "host",
...
const render = (performance: V12MascotPerformance, frame = 14) => renderToString(<RobotMascotV12Rig plan={performance} frame={frame} fps={30} enterOverride={1} />);
const withGesture = (gesture: V12Gesture, overrides: Partial<V12MascotPerformance> = {}) => plan({ left: { gesture, timing: { start: 5, peak: 11, recover: 20, wristRotation: 14 } }, ...overrides });

describe("RobotMascotV12 character golden masters", () => {
  it("renders every resting face state", () => {
    for (const face of v12FaceSchema.options.filter((option): option is V12Face => option !== "surprised")) {
  ...
  it("renders the surprised accent only inside its hold window", () => {
    ...
    expect(render(accented, 4)).toContain('data-v12-face="surprised"');
    expect(render(accented, 12)).toContain('data-v12-face="happy"');
    expect(render(accented, 4)).toMatchSnapshot("face-surprised-accent");
  });

  it("renders every gesture silhouette", () => {
    for (const gesture of v12GestureSchema.options as V12Gesture[]) {
  ...
  it("keeps mitts at cameo scale and fingers only at host scale", () => {
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).not.toContain('data-v12-hand="palm-fingers"');
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).toContain('data-v12-hand="mitt"');
    expect(render(withGesture("open_palm", { role: "host" }))).toContain('data-v12-hand="palm-fingers"');
  });

  it("draws a neck that closes the head-to-body seam", () => {
    expect(render(plan())).toContain("data-v12-neck");
    const neckTop = 482, neckBottom = 482 + 60, headBottom = 48 + SOLOMON_MASCOT_V12_GEOMETRY.headHeight, torsoTop = 720 - 195;
  ...
  it("consumes the shared geometry spec rather than private literals", () => {
    ...
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V12_GEOMETRY.headWidth}"`);
    expect(markup).toContain(`height="${SOLOMON_MASCOT_V12_GEOMETRY.headHeight}"`);
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V12_GEOMETRY.screenWidth}"`);
    expect(markup).toContain(SOLOMON_MASCOT_V12_GEOMETRY.faceColor);
    expect(auditSolomonMascotV12().passed).toBe(true);
  });
});
