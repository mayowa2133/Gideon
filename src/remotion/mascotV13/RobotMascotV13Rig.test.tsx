...
import { auditSolomonMascotV13, SOLOMON_MASCOT_V13_GEOMETRY, v13FaceSchema, v13GestureSchema, v13MascotPerformanceSchema, type V13Face, type V13Gesture, type V13MascotPerformance } from "../../shared/solomonMascotV13";
import { RobotMascotV13Rig } from "./RobotMascotV13Rig";
...
function plan(overrides: Partial<V13MascotPerformance> = {}): V13MascotPerformance {
  return v13MascotPerformanceSchema.parse({
    sceneId: "snapshot",
    role: "host",
...
const render = (performance: V13MascotPerformance, frame = 14) => renderToString(<RobotMascotV13Rig plan={performance} frame={frame} fps={30} enterOverride={1} />);
const withGesture = (gesture: V13Gesture, overrides: Partial<V13MascotPerformance> = {}) => plan({ left: { gesture, timing: { start: 5, peak: 11, recover: 20, wristRotation: 14 } }, ...overrides });

describe("RobotMascotV13 character golden masters", () => {
  it("renders every resting face state", () => {
    for (const face of v13FaceSchema.options.filter((option): option is V13Face => option !== "surprised")) {
  ...
  it("renders the surprised accent only inside its hold window", () => {
    ...
    expect(render(accented, 4)).toContain('data-v13-face="surprised"');
    expect(render(accented, 12)).toContain('data-v13-face="happy"');
    expect(render(accented, 4)).toMatchSnapshot("face-surprised-accent");
  });

  it("renders every gesture silhouette", () => {
    for (const gesture of v13GestureSchema.options as V13Gesture[]) {
  ...
  it("keeps mitts at cameo scale and fingers only at host scale", () => {
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).not.toContain('data-v13-hand="palm-fingers"');
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).toContain('data-v13-hand="mitt"');
    expect(render(withGesture("open_palm", { role: "host" }))).toContain('data-v13-hand="palm-fingers"');
  });

  it("draws a neck that closes the head-to-body seam", () => {
    expect(render(plan())).toContain("data-v13-neck");
    const neckTop = 482, neckBottom = 482 + 60, headBottom = 48 + SOLOMON_MASCOT_V13_GEOMETRY.headHeight, torsoTop = 720 - 195;
  ...
  it("consumes the shared geometry spec rather than private literals", () => {
    ...
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V13_GEOMETRY.headWidth}"`);
    expect(markup).toContain(`height="${SOLOMON_MASCOT_V13_GEOMETRY.headHeight}"`);
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V13_GEOMETRY.screenWidth}"`);
    expect(markup).toContain(SOLOMON_MASCOT_V13_GEOMETRY.faceColor);
    expect(auditSolomonMascotV13().passed).toBe(true);
  });
});
