...
import { auditSolomonMascotV14, SOLOMON_MASCOT_V14_GEOMETRY, v14FaceSchema, v14GestureSchema, v14MascotPerformanceSchema, type V14Face, type V14Gesture, type V14MascotPerformance } from "../../shared/solomonMascotV14";
import { RobotMascotV14Rig } from "./RobotMascotV14Rig";
...
function plan(overrides: Partial<V14MascotPerformance> = {}): V14MascotPerformance {
  return v14MascotPerformanceSchema.parse({
    sceneId: "snapshot",
    role: "host",
...
const render = (performance: V14MascotPerformance, frame = 14) => renderToString(<RobotMascotV14Rig plan={performance} frame={frame} fps={30} enterOverride={1} />);
const withGesture = (gesture: V14Gesture, overrides: Partial<V14MascotPerformance> = {}) => plan({ left: { gesture, timing: { start: 5, peak: 11, recover: 20, wristRotation: 14 } }, ...overrides });

describe("RobotMascotV14 character golden masters", () => {
  it("renders every resting face state", () => {
    for (const face of v14FaceSchema.options.filter((option): option is V14Face => option !== "surprised")) {
  ...
  it("renders the surprised accent only inside its hold window", () => {
    ...
    expect(render(accented, 4)).toContain('data-v14-face="surprised"');
    expect(render(accented, 12)).toContain('data-v14-face="happy"');
    expect(render(accented, 4)).toMatchSnapshot("face-surprised-accent");
  });

  it("renders every gesture silhouette", () => {
    for (const gesture of v14GestureSchema.options as V14Gesture[]) {
  ...
  it("keeps mitts at cameo scale and fingers only at host scale", () => {
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).not.toContain('data-v14-hand="palm-fingers"');
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).toContain('data-v14-hand="mitt"');
    expect(render(withGesture("open_palm", { role: "host" }))).toContain('data-v14-hand="palm-fingers"');
  });

  it("draws a neck that closes the head-to-body seam", () => {
    expect(render(plan())).toContain("data-v14-neck");
    const neckTop = 482, neckBottom = 482 + 60, headBottom = 48 + SOLOMON_MASCOT_V14_GEOMETRY.headHeight, torsoTop = 720 - 195;
  ...
  it("consumes the shared geometry spec rather than private literals", () => {
    ...
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V14_GEOMETRY.headWidth}"`);
    expect(markup).toContain(`height="${SOLOMON_MASCOT_V14_GEOMETRY.headHeight}"`);
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V14_GEOMETRY.screenWidth}"`);
    expect(markup).toContain(SOLOMON_MASCOT_V14_GEOMETRY.faceColor);
    expect(auditSolomonMascotV14().passed).toBe(true);
  });
});
