import { SOLOMON_MASCOT_V14_GEOMETRY, sampleMascotAudioV14, type V14Face, type V14Gesture, type V14MascotPerformance, type V14Mouth } from "../../shared/solomonMascotV14";
...
const { faceColor: MINT, visorColor: INK, orangeAccent: AMBER, headWidth: HEAD_W, headHeight: HEAD_H, screenWidth: SCREEN_W, screenHeight: SCREEN_H, screenCornerRadius: SCREEN_R } = SOLOMON_MASCOT_V14_GEOMETRY;
...
export const RobotMascotV14Rig: React.FC<{ plan: V14MascotPerformance; frame: number; fps?: number; enterOverride?: number; positioning?: "self" | "external" }> = ({ plan, frame, enterOverride, positioning = "self" }) => {
  ...
  const audio = sampleMascotAudioV14(plan.audioFrames, frame), mouth = mouthFor(face, plan.mouthBias, audio);
  ...
  return <div data-v14-mascot={plan.sceneId} data-v14-face={face} data-v14-left={plan.left.gesture} data-v14-right={plan.right.gesture} style={{ position: "absolute", width: 660, height: 940, zIndex: 32, ...layout, opacity: enter, transform: `${layout.transform ?? ""} translate(${hoverX + torsoGaze.dx * lean}px,${hoverY + (1 - enter) * 50 - recoil}px) rotate(${rotation}deg)`, transformOrigin: "50% 78%" }}>
    <svg viewBox="0 0 660 940" width="660" height="940" style={{ overflow: "visible", filter: "drop-shadow(0 34px 45px rgba(3,9,20,.24))" }}>
      ...
      <rect data-v14-neck x="290" y="482" width="80" height="60" rx="26" fill={`url(#${plan.sceneId}-body)`} stroke="#fff" strokeWidth="6" />
...
function activeFace(plan: V14MascotPerformance, frame: number): V14Face {
...
const Face: React.FC<{ face: V14Face; gazeX: number; gazeY: number; blink: number }> = ({ face, gazeX, gazeY, blink }) => {
...
const Mouth: React.FC<{ state: V14Mouth }> = ({ state }) => {
...
const Arm: React.FC<{ side: "left" | "right"; gesture: V14Gesture; amount: number; wristRotation: number; detail: HandDetail }> = ({ side, gesture, amount, wristRotation, detail }) => {
  ...
  return <g data-v14-gesture={`${side}-${gesture}`}>
...
const HandShape: React.FC<{ gesture: V14Gesture; side: "left" | "right"; detail: HandDetail }> = ({ gesture, side, detail }) => {
  ...
  if (gesture.startsWith("true_point")) return <g transform={`scale(${flip} 1)`} data-v14-hand="mitt-point"><ellipse rx="48" ry="48" fill="#f8fbfd" stroke="#acbed0" strokeWidth="7" /><path d="M18 -22 L94 -54 L38 6 Z" fill="#f8fbfd" stroke="#acbed0" strokeWidth="6" strokeLinejoin="round" /></g>;
  if (gesture === "point_down" || gesture === "bookmark_tap") return <g data-v14-hand="mitt-point-down"><ellipse rx="48" ry="48" fill="#f8fbfd" stroke="#acbed0" strokeWidth="7" /><path d="M-12 32 L10 106 L26 34 Z" fill="#f8fbfd" stroke="#acbed0" strokeWidth="6" strokeLinejoin="round" /></g>;
  if (gesture === "approval") return <g transform={`scale(${flip} 1)`} data-v14-hand="mitt-thumb"><ellipse rx="50" ry="46" fill="#f8fbfd" stroke="#acbed0" strokeWidth="7" /><path d="M-4 -30 L40 -88 L26 -18 Z" fill="#f8fbfd" stroke="#acbed0" strokeWidth="6" strokeLinejoin="round" /></g>;
  if (gesture === "bookmark_hold") return <g data-v14-hand="bookmark"><path d="M-46 -58 H46 V64 L0 36 L-46 64Z" fill={INK} stroke={MINT} strokeWidth="9" /><ellipse cx="-52" cy="5" rx="24" ry="38" fill="#f8fbfd" stroke="#acbed0" strokeWidth="6" /><ellipse cx="52" cy="5" rx="24" ry="38" fill="#f8fbfd" stroke="#acbed0" strokeWidth="6" /></g>;
  if (gesture === "thinking_hand") return <g transform="rotate(-24)" data-v14-hand="mitt-tilt"><ellipse rx="48" ry="53" fill="#f8fbfd" stroke="#acbed0" strokeWidth="7" /></g>;
  if (gesture === "open_palm" || gesture === "presentation_palm" || gesture === "stop_palm" || gesture === "wave" || gesture === "celebration") {
    ...
    if (detail === "mitt") return <g transform={`rotate(${rotate})`} data-v14-hand="mitt"><ellipse rx="52" ry="56" fill="#f8fbfd" stroke="#acbed0" strokeWidth="7" /></g>;
    return <g transform={`rotate(${rotate})`} data-v14-hand="palm-fingers"><ellipse rx="52" ry="56" fill="#f8fbfd" stroke="#acbed0" strokeWidth="7" />{[-34, -12, 12, 34].map((x, index) => <path key={x} d={`M${x} -32 L${x + (index - 1.5) * 3} -${82 - Math.abs(index - 1.5) * 8}`} stroke="#f8fbfd" strokeWidth="18" strokeLinecap="round" />)}</g>;
  }
  return <ellipse rx="52" ry="54" fill="#f8fbfd" stroke="#acbed0" strokeWidth="7" data-v14-hand="rest" />;
};

function gestureTarget(gesture: V14Gesture, side: "left" | "right") {
...
function limbCurve(frame: number, timing: V14MascotPerformance["left"]["timing"]) { if (frame <= timing.start) return 0; if (frame <= timing.peak) return ease((frame - timing.start) / (timing.peak - timing.start)); if (frame >= timing.recover) return 0; return 1 - ease((frame - timing.peak) / (timing.recover - timing.peak)); }
function ease(value: number) { return value * value * (3 - 2 * value); }
function gazeAt(plan: V14MascotPerformance, frame: number) { const prior = [...plan.gazePath].filter((item) => item.frame <= frame).at(-1) ?? plan.gazePath[0]!, next = plan.gazePath.find((item) => item.frame > frame) ?? prior, p = next.frame === prior.frame ? 1 : Math.min(1, (frame - prior.frame) / (next.frame - prior.frame)); const x = prior.x + (next.x - prior.x) * p, y = prior.y + (next.y - prior.y) * p; return { dx: (x - .5) * 2, dy: (y - .4) * 2 }; }
function mouthFor(face: V14Face, bias: V14Mouth, audio: ReturnType<typeof sampleMascotAudioV14>): V14Mouth { if (!audio.speaking) return bias; if (face === "surprised" && audio.onset > .45) return "surprised_circle"; if (face === "concerned") return audio.rms > .45 ? "small_open" : "concerned_curve"; if (face === "skeptical" && audio.rms < .32) return "thinking_wave"; if (audio.onset > .55 || audio.rms > .62) return "wide_open"; return audio.rms > .3 ? "small_open" : bias; }
function blinkScale(frame: number, blinkFrames: number[], face: V14Face) { if (face === "surprised") return 1; const distance = Math.min(...blinkFrames.map((beat) => Math.abs(frame - beat)), 999); return distance === 0 ? .07 : distance === 1 ? .26 : 1; }
...
function roleLayout(role: V14MascotPerformance["role"]): React.CSSProperties { if (role === "hero_close") return { left: 210, top: 320, transform: "scale(1.12)" }; if (role === "cameo_left") return { left: -128, bottom: -68, transform: "scale(.48)" }; if (role === "cameo_right") return { right: -128, bottom: -68, transform: "scale(.48)" }; return { left: 210, top: 420, transform: "scale(1)" }; }
