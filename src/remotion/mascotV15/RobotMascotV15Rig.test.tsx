...
import { auditSolomonMascotV15, SOLOMON_MASCOT_V15_GEOMETRY, v15FaceSchema, v15GestureSchema, v15MascotPerformanceSchema, type V15Face, type V15Gesture, type V15MascotPerformance } from "../../shared/solomonMascotV15";
import { RobotMascotV15Rig } from "./RobotMascotV15Rig";
...
function plan(overrides: Partial<V15MascotPerformance> = {}): V15MascotPerformance {
  return v15MascotPerformanceSchema.parse({
    sceneId: "snapshot",
    role: "host",
...
const render = (performance: V15MascotPerformance, frame = 14) => renderToString(<RobotMascotV15Rig plan={performance} frame={frame} fps={30} enterOverride={1} />);
const withGesture = (gesture: V15Gesture, overrides: Partial<V15MascotPerformance> = {}) => plan({ left: { gesture, timing: { start: 5, peak: 11, recover: 20, wristRotation: 14 } }, ...overrides });

describe("RobotMascotV15 character golden masters", () => {
  it("renders every resting face state", () => {
    for (const face of v15FaceSchema.options.filter((option): option is V15Face => option !== "surprised")) {
  ...
  it("renders the surprised accent only inside its hold window", () => {
    ...
    expect(render(accented, 4)).toContain('data-v15-face="surprised"');
    expect(render(accented, 12)).toContain('data-v15-face="happy"');
    expect(render(accented, 4)).toMatchSnapshot("face-surprised-accent");
  });

  it("renders every gesture silhouette", () => {
    for (const gesture of v15GestureSchema.options as V15Gesture[]) {
  ...
  it("keeps mitts at cameo scale and fingers only at host scale", () => {
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).not.toContain('data-v15-hand="palm-fingers"');
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).toContain('data-v15-hand="mitt"');
    expect(render(withGesture("open_palm", { role: "host" }))).toContain('data-v15-hand="palm-fingers"');
  });

  it("draws a neck that closes the head-to-body seam", () => {
    expect(render(plan())).toContain("data-v15-neck");
    const neckTop = 482, neckBottom = 482 + 60, headBottom = 48 + SOLOMON_MASCOT_V15_GEOMETRY.headHeight, torsoTop = 720 - 195;
  ...
  it("consumes the shared geometry spec rather than private literals", () => {
    ...
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V15_GEOMETRY.headWidth}"`);
    expect(markup).toContain(`height="${SOLOMON_MASCOT_V15_GEOMETRY.headHeight}"`);
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V15_GEOMETRY.screenWidth}"`);
    expect(markup).toContain(SOLOMON_MASCOT_V15_GEOMETRY.faceColor);
    expect(auditSolomonMascotV15().passed).toBe(true);
  });
});
