...
import { auditSolomonMascotV16, SOLOMON_MASCOT_V16_GEOMETRY, v16FaceSchema, v16GestureSchema, v16MascotPerformanceSchema, type V16Face, type V16Gesture, type V16MascotPerformance } from "../../shared/solomonMascotV16";
import { RobotMascotV16Rig } from "./RobotMascotV16Rig";
...
function plan(overrides: Partial<V16MascotPerformance> = {}): V16MascotPerformance {
  return v16MascotPerformanceSchema.parse({
    sceneId: "snapshot",
    role: "host",
...
const render = (performance: V16MascotPerformance, frame = 14) => renderToString(<RobotMascotV16Rig plan={performance} frame={frame} fps={30} enterOverride={1} />);
const withGesture = (gesture: V16Gesture, overrides: Partial<V16MascotPerformance> = {}) => plan({ left: { gesture, timing: { start: 5, peak: 11, recover: 20, wristRotation: 14 } }, ...overrides });

describe("RobotMascotV16 character golden masters", () => {
  it("renders every resting face state", () => {
    for (const face of v16FaceSchema.options.filter((option): option is V16Face => option !== "surprised")) {
  ...
  it("renders the surprised accent only inside its hold window", () => {
    ...
    expect(render(accented, 4)).toContain('data-v16-face="surprised"');
    expect(render(accented, 12)).toContain('data-v16-face="happy"');
    expect(render(accented, 4)).toMatchSnapshot("face-surprised-accent");
  });

  it("renders every gesture silhouette", () => {
    for (const gesture of v16GestureSchema.options as V16Gesture[]) {
  ...
  it("keeps mitts at cameo scale and fingers only at host scale", () => {
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).not.toContain('data-v16-hand="palm-fingers"');
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).toContain('data-v16-hand="mitt"');
    expect(render(withGesture("open_palm", { role: "host" }))).toContain('data-v16-hand="palm-fingers"');
  });

  it("draws a neck that closes the head-to-body seam", () => {
    expect(render(plan())).toContain("data-v16-neck");
    const neckTop = 482, neckBottom = 482 + 60, headBottom = 48 + SOLOMON_MASCOT_V16_GEOMETRY.headHeight, torsoTop = 720 - 195;
  ...
  it("consumes the shared geometry spec rather than private literals", () => {
    ...
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V16_GEOMETRY.headWidth}"`);
    expect(markup).toContain(`height="${SOLOMON_MASCOT_V16_GEOMETRY.headHeight}"`);
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V16_GEOMETRY.screenWidth}"`);
    expect(markup).toContain(SOLOMON_MASCOT_V16_GEOMETRY.faceColor);
    expect(auditSolomonMascotV16().passed).toBe(true);
  });
});
