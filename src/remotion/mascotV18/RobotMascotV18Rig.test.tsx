...
import { auditSolomonMascotV18, SOLOMON_MASCOT_V18_GEOMETRY, v18FaceSchema, v18GestureSchema, v18MascotPerformanceSchema, type V18Face, type V18Gesture, type V18MascotPerformance } from "../../shared/solomonMascotV18";
import { RobotMascotV18Rig } from "./RobotMascotV18Rig";
...
function plan(overrides: Partial<V18MascotPerformance> = {}): V18MascotPerformance {
  return v18MascotPerformanceSchema.parse({
    sceneId: "snapshot",
    role: "host",
...
const render = (performance: V18MascotPerformance, frame = 14) => renderToString(<RobotMascotV18Rig plan={performance} frame={frame} fps={30} enterOverride={1} />);
const withGesture = (gesture: V18Gesture, overrides: Partial<V18MascotPerformance> = {}) => plan({ left: { gesture, timing: { start: 5, peak: 11, recover: 20, wristRotation: 14 } }, ...overrides });

describe("RobotMascotV18 character golden masters", () => {
  it("renders every resting face state", () => {
    for (const face of v18FaceSchema.options.filter((option): option is V18Face => option !== "surprised")) {
  ...
  it("renders the surprised accent only inside its hold window", () => {
    ...
    expect(render(accented, 4)).toContain('data-v18-face="surprised"');
    expect(render(accented, 12)).toContain('data-v18-face="happy"');
    expect(render(accented, 4)).toMatchSnapshot("face-surprised-accent");
  });

  it("renders every gesture silhouette", () => {
    for (const gesture of v18GestureSchema.options as V18Gesture[]) {
  ...
  it("keeps mitts at cameo scale and fingers only at host scale", () => {
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).not.toContain('data-v18-hand="palm-fingers"');
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).toContain('data-v18-hand="mitt"');
    expect(render(withGesture("open_palm", { role: "host" }))).toContain('data-v18-hand="palm-fingers"');
  });

  it("draws a neck that closes the head-to-body seam", () => {
    expect(render(plan())).toContain("data-v18-neck");
    const neckTop = 482, neckBottom = 482 + 60, headBottom = 48 + SOLOMON_MASCOT_V18_GEOMETRY.headHeight, torsoTop = 720 - 195;
  ...
  it("consumes the shared geometry spec rather than private literals", () => {
    ...
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V18_GEOMETRY.headWidth}"`);
    expect(markup).toContain(`height="${SOLOMON_MASCOT_V18_GEOMETRY.headHeight}"`);
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V18_GEOMETRY.screenWidth}"`);
    expect(markup).toContain(SOLOMON_MASCOT_V18_GEOMETRY.faceColor);
    expect(auditSolomonMascotV18().passed).toBe(true);
  });
});
