...
import { auditSolomonMascotV19, SOLOMON_MASCOT_V19_GEOMETRY, v19FaceSchema, v19GestureSchema, v19MascotPerformanceSchema, type V19Face, type V19Gesture, type V19MascotPerformance } from "../../shared/solomonMascotV19";
import { RobotMascotV19Rig } from "./RobotMascotV19Rig";
...
function plan(overrides: Partial<V19MascotPerformance> = {}): V19MascotPerformance {
  return v19MascotPerformanceSchema.parse({
    sceneId: "snapshot",
    role: "host",
...
const render = (performance: V19MascotPerformance, frame = 14) => renderToString(<RobotMascotV19Rig plan={performance} frame={frame} fps={30} enterOverride={1} />);
const withGesture = (gesture: V19Gesture, overrides: Partial<V19MascotPerformance> = {}) => plan({ left: { gesture, timing: { start: 5, peak: 11, recover: 20, wristRotation: 14 } }, ...overrides });

describe("RobotMascotV19 character golden masters", () => {
  it("renders every resting face state", () => {
    for (const face of v19FaceSchema.options.filter((option): option is V19Face => option !== "surprised")) {
  ...
  it("renders the surprised accent only inside its hold window", () => {
    ...
    expect(render(accented, 4)).toContain('data-v19-face="surprised"');
    expect(render(accented, 12)).toContain('data-v19-face="happy"');
    expect(render(accented, 4)).toMatchSnapshot("face-surprised-accent");
  });

  it("renders every gesture silhouette", () => {
    for (const gesture of v19GestureSchema.options as V19Gesture[]) {
  ...
  it("keeps mitts at cameo scale and fingers only at host scale", () => {
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).not.toContain('data-v19-hand="palm-fingers"');
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).toContain('data-v19-hand="mitt"');
    expect(render(withGesture("open_palm", { role: "host" }))).toContain('data-v19-hand="palm-fingers"');
  });

  it("draws a neck that closes the head-to-body seam", () => {
    expect(render(plan())).toContain("data-v19-neck");
    const neckTop = 482, neckBottom = 482 + 60, headBottom = 48 + SOLOMON_MASCOT_V19_GEOMETRY.headHeight, torsoTop = 720 - 195;
  ...
  it("consumes the shared geometry spec rather than private literals", () => {
    ...
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V19_GEOMETRY.headWidth}"`);
    expect(markup).toContain(`height="${SOLOMON_MASCOT_V19_GEOMETRY.headHeight}"`);
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V19_GEOMETRY.screenWidth}"`);
    expect(markup).toContain(SOLOMON_MASCOT_V19_GEOMETRY.faceColor);
    expect(auditSolomonMascotV19().passed).toBe(true);
  });
});
