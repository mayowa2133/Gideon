...
import { auditSolomonMascotV20, SOLOMON_MASCOT_V20_GEOMETRY, v20FaceSchema, v20GestureSchema, v20MascotPerformanceSchema, type V20Face, type V20Gesture, type V20MascotPerformance } from "../../shared/solomonMascotV20";
import { RobotMascotV20Rig } from "./RobotMascotV20Rig";
...
function plan(overrides: Partial<V20MascotPerformance> = {}): V20MascotPerformance {
  return v20MascotPerformanceSchema.parse({
    sceneId: "snapshot",
    role: "host",
...
const render = (performance: V20MascotPerformance, frame = 14) => renderToString(<RobotMascotV20Rig plan={performance} frame={frame} fps={30} enterOverride={1} />);
const withGesture = (gesture: V20Gesture, overrides: Partial<V20MascotPerformance> = {}) => plan({ left: { gesture, timing: { start: 5, peak: 11, recover: 20, wristRotation: 14 } }, ...overrides });

describe("RobotMascotV20 character golden masters", () => {
  it("renders every resting face state", () => {
    for (const face of v20FaceSchema.options.filter((option): option is V20Face => option !== "surprised")) {
  ...
  it("renders the surprised accent only inside its hold window", () => {
    ...
    expect(render(accented, 4)).toContain('data-v20-face="surprised"');
    expect(render(accented, 12)).toContain('data-v20-face="happy"');
    expect(render(accented, 4)).toMatchSnapshot("face-surprised-accent");
  });

  it("renders every gesture silhouette", () => {
    for (const gesture of v20GestureSchema.options as V20Gesture[]) {
  ...
  it("keeps mitts at cameo scale and fingers only at host scale", () => {
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).not.toContain('data-v20-hand="palm-fingers"');
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).toContain('data-v20-hand="mitt"');
    expect(render(withGesture("open_palm", { role: "host" }))).toContain('data-v20-hand="palm-fingers"');
  });

  it("draws a neck that closes the head-to-body seam", () => {
    expect(render(plan())).toContain("data-v20-neck");
    const neckTop = 482, neckBottom = 482 + 60, headBottom = 48 + SOLOMON_MASCOT_V20_GEOMETRY.headHeight, torsoTop = 720 - 195;
  ...
  it("consumes the shared geometry spec rather than private literals", () => {
    ...
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V20_GEOMETRY.headWidth}"`);
    expect(markup).toContain(`height="${SOLOMON_MASCOT_V20_GEOMETRY.headHeight}"`);
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V20_GEOMETRY.screenWidth}"`);
    expect(markup).toContain(SOLOMON_MASCOT_V20_GEOMETRY.faceColor);
    expect(auditSolomonMascotV20().passed).toBe(true);
  });
});
