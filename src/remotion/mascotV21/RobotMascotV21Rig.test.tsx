...
import { auditSolomonMascotV21, SOLOMON_MASCOT_V21_GEOMETRY, v21FaceSchema, v21GestureSchema, v21MascotPerformanceSchema, type V21Face, type V21Gesture, type V21MascotPerformance } from "../../shared/solomonMascotV21";
import { RobotMascotV21Rig } from "./RobotMascotV21Rig";
...
function plan(overrides: Partial<V21MascotPerformance> = {}): V21MascotPerformance {
...
const render = (performance: V21MascotPerformance, frame = 14) => renderToString(<RobotMascotV21Rig plan={performance} frame={frame} fps={30} enterOverride={1} />);
const withGesture = (gesture: V21Gesture, overrides: Partial<V21MascotPerformance> = {}) => plan({ left: { gesture, timing: { start: 5, peak: 11, recover: 20, wristRotation: 14 } }, ...overrides });

describe("RobotMascotV21 character golden masters", () => {
  it("renders every resting face state", () => {
    for (const face of v21FaceSchema.options.filter((option): option is V21Face => option !== "surprised")) {
  ...
  it("renders the surprised accent only inside its hold window", () => {
    ...
    expect(render(accented, 4)).toContain('data-v21-face="surprised"');
    expect(render(accented, 12)).toContain('data-v21-face="happy"');
    expect(render(accented, 4)).toMatchSnapshot("face-surprised-accent");
  });

  it("renders every gesture silhouette", () => {
    for (const gesture of v21GestureSchema.options as V21Gesture[]) {
  ...
  it("keeps mitts at cameo scale and fingers only at host scale", () => {
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).not.toContain('data-v21-hand="palm-fingers"');
    expect(render(withGesture("open_palm", { role: "cameo_right" }))).toContain('data-v21-hand="mitt"');
    expect(render(withGesture("open_palm", { role: "host" }))).toContain('data-v21-hand="palm-fingers"');
  });

  it("draws a neck that closes the head-to-body seam", () => {
    expect(render(plan())).toContain("data-v21-neck");
    const neckTop = 482, neckBottom = 482 + 60, headBottom = 48 + SOLOMON_MASCOT_V21_GEOMETRY.headHeight, torsoTop = 720 - 195;
  ...
  it("consumes the shared geometry spec rather than private literals", () => {
    ...
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V21_GEOMETRY.headWidth}"`);
    expect(markup).toContain(`height="${SOLOMON_MASCOT_V21_GEOMETRY.headHeight}"`);
    expect(markup).toContain(`width="${SOLOMON_MASCOT_V21_GEOMETRY.screenWidth}"`);
    expect(markup).toContain(SOLOMON_MASCOT_V21_GEOMETRY.faceColor);
    expect(auditSolomonMascotV21().passed).toBe(true);
  });
});
