...
import { asV2Scene, isMotionLedRealInternship, realCta, type MeetFilm, type MeetScene } from "../../shared/meetSolomonRealInternships";
...
function MotionField({ frame, dark, accent }: { frame: number; dark: boolean; accent: string }) {
  return <div aria-hidden style={{ position: "absolute", inset: 0, overflow: "hidden", opacity: dark ? .16 : .22 }}>
    <div style={{ ...abs(-280 + (frame * 2.8) % 260, 490 + Math.sin(frame / 10) * 55, 620, 620), borderRadius: "50%", border: `7px solid ${accent}` }} />
    <div style={{ ...abs(730 + Math.cos(frame / 10) * 90, 1040 + Math.sin(frame / 9) * 70, 460, 460), borderRadius: 70, background: accent, transform: `rotate(${frame * .75}deg)` }} />
    <div style={{ ...abs(-200 + (frame * 5) % 1480, 1540, 330, 16), borderRadius: 20, background: accent }} />
    {[0, 1, 2].map(i => <div key={i} style={{ ...abs(70 + i * 335 + Math.sin((frame + i * 15) / 7) * 34, 1640 + i * 42, 245, 7), background: accent }} />)}
  </div>;
}

function Cta({ category, local, motion }: { category: MeetFilm["category"]; local: number; motion: boolean }) {
  const firstJob = category === "new_grad";
  const styles = (delay: number): CSSProperties => motion ? { opacity: 1, transform: `translateY(${Math.sin((local + delay) / 6) * 8}px) scale(${1 + Math.sin((local + delay) / 9) * .012})` } : {};
  return <div data-real-cta={realCta(category)}>
    <div style={{ ...abs(65, 220, 950), textAlign: "center", fontSize: 31, letterSpacing: 3, ...styles(0) }}>{firstJob ? "YOUR FIRST MOVE" : "YOUR NEXT STEP"}</div>
    <div style={{ ...abs(65, 335, 950), textAlign: "center", fontWeight: 800, fontSize: 92, lineHeight: 1.08, letterSpacing: -4, ...styles(4) }}>{firstJob ? <>Start your first<br />job search.</> : <>Find your next<br />{category === "law" ? "legal" : category}<br />internship.</>}</div>
    <div style={{ ...abs(65, firstJob ? 650 : 725, 950), textAlign: "center", fontFamily: serif, fontStyle: "italic", fontSize: 64, ...styles(9) }}>with</div>
    <div style={{ ...abs(65, firstJob ? 765 : 830, 950), textAlign: "center", fontWeight: 800, fontSize: 148, color: "#176b50", letterSpacing: -6, ...styles(13) }}>Solomon.</div>
  </div>;
}

function Art({ scene, category, benefitLed, motion, local }: { scene: MeetScene; category: MeetFilm["category"]; benefitLed: boolean; motion: boolean; local: number }) {
  const words = category === "finance" ? ["AUDIT", "ANALYSIS"] : category === "software" ? ["CODE", "BUILD"] : category === "law" ? ["LEGAL OPS", "DATA"] : ["FIRST ROLE", "REAL OPENING"];
  ...
  if (scene.layout === "requirements" || scene.layout === "fit") return <>{(category === "new_grad" ? scene.layout === "requirements" ? ["TARGET OCCUPATIONS", "YOUR PATH"] : ["AUTOMATIC FEED", "STAGES TOGETHER"] : benefitLed ? scene.layout === "requirements" ? ["TAILOR TO THE ROLE", "KEEP YOUR EVIDENCE TRUE"] : ["RESUME", "APPLICATION CHECKLIST"] : scene.layout === "requirements" ? ["READ THE DESCRIPTION", "CHECK THE REQUIREMENTS"] : ["YOUR SKILLS", "YOUR AVAILABILITY"]).map((word, i) => <div key={word} style={{ ...abs(90, 620 + i * 230, 900, 170), ...tile, ...motionTile, display: "flex", alignItems: "center", justifyContent: "center", ...(motion ? rise(local, i * 6) : {}) }}>{word}</div>)}{!benefitLed && <div style={{ ...abs(65, 1135, 950), textAlign: "center", fontSize: 24 }}>ILLUSTRATION · YOU DECIDE WHETHER TO APPLY</div>}</>;
  if (scene.layout === "reset") return <div style={{ ...abs(90, 660, 900, 180), ...tile, ...motionTile, ...(motion ? rise(local) : {}) }}>{category === "new_grad" ? "ONE SEARCH. ONE WORKSPACE." : "LOOK BEYOND THE TITLE."}</div>;
  ...
  if (scene.layout === "cta") return <Cta category={category} local={local} motion={motion} />;
  return null;
}
...
export const MeetSolomonRealFrame: React.FC<{ film: MeetFilm; frame: number }> = ({ film, frame }) => {
  ...
  const accent = film.category === "finance" ? "#d7a333" : film.category === "software" ? "#73a8d8" : film.category === "law" ? "#b495ce" : "#61b997";
  const categoryLabel = film.category === "law" ? "LEGAL OPERATIONS INTERNSHIPS" : film.category === "new_grad" ? "NEW GRAD JOBS" : `${film.category.toUpperCase()} INTERNSHIPS`;
  return <AbsoluteFill style={{ background: COLORS[scene.background], color: dark ? "#f4efe5" : "#182320", fontFamily: '"Manrope Variable", Arial, sans-serif', overflow: "hidden" }}>
    {motion && <MotionField frame={local + sceneIndex * 17} dark={dark} accent={accent} />}
    ...
    <div style={{ position: "absolute", inset: 0, ...(motion ? { zIndex: 1, transform: `translateY(${Math.sin(local / 5) * 13}px) scale(${1 + Math.sin(local / 11) * .008})` } : {}) }}><Art scene={scene} category={film.category} benefitLed={benefitLed} motion={motion} local={local} /></div>
    {scene.proofs.map((p, i) => {
      const proofEnter = motion ? enter(local, i * 5) : 1;
      return <InternshipProof key={`${p.id}-${i}`} evidence={film.evidence.find(e => e.id === p.id)!} placement={p} style={motion ? { zIndex: 2, opacity: proofEnter, transform: `translate(${Math.sin((local + i * 9) / 7) * 12}px, ${(1 - proofEnter) * 120 + Math.sin((local + i * 8) / 5) * 20}px) scale(${.88 + proofEnter * .12 + Math.sin(local / 10) * .012}) rotate(${(i - (scene.proofs.length - 1) / 2) * 1.6 + Math.sin(local / 13) * .6}deg)`, boxShadow: "0 28px 65px #07110d38" } : undefined} />;
    })}
    ...
    {motion && <div style={{ ...abs(65, 1840, 950, 5), zIndex: 8, background: dark ? "#ffffff22" : "#13251d18", borderRadius: 5 }}><div style={{ height: "100%", width: `${((sceneIndex + Math.min(1, local / Math.max(1, scene.to - scene.from))) / film.scenes.length) * 100}%`, background: accent, borderRadius: 5 }} /></div>}
  </AbsoluteFill>;
};
