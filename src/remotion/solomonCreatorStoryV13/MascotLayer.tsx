...
import { mascotBoxForScene } from "../../shared/creatorStoryV13Quality";
import type { V13Scene } from "../../shared/solomonCreatorStoryV13";
import type { V13MascotPerformance } from "../../shared/solomonMascotV13";
import { RobotMascotV13Rig } from "../mascotV13/RobotMascotV13Rig";
...
//
// V13 drops the dashed mascot-to-content interaction line that V9 used and V11
...
export const MascotLayer: React.FC<{ scenes: V13Scene[] }> = ({ scenes }) => {
  ...
  const visiblePlan: V13MascotPerformance = plan.role === "absent" ? previous.mascot : plan;
  if (visiblePlan.role === "absent") return null;

  return <div data-v13-mascot-layer={scene.id} style={{ position: "absolute", inset: 0, zIndex: 32, pointerEvents: "none" }}>
    <div data-v13-mascot-box style={{ position: "absolute", left: box.x, top: box.y, width: 660, height: 940, transformOrigin: "0 0", transform: `scale(${box.scale * stretch}, ${box.scale * squash})`, opacity: plan.role === "absent" ? 0 : entrance }}>
      <RobotMascotV13Rig plan={visiblePlan} frame={frame - scene.from} enterOverride={1} positioning="external" />
    </div>
  </div>;
};
