...
import { mascotBoxForScene } from "../../shared/creatorStoryV14Quality";
import type { V14Scene } from "../../shared/solomonCreatorStoryV14";
import type { V14MascotPerformance } from "../../shared/solomonMascotV14";
import { RobotMascotV14Rig } from "../mascotV14/RobotMascotV14Rig";
...
//
// V14 drops the dashed mascot-to-content interaction line that V9 used and V11
...
export const MascotLayer: React.FC<{ scenes: V14Scene[] }> = ({ scenes }) => {
  ...
  const visiblePlan: V14MascotPerformance = plan.role === "absent" ? previous.mascot : plan;
  if (visiblePlan.role === "absent") return null;

  return <div data-v14-mascot-layer={scene.id} style={{ position: "absolute", inset: 0, zIndex: 32, pointerEvents: "none" }}>
    <div data-v14-mascot-box style={{ position: "absolute", left: box.x, top: box.y, width: 660, height: 940, transformOrigin: "0 0", transform: `scale(${box.scale * stretch}, ${box.scale * squash})`, opacity: plan.role === "absent" ? 0 : entrance }}>
      <RobotMascotV14Rig plan={visiblePlan} frame={frame - scene.from} enterOverride={1} positioning="external" />
    </div>
  </div>;
};
