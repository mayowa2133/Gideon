...
import { mascotBoxForScene } from "../../shared/creatorStoryV15Quality";
import type { V15Scene } from "../../shared/solomonCreatorStoryV15";
import type { V15MascotPerformance } from "../../shared/solomonMascotV15";
import { RobotMascotV15Rig } from "../mascotV15/RobotMascotV15Rig";
...
//
// V15 drops the dashed mascot-to-content interaction line that V9 used and V11
...
export const MascotLayer: React.FC<{ scenes: V15Scene[] }> = ({ scenes }) => {
  ...
  const visiblePlan: V15MascotPerformance = plan.role === "absent" ? previous.mascot : plan;
  if (visiblePlan.role === "absent") return null;

  return <div data-v15-mascot-layer={scene.id} style={{ position: "absolute", inset: 0, zIndex: 32, pointerEvents: "none" }}>
    <div data-v15-mascot-box style={{ position: "absolute", left: box.x, top: box.y, width: 660, height: 940, transformOrigin: "0 0", transform: `scale(${box.scale * stretch}, ${box.scale * squash})`, opacity: plan.role === "absent" ? 0 : entrance }}>
      <RobotMascotV15Rig plan={visiblePlan} frame={frame - scene.from} enterOverride={1} positioning="external" />
    </div>
  </div>;
};
