...
import { mascotBoxForScene } from "../../shared/creatorStoryV16Quality";
import type { V16Scene } from "../../shared/solomonCreatorStoryV16";
import type { V16MascotPerformance } from "../../shared/solomonMascotV16";
import { RobotMascotV16Rig } from "../mascotV16/RobotMascotV16Rig";
...
//
// V16 drops the dashed mascot-to-content interaction line that V9 used and V11
...
export const MascotLayer: React.FC<{ scenes: V16Scene[] }> = ({ scenes }) => {
  ...
  const visiblePlan: V16MascotPerformance = plan.role === "absent" ? previous.mascot : plan;
  if (visiblePlan.role === "absent") return null;

  return <div data-v16-mascot-layer={scene.id} style={{ position: "absolute", inset: 0, zIndex: 32, pointerEvents: "none" }}>
    <div data-v16-mascot-box style={{ position: "absolute", left: box.x, top: box.y, width: 660, height: 940, transformOrigin: "0 0", transform: `scale(${box.scale * stretch}, ${box.scale * squash})`, opacity: plan.role === "absent" ? 0 : entrance }}>
      <RobotMascotV16Rig plan={visiblePlan} frame={frame - scene.from} enterOverride={1} positioning="external" />
    </div>
  </div>;
};
