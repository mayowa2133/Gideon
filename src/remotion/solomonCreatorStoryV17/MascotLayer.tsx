...
import { mascotBoxForScene } from "../../shared/creatorStoryV17Quality";
import type { V17Scene } from "../../shared/solomonCreatorStoryV17";
import type { V17MascotPerformance } from "../../shared/solomonMascotV17";
import { RobotMascotV17Rig } from "../mascotV17/RobotMascotV17Rig";
...
//
// V17 drops the dashed mascot-to-content interaction line that V9 used and V11
...
export const MascotLayer: React.FC<{ scenes: V17Scene[] }> = ({ scenes }) => {
  ...
  const visiblePlan: V17MascotPerformance = plan.role === "absent" ? previous.mascot : plan;
  if (visiblePlan.role === "absent") return null;

  return <div data-v17-mascot-layer={scene.id} style={{ position: "absolute", inset: 0, zIndex: 32, pointerEvents: "none" }}>
    <div data-v17-mascot-box style={{ position: "absolute", left: box.x, top: box.y, width: 660, height: 940, transformOrigin: "0 0", transform: `scale(${box.scale * stretch}, ${box.scale * squash})`, opacity: plan.role === "absent" ? 0 : entrance }}>
      <RobotMascotV17Rig plan={visiblePlan} frame={frame - scene.from} enterOverride={1} positioning="external" />
    </div>
  </div>;
};
