...
import { mascotBoxForScene } from "../../shared/creatorStoryV18Quality";
import type { V18Scene } from "../../shared/solomonCreatorStoryV18";
import type { V18MascotPerformance } from "../../shared/solomonMascotV18";
import { RobotMascotV18Rig } from "../mascotV18/RobotMascotV18Rig";
...
//
// V18 drops the dashed mascot-to-content interaction line that V9 used and V11
...
export const MascotLayer: React.FC<{ scenes: V18Scene[] }> = ({ scenes }) => {
  ...
  const visiblePlan: V18MascotPerformance = plan.role === "absent" ? previous.mascot : plan;
  if (visiblePlan.role === "absent") return null;

  return <div data-v18-mascot-layer={scene.id} style={{ position: "absolute", inset: 0, zIndex: 32, pointerEvents: "none" }}>
    <div data-v18-mascot-box style={{ position: "absolute", left: box.x, top: box.y, width: 660, height: 940, transformOrigin: "0 0", transform: `scale(${box.scale * stretch}, ${box.scale * squash})`, opacity: plan.role === "absent" ? 0 : entrance }}>
      <RobotMascotV18Rig plan={visiblePlan} frame={frame - scene.from} enterOverride={1} positioning="external" />
    </div>
  </div>;
};
