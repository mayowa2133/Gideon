...
import { mascotBoxForScene } from "../../shared/creatorStoryV20Quality";
import type { V20Scene } from "../../shared/solomonCreatorStoryV20";
import type { V20MascotPerformance } from "../../shared/solomonMascotV20";
import { RobotMascotV20Rig } from "../mascotV20/RobotMascotV20Rig";
...
//
// V20 drops the dashed mascot-to-content interaction line that V9 used and V11
...
export const MascotLayer: React.FC<{ scenes: V20Scene[] }> = ({ scenes }) => {
  ...
  const visiblePlan: V20MascotPerformance = plan.role === "absent" ? previous.mascot : plan;
  if (visiblePlan.role === "absent") return null;

  return <div data-v20-mascot-layer={scene.id} style={{ position: "absolute", inset: 0, zIndex: 32, pointerEvents: "none" }}>
    <div data-v20-mascot-box style={{ position: "absolute", left: box.x, top: box.y, width: 660, height: 940, transformOrigin: "0 0", transform: `scale(${box.scale * stretch}, ${box.scale * squash})`, opacity: plan.role === "absent" ? 0 : entrance }}>
      <RobotMascotV20Rig plan={visiblePlan} frame={frame - scene.from} enterOverride={1} positioning="external" />
    </div>
  </div>;
};
