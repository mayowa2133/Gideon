...
import { V20_BACKDROPS, V20_CAMERA_ENVELOPE, V20_TEXT_ZONES } from "../../shared/creatorStoryV20Quality";
import { SOLOMON_CREATOR_STORY_V20_HEADLINES, type SolomonCreatorStoryV20Manifest, type V20AssetId, type V20Caption, type V20Scene } from "../../shared/solomonCreatorStoryV20";
...
const SerifHeadline:React.FC<{sceneId:string;accentColor:string;fontSize:number;top:number;left?:number;right?:number;align?:"left"|"center"}> = ({sceneId,accentColor,fontSize,top,left=70,right=70,align="center"}) => {
  const headline=SOLOMON_CREATOR_STORY_V20_HEADLINES[sceneId];
  ...
  const safeTop=Math.max(top,V20_TEXT_ZONES.headlineTopMin);
  return <div data-v20-headline={sceneId} style={{position:"absolute",left,right,top:safeTop,fontFamily:"Fraunces Variable,serif",fontSize,lineHeight:.94,fontWeight:850,textAlign:align,color:"var(--v20-fg)"}}>{headline.primary}{headline.accentItalic&&<><br/><i style={{color:accentColor}}>{headline.accentItalic}</i></>}</div>;
};
...
const KineticHeadline:React.FC<{sceneId:string;accentColor:string;top:number;maxHeightPx:number;duration:number;widthPx?:number}> = ({sceneId,accentColor,top,maxHeightPx,duration,widthPx=940}) => {
  ...
  return <div data-v20-kinetic-headline={sceneId} data-v20-kinetic-word={chunk.map(({text})=>text).join(" ")} style={{position:"absolute",left:0,right:0,top,display:"flex",flexDirection:"column",alignItems:"center",pointerEvents:"none"}}>
    {chunk.map(({text,accent})=><div key={text} style={{fontFamily:"Fraunces Variable,serif",fontSize:size,lineHeight:.92,fontWeight:900,textAlign:"center",whiteSpace:"nowrap",color:accent?accentColor:"var(--v20-fg)",transform:`scale(${.62+.38*pop})`,opacity:Math.min(1,.25+pop*1.3),filter:`blur(${Math.max(0,(1-pop)*6)}px)`}}>{text}</div>)}
  </div>;
};

const INK="#07111f", PAPER="#fbf8f0", WHITE="#fff", MINT="#39f2b5", GREEN="#087052", CORAL="#ff6f61", AMBER="#ff9d18";
const FILES:Record<V20AssetId,string>={tracker_before:"proof-tracker-before.mp4",tracker_after:"proof-tracker-after.mp4",opportunity:"proof-opportunity.mp4",contact:"proof-contact.mp4",outreach_blank:"proof-outreach-blank.mp4",outreach_complete:"proof-outreach-complete.mp4"};
...
export const SolomonCreatorStoryV20:React.FC<SolomonCreatorStoryV20Manifest> = (manifest) => <AbsoluteFill style={{background:INK,color:INK,fontFamily:"Manrope Variable,Manrope,sans-serif",overflow:"hidden"}}>
...
const Backdrop:React.FC<React.PropsWithChildren<{token:keyof typeof V20_BACKDROPS}>> = ({token,children}) => {
  const backdrop=V20_BACKDROPS[token];
  return <AbsoluteFill style={{background:backdrop.css,["--v20-fg" as string]:backdrop.foreground}}>{children}</AbsoluteFill>;
};
const Scene:React.FC<{scene:V20Scene}> = ({scene}) => <EditorialCamera scene={scene}><Backdrop token={scene.backdrop}><SceneContent scene={scene}/></Backdrop></EditorialCamera>;

const SceneContent:React.FC<{scene:V20Scene}> = ({scene}) => {
...
const Grounded:React.FC<{scene:V20Scene}> = ({scene}) => {const frame=useCurrentFrame(),reveal=interpolate(frame,[10,55],[0,1],{extrapolateLeft:"clamp",extrapolateRight:"clamp"}),arrive=spring({frame,fps:30,config:{damping:18,stiffness:220},durationInFrames:7});return <EvidenceBackground accent="#e6f6ee"><div style={{position:"absolute",left:55,top:300,width:970,height:820,transform:`translateX(${(1-arrive)*640}px)`}}><EvidenceCard border={GREEN}><EvidenceCrop asset="outreach_complete" crop={CROPS.message} width={970} height={820}/><div style={{position:"absolute",left:67,top:328,width:760*reveal,height:72,borderRadius:12,background:"rgba(57,242,181,.24)",borderBottom:`7px solid ${MINT}`}}/></EvidenceCard></div><div style={{position:"absolute",left:80,right:80,top:1175,display:"flex",gap:18,alignItems:"center",justifyContent:"center",fontSize:29,fontWeight:950}}><span style={{padding:"14px 18px",borderRadius:14,background:"#ffe2dc",color:CORAL,textDecoration:"line-through"}}>GENERIC OPENER</span><span style={{fontSize:52}}>→</span><span style={{padding:"14px 18px",borderRadius:14,background:MINT}}>ROLE + RELEVANCE</span></div></EvidenceBackground>;};

const Control:React.FC<{scene:V20Scene}> = ({scene}) => {const frame=useCurrentFrame();return <EvidenceBackground><div style={{position:"absolute",left:330,top:275,width:700,height:830}}><EvidenceCard border={GREEN}><EvidenceCrop asset="outreach_complete" crop={CROPS.messageEdit} width={700} height={830} playback/></EvidenceCard></div><div style={{position:"absolute",left:350,right:55,top:1145,display:"grid",gap:12}}><ProofLabel text="SAVE EDIT" primary/><ProofLabel text="CANCEL · SEND UNTOUCHED"/><div style={{padding:"16px 20px",borderRadius:15,background:INK,color:WHITE,fontSize:27,fontWeight:950}}>NOTHING SENDS WITHOUT YOU</div></div><Cursor frame={frame} from={{x:890,y:1040}} to={{x:660,y:985}} clickAt={78}/></EvidenceBackground>;};

const Payoff:React.FC<{scene:V20Scene}> = ({scene}) => {const frame=useCurrentFrame(),collapse=interpolate(frame,[0,42],[0,1],{extrapolateRight:"clamp"}),result=spring({frame:Math.max(0,frame-35),fps:30,config:{damping:15,stiffness:155},durationInFrames:8});const windows=[{asset:"tracker_after" as const,crop:CROPS.trackerAfter,x:30,y:330},{asset:"opportunity" as const,crop:CROPS.opportunityHeader,x:560,y:300},{asset:"contact" as const,crop:CROPS.contactCard,x:65,y:870},{asset:"outreach_complete" as const,crop:CROPS.message,x:565,y:850},{asset:"outreach_blank" as const,crop:CROPS.outreachBlank,x:310,y:1230}];return <AbsoluteFill >{windows.map((item,index)=><div key={index} style={{position:"absolute",left:item.x+(390-item.x)*collapse,top:item.y+(610-item.y)*collapse,width:480,height:300,borderRadius:23,overflow:"hidden",background:WHITE,border:"2px solid #b9cbc4",opacity:1-collapse,transform:`scale(${1-collapse*.55}) rotate(${index%2?-4:4}deg)`}}><EvidenceCrop asset={item.asset} crop={item.crop} width={480} height={300}/></div>)}<div data-v20-connected-result style={{position:"absolute",left:42,top:285,width:996,height:1010,borderRadius:36,background:WHITE,border:`7px solid ${GREEN}`,boxShadow:"0 32px 90px rgba(7,17,31,.22)",opacity:result,transform:`scale(${.7+.3*result})`,overflow:"hidden"}}><ConnectedBoard frame={frame}/></div><div style={{opacity:result}}><KineticHeadline sceneId="payoff" accentColor={GREEN} top={1344} maxHeightPx={210} duration={105}/></div></AbsoluteFill>;};

const Result:React.FC<{scene:V20Scene}> = ({scene}) => {const frame=useCurrentFrame(),pulse=1+Math.sin(frame/9)*.012;return <EvidenceBackground accent="#d5f8ec"><div data-v20-connected-result style={{position:"absolute",left:45,top:260,width:990,height:1000,borderRadius:36,background:WHITE,border:`7px solid ${GREEN}`,boxShadow:"0 32px 90px rgba(7,17,31,.22)",transform:`scale(${pulse})`,overflow:"hidden"}}><ConnectedBoard frame={frame+80}/></div><SerifHeadline sceneId="result" accentColor={GREEN} fontSize={67} top={1325}/></EvidenceBackground>;};
...
const Cta:React.FC<{scene:V20Scene}> = ({scene}) => {const frame=useCurrentFrame(),type=spring({frame:Math.max(0,frame-38),fps:30,config:{damping:14,stiffness:190},durationInFrames:8}),pulse=frame>=58?1+Math.sin((frame-58)/4)*.05*Math.max(0,1-(frame-58)/48):1;return <AbsoluteFill><SerifHeadline sceneId="cta" accentColor={MINT} fontSize={70} top={212}/><div data-v20-comment-box style={{position:"absolute",left:130,right:130,top:1252,transform:`scale(${pulse})`}}><div style={{padding:"22px 28px",borderRadius:24,background:"rgba(255,255,255,.94)",border:`4px solid ${MINT}`,boxShadow:`0 0 ${34+type*46}px rgba(57,242,181,${.2+.35*type})`,display:"flex",alignItems:"center",gap:16}}><div style={{width:52,height:52,borderRadius:999,background:INK,display:"grid",placeItems:"center",color:MINT,fontSize:26,fontWeight:950}}>S</div><div style={{fontSize:38,fontWeight:950,letterSpacing:1,color:INK}}>{"SOLOMON".slice(0,Math.max(1,Math.round(type*7)))}<span style={{opacity:.35}}>|</span></div></div><div style={{marginTop:18,textAlign:"center",fontSize:24,fontWeight:900,color:"var(--v20-fg)",opacity:.92}}>COMMENT AND THE DEMO ARRIVES IN YOUR DMS</div></div><div style={{position:"absolute",left:365,right:365,top:1636,padding:"10px 14px",borderRadius:999,background:INK,color:WHITE,textAlign:"center",fontSize:23,fontWeight:900,letterSpacing:2}}>@SOLOMON</div></AbsoluteFill>;};

const Sting:React.FC<{scene:V20Scene}> = ({scene}) => {const frame=useCurrentFrame(),p=spring({frame,fps:30,config:{damping:15,stiffness:175},durationInFrames:8});return <AbsoluteFill style={{display:"grid",placeItems:"center"}}><div style={{fontFamily:"Fraunces Variable,serif",fontSize:120,fontWeight:900,color:WHITE,transform:`scale(${.8+.2*p})`,opacity:p}}>{(SOLOMON_CREATOR_STORY_V20_HEADLINES["sting"]?.primary??"SOLOMON.").replace(/\.$/,"")}<span style={{color:AMBER}}>.</span></div></AbsoluteFill>;};

const EvidenceCrop:React.FC<{asset:V20AssetId;crop:Crop;width:number;height:number;playback?:boolean}> = ({asset,crop,width,height,playback=false}) => {const scale=Math.max(width/crop.width,height/crop.height),videoWidth=1440*scale,videoHeight=900*scale,focusScale=1.02;return <div data-v20-product={asset} style={{position:"absolute",inset:0,overflow:"hidden",background:WHITE}}><Video src={staticFile(FILES[asset])} trimBefore={crop.trim} muted playbackRate={playback?1:0.35} style={{position:"absolute",left:-crop.x*scale,top:-crop.y*scale,width:videoWidth,height:videoHeight,maxWidth:"none",transform:`scale(${focusScale})`,transformOrigin:`${(crop.x+crop.width/2)/14.4}% ${(crop.y+crop.height/2)/9}%`}}/></div>;};
...
// Camera stays inside V20_CAMERA_ENVELOPE so no layout rect can cross the frame
// edge, and "calm" scenes (reading moments) damp the sway to near zero for their
// middle stretch, which is what restores the reference-style calm frames.
const EditorialCamera:React.FC<React.PropsWithChildren<{scene:V20Scene}>> = ({scene,children}) => {
  const frame=useCurrentFrame(),duration=Math.max(1,scene.to-scene.from-1);
  ...
  const baseScale=V20_CAMERA_ENVELOPE.baseScaleMax,push=progress*Math.min(V20_CAMERA_ENVELOPE.pushMax,declaredTravel*.1);
  const calm=isCalmScene(scene);
  const calmDamp=calm?(progress>.12&&progress<.88?.04:.35):.75;
  const swayX=Math.sin((frame+scene.from*.13)/13)*V20_CAMERA_ENVELOPE.swayXMax*calmDamp;
  const swayY=Math.cos((frame+scene.from*.17)/17)*V20_CAMERA_ENVELOPE.swayYMax*calmDamp;
  const focusX=(scene.camera.focus.x-.5)*-V20_CAMERA_ENVELOPE.focusShiftMax,focusY=(scene.camera.focus.y-.5)*-V20_CAMERA_ENVELOPE.focusShiftMax;
  const heldPush=calm&&progress>.2&&progress<.8?progress*0+.2*Math.min(V20_CAMERA_ENVELOPE.pushMax,declaredTravel*.1):push;
  return <AbsoluteFill data-v20-camera={scene.camera.recipe} data-v20-calm={calm?"true":"false"} style={{transform:`translate(${swayX+focusX*progress}px,${swayY+focusY*progress}px) scale(${baseScale+heldPush})`,transformOrigin:`${scene.camera.focus.x*100}% ${scene.camera.focus.y*100}%`}}>{children}</AbsoluteFill>;
};
function isCalmScene(scene:V20Scene){return scene.typography==="product_annotation";}
const Disclosure=()=> <div data-v20-disclosure style={{position:"absolute",left:26,top:26,zIndex:80,padding:"6px 10px",borderRadius:8,background:"rgba(255,255,255,.84)",border:"1px solid rgba(7,112,82,.25)",color:GREEN,fontSize:14,fontWeight:900,letterSpacing:2}}>DEMO DATA</div>;
...
const ReasonStack:React.FC<{frame:number}> = ({frame}) => <div style={{position:"absolute",left:90,right:90,top:1100,display:"grid",gap:13}}>{["WORKS AT NORTHSTAR LABS","RELEVANT RECRUITING ROLE","SUPPORTING EVIDENCE INCLUDED"].map((text,index)=><ProofLabel key={text} text={text} primary={index===0} delay={index*7} frame={frame}/>)}</div>;
