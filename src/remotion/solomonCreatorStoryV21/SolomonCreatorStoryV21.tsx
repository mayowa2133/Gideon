...
import { V21_BACKDROPS, V21_TEXT_ZONES, v21CameraTransform } from "../../shared/creatorStoryV21Quality";
import { SOLOMON_CREATOR_STORY_V21_HEADLINES, type SolomonCreatorStoryV21Manifest, type V21AssetId, type V21Caption, type V21Scene } from "../../shared/solomonCreatorStoryV21";
...
const SerifHeadline:React.FC<{sceneId:string;accentColor:string;fontSize:number;top:number;left?:number;right?:number;align?:"left"|"center"}> = ({sceneId,accentColor,fontSize,top,left=70,right=70,align="center"}) => {
  const headline=SOLOMON_CREATOR_STORY_V21_HEADLINES[sceneId];
  ...
  const safeTop=Math.max(top,V21_TEXT_ZONES.headlineTopMin);
  return <div data-v21-headline={sceneId} style={{position:"absolute",left,right,top:safeTop,fontFamily:"Fraunces Variable,serif",fontSize,lineHeight:.94,fontWeight:850,textAlign:align,color:"var(--v21-fg)"}}>{headline.primary}{headline.accentItalic&&<><br/><i style={{color:accentColor}}>{headline.accentItalic}</i></>}</div>;
};
...
const KineticHeadline:React.FC<{sceneId:string;accentColor:string;top:number;maxHeightPx:number;duration:number;widthPx?:number}> = ({sceneId,accentColor,top,maxHeightPx,duration,widthPx=940}) => {
  ...
  return <div data-v21-kinetic-headline={sceneId} data-v21-kinetic-word={chunk.map(({text})=>text).join(" ")} style={{position:"absolute",left:0,right:0,top,display:"flex",flexDirection:"column",alignItems:"center",pointerEvents:"none"}}>
    {chunk.map(({text,accent})=><div key={text} style={{fontFamily:"Fraunces Variable,serif",fontSize:size,lineHeight:.92,fontWeight:900,textAlign:"center",whiteSpace:"nowrap",color:accent?accentColor:"var(--v21-fg)",transform:`scale(${.62+.38*pop})`,opacity:Math.min(1,.25+pop*1.3),filter:`blur(${Math.max(0,(1-pop)*6)}px)`}}>{text}</div>)}
  </div>;
};

const INK="#07111f", PAPER="#fbf8f0", WHITE="#fff", MINT="#39f2b5", GREEN="#087052", CORAL="#ff6f61", AMBER="#ff9d18";
const FILES:Record<V21AssetId,string>={tracker_before:"proof-tracker-before.mp4",tracker_after:"proof-tracker-after.mp4",opportunity:"proof-opportunity.mp4",contact:"proof-contact.mp4",outreach_blank:"proof-outreach-blank.mp4",outreach_complete:"proof-outreach-complete.mp4"};
...
export const SolomonCreatorStoryV21:React.FC<SolomonCreatorStoryV21Manifest> = (manifest) => <AbsoluteFill style={{background:INK,color:INK,fontFamily:"Manrope Variable,Manrope,sans-serif",overflow:"hidden"}}>
...
const Backdrop:React.FC<React.PropsWithChildren<{token:keyof typeof V21_BACKDROPS}>> = ({token,children}) => {
  const backdrop=V21_BACKDROPS[token];
  return <AbsoluteFill style={{background:backdrop.css,["--v21-fg" as string]:backdrop.foreground}}>{children}</AbsoluteFill>;
};
const Scene:React.FC<{scene:V21Scene}> = ({scene}) => <EditorialCamera scene={scene}><Backdrop token={scene.backdrop}><SceneContent scene={scene}/></Backdrop></EditorialCamera>;

const SceneContent:React.FC<{scene:V21Scene}> = ({scene}) => {
...
const Cta:React.FC<{scene:V21Scene}> = ({scene}) => {const frame=useCurrentFrame(),type=spring({frame:Math.max(0,frame-38),fps:30,config:{damping:14,stiffness:190},durationInFrames:8}),pulse=frame>=58?1+Math.sin((frame-58)/4)*.05*Math.max(0,1-(frame-58)/48):1;return <AbsoluteFill><SerifHeadline sceneId="cta" accentColor={MINT} fontSize={70} top={212}/><div data-v21-comment-box style={{position:"absolute",left:130,right:130,top:1252,transform:`scale(${pulse})`}}><div style={{padding:"22px 28px",borderRadius:24,background:"rgba(255,255,255,.94)",border:`4px solid ${MINT}`,boxShadow:`0 0 ${34+type*46}px rgba(57,242,181,${.2+.35*type})`,display:"flex",alignItems:"center",gap:16}}><div style={{width:52,height:52,borderRadius:999,background:INK,display:"grid",placeItems:"center",color:MINT,fontSize:26,fontWeight:950}}>S</div><div style={{fontSize:38,fontWeight:950,letterSpacing:1,color:INK}}>{"SOLOMON".slice(0,Math.max(1,Math.round(type*7)))}<span style={{opacity:.35}}>|</span></div></div><div style={{marginTop:18,textAlign:"center",fontSize:24,fontWeight:900,color:"var(--v21-fg)",opacity:.92}}>COMMENT AND THE DEMO ARRIVES IN YOUR DMS</div></div><div style={{position:"absolute",left:365,right:365,top:1636,padding:"10px 14px",borderRadius:999,background:INK,color:WHITE,textAlign:"center",fontSize:23,fontWeight:900,letterSpacing:2}}>@SOLOMON</div></AbsoluteFill>;};

const Sting:React.FC<{scene:V21Scene}> = ({scene}) => {const frame=useCurrentFrame(),p=spring({frame,fps:30,config:{damping:15,stiffness:175},durationInFrames:8});return <AbsoluteFill style={{display:"grid",placeItems:"center"}}><div style={{fontFamily:"Fraunces Variable,serif",fontSize:120,fontWeight:900,color:WHITE,transform:`scale(${.8+.2*p})`,opacity:p}}>{(SOLOMON_CREATOR_STORY_V21_HEADLINES["sting"]?.primary??"SOLOMON.").replace(/\.$/,"")}<span style={{color:AMBER}}>.</span></div></AbsoluteFill>;};

const EvidenceCrop:React.FC<{asset:V21AssetId;crop:Crop;width:number;height:number;playback?:boolean}> = ({asset,crop,width,height,playback=false}) => {const scale=Math.max(width/crop.width,height/crop.height),videoWidth=1440*scale,videoHeight=900*scale,focusScale=1.02;return <div data-v21-product={asset} style={{position:"absolute",inset:0,overflow:"hidden",background:WHITE}}><Video src={staticFile(FILES[asset])} trimBefore={crop.trim} muted playbackRate={playback?1:0.35} style={{position:"absolute",left:-crop.x*scale,top:-crop.y*scale,width:videoWidth,height:videoHeight,maxWidth:"none",transform:`scale(${focusScale})`,transformOrigin:`${(crop.x+crop.width/2)/14.4}% ${(crop.y+crop.height/2)/9}%`}}/></div>;};
...
// Camera stays inside V21_CAMERA_ENVELOPE so no layout rect can cross the frame
// edge. V21 locks it off: the continuous sine sway is gone and the declared push
...
// measureV21Motion decodes at 180x320 and 5fps and averages it away. The
// reference videos hold their shots and let the cut supply the energy.
//
// The transform itself lives in creatorStoryV21Quality as the pure function
// v21CameraTransform, so HeldStability.test can assert frame-to-frame identity
// without a Remotion context. This component is only the wiring.
const EditorialCamera:React.FC<React.PropsWithChildren<{scene:V21Scene}>> = ({scene,children}) => {
  const frame=useCurrentFrame();
  const camera=v21CameraTransform(scene,frame);
  return <AbsoluteFill data-v21-camera={scene.camera.recipe} data-v21-calm={camera.calm?"true":"false"} style={{transform:camera.transform,transformOrigin:`${scene.camera.focus.x*100}% ${scene.camera.focus.y*100}%`}}>{children}</AbsoluteFill>;
};
const Disclosure=()=> <div data-v21-disclosure style={{position:"absolute",left:26,top:26,zIndex:80,padding:"6px 10px",borderRadius:8,background:"rgba(255,255,255,.84)",border:"1px solid rgba(7,112,82,.25)",color:GREEN,fontSize:14,fontWeight:900,letterSpacing:2}}>DEMO DATA</div>;
