...
import { mascotIdleFloat, v22MascotPerformanceSchema, type V22MascotPerformance } from "../../shared/solomonMascotV22";
import { RobotMascotV22Rig } from "../mascotV22/RobotMascotV22Rig";

// The V22 regression lock.
//
...
//
// None of the 27 render gates could see it. measureV22Motion decodes at 180x320
...
function heldPlan(): V22MascotPerformance {
...
describe("V22 held-element stability", () => {
  it("holds the mascot geometry byte-identical across a held window", () => {
    const plan = heldPlan();
    const frames = HELD_WINDOW.map((frame) => geometryOf(serialize(<RobotMascotV22Rig plan={plan} frame={frame} fps={30} enterOverride={1} positioning="external" />)));
  ...
  it("still animates the face and mouth inside that same window", () => {
    const plan = heldPlan();
    const rendered = HELD_WINDOW.map((frame) => serialize(<RobotMascotV22Rig plan={plan} frame={frame} fps={30} enterOverride={1} positioning="external" />));
  ...
  it("keeps the rig on whole device pixels while a gesture is mid-swing", () => {
    ...
    const offsets = Array.from({ length: 16 }, (_, index) => 5 + index).map((frame) => {
      const markup = serialize(<RobotMascotV22Rig plan={plan} frame={frame} fps={30} enterOverride={1} positioning="external" pixelScale={scale} />);
  ...
  it("never lets the camera transform change on more than a minority of frames", () => {
    ...
    const transforms = Array.from({ length: 96 }, (_, frame) => v22CameraTransform(scene, frame).transform);
  ...
  it("holds a calm scene's camera completely still through its reading window", () => {
    ...
    const window = Array.from({ length: 40 }, (_, index) => 30 + index).map((frame) => v22CameraTransform(scene, frame).transform);
