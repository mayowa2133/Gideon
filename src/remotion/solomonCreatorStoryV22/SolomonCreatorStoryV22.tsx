...
const CROPS={
  ...
  opportunityHeader:{x:72,y:75,width:760,height:265,trim:52},opportunityTitle:{x:76,y:88,width:740,height:116,trim:52},opportunityPanel:{x:75,y:95,width:900,height:520,trim:142},
  contactCard:{x:86,y:510,width:430,height:390,trim:130},contactHeader:{x:96,y:556,width:408,height:175,trim:130},contactProof:{x:104,y:610,width:390,height:265,trim:140},
  outreachBlank:{x:82,y:175,width:520,height:590,trim:85},outreachDraftCard:{x:96,y:188,width:490,height:562,trim:85},
  message:{x:602,y:196,width:742,height:300,trim:180,motion:{frames:12,step:3,hold:4}},messageDraftEdit:{x:602,y:196,width:742,height:300,trim:75,motion:{frames:12,step:3,hold:4}},messageEdit:{x:602,y:196,width:742,height:300,trim:20,motion:{frames:12,step:3,hold:4}}
...
const Role:React.FC<{scene:V22Scene}> = ({scene}) => {const frame=useCurrentFrame();return <EvidenceBackground><div style={{position:"absolute",left:40,top:400,width:1000,height:157}}><EvidenceCard><EvidenceCrop asset="opportunity" crop={CROPS.opportunityTitle} width={1000} height={157}/></EvidenceCard></div><div style={{position:"absolute",left:80,bottom:V22_CONTENT_FLOOR_BOTTOM,width:620,display:"grid",gap:10}}><ProofLabel text="PRODUCT ENGINEER" primary/><ProofLabel text="NORTHSTAR LABS"/></div></EvidenceBackground>;};
...
const Grounded:React.FC<{scene:V22Scene}> = ({scene}) => {const frame=useCurrentFrame(),reveal=interpolate(frame,[10,55],[0,1],{extrapolateLeft:"clamp",extrapolateRight:"clamp"}),arrive=spring({frame,fps:30,config:{damping:18,stiffness:220},durationInFrames:7});return <EvidenceBackground><div style={{position:"absolute",left:55,top:360,width:970,height:392,transform:`translateX(${(1-arrive)*640}px)`}}><EvidenceCard border={GREEN}><EvidenceCrop asset="outreach_complete" crop={CROPS.message} width={970} height={392}/><div style={{position:"absolute",left:34,top:299,width:900*reveal,height:77,borderRadius:12,background:"rgba(57,242,181,.24)",borderBottom:`7px solid ${MINT}`}}/></EvidenceCard></div><div style={{position:"absolute",left:80,right:80,bottom:V22_CONTENT_FLOOR_BOTTOM,display:"flex",gap:18,alignItems:"center",justifyContent:"center",fontSize:29,fontWeight:950}}><span style={{padding:"14px 18px",borderRadius:14,background:"#ffe2dc",color:MINT,textDecoration:"line-through"}}>GENERIC OPENER</span><span style={{fontSize:52}}>→</span><span style={{padding:"14px 18px",borderRadius:14,background:MINT}}>ROLE + RELEVANCE</span></div></EvidenceBackground>;};
