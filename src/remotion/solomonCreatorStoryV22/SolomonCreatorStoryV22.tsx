...
const Role:React.FC<{scene:V22Scene}> = ({scene}) => {const frame=useCurrentFrame();return <EvidenceBackground><div style={{position:"absolute",left:55,top:300,width:700,height:590}}><EvidenceCard><EvidenceCrop asset="opportunity" crop={CROPS.opportunityHeader} width={700} height={620}/></EvidenceCard></div><div style={{position:"absolute",left:80,bottom:V22_CONTENT_FLOOR_BOTTOM,width:620,display:"grid",gap:10}}><ProofLabel text="PRODUCT ENGINEER" primary/><ProofLabel text="NORTHSTAR LABS"/></div><Connector frame={frame} from={{x:410,y:930}} to={{x:410,y:970}}/></EvidenceBackground>;};

const Reason:React.FC<{scene:V22Scene}> = ({scene}) => {const frame=useCurrentFrame();return <EvidenceBackground><div style={{position:"absolute",left:340,top:280,width:690,height:780}}><EvidenceCard><EvidenceCrop asset="contact" crop={CROPS.contactCard} width={690} height={780}/></EvidenceCard></div><div style={{position:"absolute",left:350,right:55,bottom:V22_CONTENT_FLOOR_BOTTOM,display:"grid",gap:10}}>{["WORKS AT NORTHSTAR LABS","RELEVANT RECRUITING ROLE","SUPPORTING EVIDENCE INCLUDED"].map((text,index)=><ProofLabel key={text} text={text} primary={index===0} delay={index*7} frame={frame}/>)}</div></EvidenceBackground>;};
...
const Control:React.FC<{scene:V22Scene}> = ({scene}) => {const frame=useCurrentFrame();return <EvidenceBackground><div style={{position:"absolute",left:330,top:275,width:700,height:720}}><EvidenceCard border={GREEN}><EvidenceCrop asset="outreach_complete" crop={CROPS.messageEdit} width={700} height={830} playback/></EvidenceCard></div><div style={{position:"absolute",left:350,right:55,bottom:V22_CONTENT_FLOOR_BOTTOM,display:"grid",gap:10}}><ProofLabel text="SAVE EDIT" primary/><ProofLabel text="CANCEL · SEND UNTOUCHED"/><div style={{padding:"16px 20px",borderRadius:15,background:INK,color:WHITE,fontSize:27,fontWeight:950}}>NOTHING SENDS WITHOUT YOU</div></div><Cursor frame={frame} from={{x:890,y:1040}} to={{x:660,y:985}} clickAt={78}/></EvidenceBackground>;};
