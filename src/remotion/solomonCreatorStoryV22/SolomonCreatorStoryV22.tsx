...
const KineticHeadline:React.FC<{sceneId:string;accentColor:string;top:number;maxHeightPx:number;duration:number;widthPx?:number}> = ({sceneId,accentColor,top,maxHeightPx,duration,widthPx=940}) => {
  ...
  // The chunk swap used to replace the whole line in a single frame and pop it
  // from scale .62/blur 6px in 7 frames. That discontinuity scores like a cut:
  // ffmpeg read these swaps at 0.128-0.135 against a 0.10 threshold, so
  // sub-perceptual render jitter flipped them in and out and the shot count
  // moved between identical renders. The outgoing chunk now fades before it is
  // replaced and the incoming one rises from nothing, so the same change is
  // spread across frames instead of landing in one.
  const fade=Math.max(3,Math.min(7,Math.floor(hold/3)));
  // The rise has to finish inside the hold, or a short chunk fades out again
  ...
  return <div data-v22-kinetic-headline={sceneId} data-v22-kinetic-word={chunk.map(({text})=>text).join(" ")} style={{position:"absolute",left:0,right:0,top,display:"flex",flexDirection:"column",alignItems:"center",pointerEvents:"none"}}>
    {chunk.map(({text,accent})=><div key={text} style={{fontFamily:"Fraunces Variable,serif",fontSize:size,lineHeight:.92,fontWeight:900,textAlign:"center",whiteSpace:"nowrap",color:accent?accentColor:"var(--v22-fg)",transform:`scale(${.86+.14*pop})`,opacity:Math.min(1,pop*1.1)*exit,filter:`blur(${Math.max(0,(1-pop)*2.5)}px)`}}>{text}</div>)}
  </div>;
};
...
const Hook:React.FC<{scene:V22Scene}> = ({scene}) => {const frame=useCurrentFrame(),flash=interpolate(frame,[0,5,12,20],[.2,1,.55,.15]);return <AbsoluteFill ><div style={{position:"absolute",inset:0,background:`radial-gradient(circle at 50% 38%,rgba(57,242,181,${flash*.24}),transparent 48%)`}}/><SerifHeadline sceneId="hook" accentColor={CORAL} fontSize={69} top={1430}/></AbsoluteFill>;};
...
const Contact:React.FC<{scene:V22Scene;reasons:boolean}> = ({scene,reasons}) => {const frame=useCurrentFrame(),card=spring({frame,fps:30,config:{damping:16,stiffness:180},durationInFrames:8});return <EvidenceBackground accent={reasons?"#dff9f0":"#eef8f4"}><div style={{position:"absolute",left:reasons?55:340,top:reasons?280:300,width:reasons?970:680,height:reasons?530:510,transform:`translateX(${(1-card)*(reasons?-100:140)}px) scale(${.86+.14*card})`}}><EvidenceCard><EvidenceCrop asset="contact" crop={reasons?CROPS.contactProof:CROPS.contactCard} width={reasons?970:680} height={reasons?735:595}/></EvidenceCard></div>{reasons?<ReasonStack frame={frame}/>:<div style={{position:"absolute",left:350,right:55,bottom:V22_CONTENT_FLOOR_BOTTOM,display:"grid",gap:10}}><ProofLabel text="AVERY CHEN" primary/><ProofLabel text="SENIOR TECHNICAL RECRUITER"/><ProofLabel text="NORTHSTAR LABS"/></div>}</EvidenceBackground>;};

const Friction:React.FC<{scene:V22Scene}> = ({scene}) => {const frame=useCurrentFrame();return <AbsoluteFill ><KineticHeadline sceneId="friction" accentColor={CORAL} top={228} maxHeightPx={258} duration={30}/>{[{asset:"opportunity" as const,crop:CROPS.opportunityHeader},{asset:"contact" as const,crop:CROPS.contactCard},{asset:"opportunity" as const,crop:CROPS.opportunityPanel},{asset:"contact" as const,crop:CROPS.contactProof},{asset:"outreach_blank" as const,crop:CROPS.outreachBlank}].map((surface,index)=><div key={`${surface.asset}-${index}`} style={{position:"absolute",left:65+(index%2)*460,top:500+Math.floor(index/2)*250,width:420,height:205,borderRadius:27,overflow:"hidden",background:WHITE,border:"2px solid #efc4bb",boxShadow:"0 22px 50px rgba(103,44,32,.15)",transform:`translate(${interpolate(frame,[0,10+index*2],[index%2?-430:430,0],{extrapolateRight:"clamp"})}px,${Math.sin((frame+index*7)/8)*5}px) rotate(${index%2?-3:3}deg)`}}><EvidenceCrop asset={surface.asset} crop={surface.crop} width={420} height={205}/></div>)}</AbsoluteFill>;};
...
const Caption:React.FC<{caption:V22Caption;scenes:V22Scene[]}> = ({caption,scenes}) => {const frame=useCurrentFrame(),{durationInFrames}=useVideoConfig(),enter=spring({frame,fps:30,config:{damping:17,stiffness:185},durationInFrames:9}),opacity=interpolate(frame,[Math.max(0,durationInFrames-10),durationInFrames],[1,0],{extrapolateLeft:"clamp",extrapolateRight:"clamp"});
  ...
  return <div data-v22-caption={caption.id} data-v22-word-group={group.text} style={{position:"absolute",left:45,right:45,[caption.zone==="top"?"top":"bottom"]:caption.zone==="top"?82:92,display:"flex",justifyContent:"center",zIndex:70,opacity:opacity*swapFade,transform:`translateY(${(1-enter)*(caption.zone==="top"?-25:25)}px)`}}><div style={{fontFamily:"Fraunces Variable,serif",color:tone,fontSize:group.emphasis?96:82,lineHeight:.98,textAlign:"center",fontWeight:900,textShadow:`0 0 26px ${halo},0 2px 10px ${halo}`,transform:`scale(${scale})`,transformOrigin:caption.zone==="top"?"50% 0%":"50% 100%"}}>{pre}{highlighted&&<span style={{color:CORAL}}>{highlighted}</span>}{post}</div></div>;};
