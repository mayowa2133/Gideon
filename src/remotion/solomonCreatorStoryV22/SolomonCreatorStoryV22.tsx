...
import { SOLOMON_CREATOR_STORY_V22_CTA, SOLOMON_CREATOR_STORY_V22_HEADLINES, type SolomonCreatorStoryV22Manifest, type V22AssetId, type V22Caption, type V22Scene } from "../../shared/solomonCreatorStoryV22";
...
const Cta:React.FC<{scene:V22Scene}> = ({scene}) => {const frame=useCurrentFrame(),type=spring({frame:Math.max(0,frame-38),fps:30,config:{damping:20,stiffness:190},durationInFrames:16}),pulse=frame>=58?1+Math.sin((frame-58)/4)*.05*Math.max(0,1-(frame-58)/48):1;return <AbsoluteFill><div data-v22-comment-box style={{position:"absolute",left:130,right:130,top:1252,transform:`scale(${pulse})`}}><div style={{padding:"22px 28px",borderRadius:24,background:"rgba(255,255,255,.94)",border:`4px solid ${MINT}`,boxShadow:`0 0 ${34+type*46}px rgba(57,242,181,${.2+.35*type})`,display:"flex",alignItems:"center",gap:16}}><div style={{width:52,height:52,borderRadius:999,background:INK,display:"grid",placeItems:"center",color:MINT,fontSize:26,fontWeight:950}}>S</div><div style={{fontSize:38,fontWeight:950,letterSpacing:1,color:INK}}>{"SOLOMON".slice(0,Math.max(1,Math.round(type*7)))}<span style={{opacity:.35}}>|</span></div></div><div style={{marginTop:18,textAlign:"center",fontSize:24,fontWeight:900,color:"var(--v22-fg)",opacity:.92}}>{SOLOMON_CREATOR_STORY_V22_CTA}</div></div><div style={{position:"absolute",left:365,right:365,top:1636,padding:"10px 14px",borderRadius:999,background:INK,color:WHITE,textAlign:"center",fontSize:23,fontWeight:900,letterSpacing:2}}>@SOLOMON</div></AbsoluteFill>;};
