...
const Grounded:React.FC<{scene:V22Scene}> = ({scene}) => {const frame=useCurrentFrame(),reveal=interpolate(frame,[10,55],[0,1],{extrapolateLeft:"clamp",extrapolateRight:"clamp"}),arrive=spring({frame,fps:30,config:{damping:18,stiffness:220},durationInFrames:7});return <EvidenceBackground><div style={{position:"absolute",left:55,top:300,width:970,height:640,transform:`translateX(${(1-arrive)*640}px)`}}><EvidenceCard border={GREEN}><EvidenceCrop asset="outreach_complete" crop={CROPS.message} width={970} height={820}/><div style={{position:"absolute",left:67,top:328,width:760*reveal,height:72,borderRadius:12,background:"rgba(57,242,181,.24)",borderBottom:`7px solid ${MINT}`}}/></EvidenceCard></div><div style={{position:"absolute",left:80,right:80,bottom:V22_CONTENT_FLOOR_BOTTOM,display:"flex",gap:18,alignItems:"center",justifyContent:"center",fontSize:29,fontWeight:950}}><span style={{padding:"14px 18px",borderRadius:14,background:"#ffe2dc",color:MINT,textDecoration:"line-through"}}>GENERIC OPENER</span><span style={{fontSize:52}}>→</span><span style={{padding:"14px 18px",borderRadius:14,background:MINT}}>ROLE + RELEVANCE</span></div></EvidenceBackground>;};
