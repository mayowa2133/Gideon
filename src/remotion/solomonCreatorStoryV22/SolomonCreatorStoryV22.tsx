...
const ProofLabel:React.FC<{text:string;primary?:boolean;delay?:number;frame?:number}> = ({text,primary=false,delay=0,frame=30}) => {const p=spring({frame:Math.max(0,frame-delay),fps:30,config:{damping:20,stiffness:190},durationInFrames:16});return <div style={{padding:"15px 20px",borderRadius:15,background:primary?INK:WHITE,color:primary?WHITE:INK,boxShadow:"0 6px 18px rgba(7,17,31,.10)",fontSize:25,fontWeight:950,opacity:p,transform:`translateX(${(1-p)*45}px)`}}>{text}</div>;};
