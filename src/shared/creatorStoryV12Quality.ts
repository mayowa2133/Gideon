export const V12_BANNED_PRIMARY_STRINGS = [
  "seeded fixture",
  "no model was called",
...
export interface V12SemanticEvent { frame: number; kind: "semantic" | "decorative"; description: string }
export interface V12Rect { id: string; kind: "mascot" | "caption" | "editorial" | "product" | "annotation" | "cta" | "social_ui"; left: number; top: number; right: number; bottom: number }
export interface V12CompositionFingerprint { frame: number; product: V12Rect | null; mascot: V12Rect | null; background: string; cameraScale: number; semanticState: string }

export function auditV12BannedStrings(primaryOcr: string, disclosureOcr: string) {
  const normalized = normalize(primaryOcr);
  const matches = V12_BANNED_PRIMARY_STRINGS.filter((value) => normalized.includes(normalize(value)));
...
export function auditV12StoryConsistency(input: {
...
export function auditV12Hook(spoken: string, headline?: { primary: string; accentItalic: string }) {
...
export function auditV12Cta(input: { text: string; spoken: string; keyword: string; action: string; brand: string; accountIdentity: string; destinationVerified: boolean; commentDeliveryVerified: boolean }) {
...
export function auditV12Captions(captions: Array<{ id: string; from: number; to: number; beatText: string; highlight?: string }>) {
...
export function auditV12NumeralAnchors(anchors: Array<{ graphic: string; spokenToken: string; sceneId: string }>, script: string, headlines: Record<string, { primary: string; accentItalic: string }>) {
...
export function auditV12SemanticMotion(events: V12SemanticEvent[], start: number, end: number, maximumGapFrames = 24) {
...
export function auditV12Transitions(transitions: Array<{type:"cut"|"slide"|"match_cut"|"object_wipe"|"dissolve";purpose:string}>, maximumDissolves = 2) {
...
export function auditV12Layout(rectangles: V12Rect[]) {
...
export function auditV12CompositionSimilarity(fingerprints: V12CompositionFingerprint[], maximumUnchangedFrames = 60) {
...
export function auditV12PhoneScale(input:{requiredTextCoverage:number;primaryFocalPoints:number;contactReadable:boolean;messageReadable:boolean;mascotFaceReadable:boolean;captionCollisionCount:number;evidencePixelsArePrimary:boolean}){
...
function sameFingerprint(left:V12CompositionFingerprint,right:V12CompositionFingerprint){return left.background===right.background&&left.semanticState===right.semanticState&&Math.abs(left.cameraScale-right.cameraScale)<.03&&sameRect(left.product,right.product)&&sameRect(left.mascot,right.mascot);}
function sameRect(left:V12Rect|null,right:V12Rect|null){if(!left||!right)return left===right;return Math.abs(left.left-right.left)<.03&&Math.abs(left.top-right.top)<.03&&Math.abs(left.right-right.right)<.03&&Math.abs(left.bottom-right.bottom)<.03;}
function intersects(left:V12Rect,right:V12Rect){return left.left<right.right&&left.right>right.left&&left.top<right.bottom&&left.bottom>right.top;}
function forbidden(left:V12Rect["kind"],right:V12Rect["kind"]){const pair=new Set([left,right]);return pair.has("mascot")&&(pair.has("caption")||pair.has("editorial")||pair.has("product")||pair.has("annotation")||pair.has("cta"))||pair.has("product")&&(pair.has("caption")||pair.has("annotation")||pair.has("social_ui"));}
...
export const V12_CANVAS={width:1080,height:1920} as const;
...
export const V12_SAFE_AREA={marginPx:24,topChromePx:96,bottomChromePx:112} as const;
...
export const V12_CAMERA_ENVELOPE={baseScaleMax:1.012,pushMax:.013,swayXMax:6,swayYMax:5,focusShiftMax:8} as const;
...
export const V12_TEXT_ZONES={captionTopBand:[70,200] as const,headlineTopMin:212} as const;
...
export const V12_RIG={width:660,height:940} as const;

export function mascotBoxForScene(scene:{layout:V12Rect[]}){
  const rect=scene.layout.find(({kind})=>kind==="mascot");
  if(!rect)return{x:0,y:0,scale:0};
  const left=rect.left*V12_CANVAS.width,top=rect.top*V12_CANVAS.height;
  const width=(rect.right-rect.left)*V12_CANVAS.width,height=(rect.bottom-rect.top)*V12_CANVAS.height;
  ...
  const scale=Math.min(width/V12_RIG.width,height/V12_RIG.height);
  return{x:left+(width-V12_RIG.width*scale)/2,y:top+(height-V12_RIG.height*scale),scale};
}

// Confirms the box the renderer will use stays inside the rect the manifest
// declares — the invariant that makes auditV12Layout's forbidden-pair collision
// check meaningful for what is actually on screen.
export function auditV12MascotPlacement(scenes:Array<{id:string;layout:V12Rect[];mascotRole?:string}>){
  ...
  for(const scene of scenes){
    ...
    const right=box.x+V12_RIG.width*box.scale,bottom=box.y+V12_RIG.height*box.scale;
    const tolerance=1;
    if(box.x<rect.left*V12_CANVAS.width-tolerance)failures.push(`${scene.id}:mascot_left_of_declared_rect`);
    if(right>rect.right*V12_CANVAS.width+tolerance)failures.push(`${scene.id}:mascot_right_of_declared_rect`);
    if(box.y<rect.top*V12_CANVAS.height-tolerance)failures.push(`${scene.id}:mascot_above_declared_rect`);
    if(bottom>rect.bottom*V12_CANVAS.height+tolerance)failures.push(`${scene.id}:mascot_below_declared_rect`);
  }
  return{passed:failures.length===0,failures,placements};
}

export interface V12CameraPlan{scaleFrom:number;scaleTo:number;focus:{x:number;y:number}}
...
export function projectRectUnderCamera(rect:V12Rect,camera:V12CameraPlan,envelope=V12_CAMERA_ENVELOPE,canvas=V12_CANVAS){
...
export function auditV12RenderedBounds(scenes:Array<{id:string;layout:V12Rect[];camera:V12CameraPlan;mascotRole?:string}>,envelope=V12_CAMERA_ENVELOPE,safeArea=V12_SAFE_AREA,canvas=V12_CANVAS){
...
export const V12_MOTION_BANDS={nearStaticFramePercent:[10,30],medianFrameChange:[1.2,3.5],continuousMovementExcludingCuts:[1.2,4],longestLowMotionSeconds:[0,2],majorTransitionPeakCount:[12,Number.POSITIVE_INFINITY]} as const;

export function evaluateV12MotionBands(motion:{nearStaticFramePercent:number;medianFrameChange:number;continuousMovementExcludingCuts:number;longestLowMotionSeconds:number;majorTransitionPeakCount:number},bands=V12_MOTION_BANDS){
