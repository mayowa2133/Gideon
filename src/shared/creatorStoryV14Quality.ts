export const V14_BANNED_PRIMARY_STRINGS = [
  "seeded fixture",
  "no model was called",
...
export interface V14SemanticEvent { frame: number; kind: "semantic" | "decorative"; description: string }
export interface V14Rect { id: string; kind: "mascot" | "caption" | "editorial" | "product" | "annotation" | "cta" | "social_ui"; left: number; top: number; right: number; bottom: number }
export interface V14CompositionFingerprint { frame: number; product: V14Rect | null; mascot: V14Rect | null; background: string; cameraScale: number; semanticState: string }

export function auditV14BannedStrings(primaryOcr: string, disclosureOcr: string) {
  const normalized = normalize(primaryOcr);
  const matches = V14_BANNED_PRIMARY_STRINGS.filter((value) => normalized.includes(normalize(value)));
...
export function auditV14StoryConsistency(input: {
...
export function auditV14Hook(spoken: string, headline?: { primary: string; accentItalic: string }) {
...
export function auditV14Cta(input: { text: string; spoken: string; keyword: string; action: string; brand: string; accountIdentity: string; destinationVerified: boolean; commentDeliveryVerified: boolean }) {
...
export function auditV14Captions(captions: Array<{ id: string; from: number; to: number; beatText: string; highlight?: string }>) {
...
export function auditV14NumeralAnchors(anchors: Array<{ graphic: string; spokenToken: string; sceneId: string }>, script: string, headlines: Record<string, { primary: string; accentItalic: string }>) {
...
export function auditV14SemanticMotion(events: V14SemanticEvent[], start: number, end: number, maximumGapFrames = 24) {
...
export function auditV14Transitions(transitions: Array<{type:"cut"|"slide"|"match_cut"|"object_wipe"|"dissolve";purpose:string}>, maximumDissolves = 2) {
...
export function auditV14Layout(rectangles: V14Rect[]) {
...
export function auditV14CompositionSimilarity(fingerprints: V14CompositionFingerprint[], maximumUnchangedFrames = 60) {
...
export function auditV14PhoneScale(input:{requiredTextCoverage:number;primaryFocalPoints:number;contactReadable:boolean;messageReadable:boolean;mascotFaceReadable:boolean;captionCollisionCount:number;evidencePixelsArePrimary:boolean}){
...
function sameFingerprint(left:V14CompositionFingerprint,right:V14CompositionFingerprint){return left.background===right.background&&left.semanticState===right.semanticState&&Math.abs(left.cameraScale-right.cameraScale)<.03&&sameRect(left.product,right.product)&&sameRect(left.mascot,right.mascot);}
function sameRect(left:V14Rect|null,right:V14Rect|null){if(!left||!right)return left===right;return Math.abs(left.left-right.left)<.03&&Math.abs(left.top-right.top)<.03&&Math.abs(left.right-right.right)<.03&&Math.abs(left.bottom-right.bottom)<.03;}
function intersects(left:V14Rect,right:V14Rect){return left.left<right.right&&left.right>right.left&&left.top<right.bottom&&left.bottom>right.top;}
function forbidden(left:V14Rect["kind"],right:V14Rect["kind"]){const pair=new Set([left,right]);return pair.has("mascot")&&(pair.has("caption")||pair.has("editorial")||pair.has("product")||pair.has("annotation")||pair.has("cta"))||pair.has("product")&&(pair.has("caption")||pair.has("annotation")||pair.has("social_ui"));}
...
export const V14_CANVAS={width:1080,height:1920} as const;
...
export const V14_SAFE_AREA={marginPx:24,topChromePx:96,bottomChromePx:112} as const;
...
export const V14_CAMERA_ENVELOPE={baseScaleMax:1.012,pushMax:.013,swayXMax:6,swayYMax:5,focusShiftMax:8} as const;
...
export const V14_TEXT_ZONES={captionTopBand:[70,200] as const,headlineTopMin:212} as const;
...
export const V14_RIG={width:660,height:940} as const;

export function mascotBoxForScene(scene:{layout:V14Rect[]}){
  const rect=scene.layout.find(({kind})=>kind==="mascot");
  if(!rect)return{x:0,y:0,scale:0};
  const left=rect.left*V14_CANVAS.width,top=rect.top*V14_CANVAS.height;
  const width=(rect.right-rect.left)*V14_CANVAS.width,height=(rect.bottom-rect.top)*V14_CANVAS.height;
  ...
  const scale=Math.min(width/V14_RIG.width,height/V14_RIG.height);
  return{x:left+(width-V14_RIG.width*scale)/2,y:top+(height-V14_RIG.height*scale),scale};
}

// Confirms the box the renderer will use stays inside the rect the manifest
// declares — the invariant that makes auditV14Layout's forbidden-pair collision
// check meaningful for what is actually on screen.
export function auditV14MascotPlacement(scenes:Array<{id:string;layout:V14Rect[];mascotRole?:string}>){
  ...
  for(const scene of scenes){
    ...
    const right=box.x+V14_RIG.width*box.scale,bottom=box.y+V14_RIG.height*box.scale;
    const tolerance=1;
    if(box.x<rect.left*V14_CANVAS.width-tolerance)failures.push(`${scene.id}:mascot_left_of_declared_rect`);
    if(right>rect.right*V14_CANVAS.width+tolerance)failures.push(`${scene.id}:mascot_right_of_declared_rect`);
    if(box.y<rect.top*V14_CANVAS.height-tolerance)failures.push(`${scene.id}:mascot_above_declared_rect`);
    if(bottom>rect.bottom*V14_CANVAS.height+tolerance)failures.push(`${scene.id}:mascot_below_declared_rect`);
  }
  return{passed:failures.length===0,failures,placements};
}

export interface V14CameraPlan{scaleFrom:number;scaleTo:number;focus:{x:number;y:number}}
...
export function projectRectUnderCamera(rect:V14Rect,camera:V14CameraPlan,envelope=V14_CAMERA_ENVELOPE,canvas=V14_CANVAS){
...
export function auditV14RenderedBounds(scenes:Array<{id:string;layout:V14Rect[];camera:V14CameraPlan;mascotRole?:string}>,envelope=V14_CAMERA_ENVELOPE,safeArea=V14_SAFE_AREA,canvas=V14_CANVAS){
...
export const V14_MOTION_BANDS={nearStaticFramePercent:[10,30],medianFrameChange:[1.2,3.5],continuousMovementExcludingCuts:[1.2,4],longestLowMotionSeconds:[0,2],majorTransitionPeakCount:[12,Number.POSITIVE_INFINITY]} as const;

export function evaluateV14MotionBands(motion:{nearStaticFramePercent:number;medianFrameChange:number;continuousMovementExcludingCuts:number;longestLowMotionSeconds:number;majorTransitionPeakCount:number},bands=V14_MOTION_BANDS){
