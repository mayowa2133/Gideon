export const V16_BANNED_PRIMARY_STRINGS = [
  "seeded fixture",
  "no model was called",
...
export interface V16SemanticEvent { frame: number; kind: "semantic" | "decorative"; description: string }
export interface V16Rect { id: string; kind: "mascot" | "caption" | "editorial" | "product" | "annotation" | "cta" | "social_ui"; left: number; top: number; right: number; bottom: number }
export interface V16CompositionFingerprint { frame: number; product: V16Rect | null; mascot: V16Rect | null; background: string; cameraScale: number; semanticState: string }

export function auditV16BannedStrings(primaryOcr: string, disclosureOcr: string) {
  const normalized = normalize(primaryOcr);
  const matches = V16_BANNED_PRIMARY_STRINGS.filter((value) => normalized.includes(normalize(value)));
...
export function auditV16StoryConsistency(input: {
...
export function auditV16Hook(spoken: string, headline?: { primary: string; accentItalic: string }) {
...
export function auditV16Cta(input: { text: string; spoken: string; keyword: string; action: string; brand: string; accountIdentity: string; destinationVerified: boolean; commentDeliveryVerified: boolean }) {
...
export function auditV16Captions(captions: Array<{ id: string; from: number; to: number; beatText: string; highlight?: string }>) {
...
export function auditV16NumeralAnchors(anchors: Array<{ graphic: string; spokenToken: string; sceneId: string }>, script: string, headlines: Record<string, { primary: string; accentItalic: string }>) {
...
export function auditV16SemanticMotion(events: V16SemanticEvent[], start: number, end: number, maximumGapFrames = 24) {
...
export function auditV16Transitions(transitions: Array<{type:"cut"|"slide"|"match_cut"|"object_wipe"|"dissolve";purpose:string}>, maximumDissolves = 2) {
...
export function auditV16Layout(rectangles: V16Rect[]) {
...
export function auditV16CompositionSimilarity(fingerprints: V16CompositionFingerprint[], maximumUnchangedFrames = 60) {
...
export function auditV16PhoneScale(input:{requiredTextCoverage:number;primaryFocalPoints:number;contactReadable:boolean;messageReadable:boolean;mascotFaceReadable:boolean;captionCollisionCount:number;evidencePixelsArePrimary:boolean}){
...
function sameFingerprint(left:V16CompositionFingerprint,right:V16CompositionFingerprint){return left.background===right.background&&left.semanticState===right.semanticState&&Math.abs(left.cameraScale-right.cameraScale)<.03&&sameRect(left.product,right.product)&&sameRect(left.mascot,right.mascot);}
function sameRect(left:V16Rect|null,right:V16Rect|null){if(!left||!right)return left===right;return Math.abs(left.left-right.left)<.03&&Math.abs(left.top-right.top)<.03&&Math.abs(left.right-right.right)<.03&&Math.abs(left.bottom-right.bottom)<.03;}
function intersects(left:V16Rect,right:V16Rect){return left.left<right.right&&left.right>right.left&&left.top<right.bottom&&left.bottom>right.top;}
function forbidden(left:V16Rect["kind"],right:V16Rect["kind"]){const pair=new Set([left,right]);return pair.has("mascot")&&(pair.has("caption")||pair.has("editorial")||pair.has("product")||pair.has("annotation")||pair.has("cta"))||pair.has("product")&&(pair.has("caption")||pair.has("annotation")||pair.has("social_ui"));}
...
export const V16_CANVAS={width:1080,height:1920} as const;
...
export const V16_SAFE_AREA={marginPx:24,topChromePx:96,bottomChromePx:112} as const;
...
export const V16_CAMERA_ENVELOPE={baseScaleMax:1.012,pushMax:.013,swayXMax:6,swayYMax:5,focusShiftMax:8} as const;
...
export const V16_TEXT_ZONES={captionTopBand:[70,200] as const,headlineTopMin:212} as const;
...
export const V16_RIG={width:660,height:940} as const;

export function mascotBoxForScene(scene:{layout:V16Rect[]}){
  const rect=scene.layout.find(({kind})=>kind==="mascot");
  if(!rect)return{x:0,y:0,scale:0};
  const left=rect.left*V16_CANVAS.width,top=rect.top*V16_CANVAS.height;
  const width=(rect.right-rect.left)*V16_CANVAS.width,height=(rect.bottom-rect.top)*V16_CANVAS.height;
  ...
  const scale=Math.min(width/V16_RIG.width,height/V16_RIG.height);
  return{x:left+(width-V16_RIG.width*scale)/2,y:top+(height-V16_RIG.height*scale),scale};
}

// Confirms the box the renderer will use stays inside the rect the manifest
// declares — the invariant that makes auditV16Layout's forbidden-pair collision
// check meaningful for what is actually on screen.
export function auditV16MascotPlacement(scenes:Array<{id:string;layout:V16Rect[];mascotRole?:string}>){
  ...
  for(const scene of scenes){
    ...
    const right=box.x+V16_RIG.width*box.scale,bottom=box.y+V16_RIG.height*box.scale;
    const tolerance=1;
    if(box.x<rect.left*V16_CANVAS.width-tolerance)failures.push(`${scene.id}:mascot_left_of_declared_rect`);
    if(right>rect.right*V16_CANVAS.width+tolerance)failures.push(`${scene.id}:mascot_right_of_declared_rect`);
    if(box.y<rect.top*V16_CANVAS.height-tolerance)failures.push(`${scene.id}:mascot_above_declared_rect`);
    if(bottom>rect.bottom*V16_CANVAS.height+tolerance)failures.push(`${scene.id}:mascot_below_declared_rect`);
  }
  return{passed:failures.length===0,failures,placements};
}

export interface V16CameraPlan{scaleFrom:number;scaleTo:number;focus:{x:number;y:number}}
...
export function projectRectUnderCamera(rect:V16Rect,camera:V16CameraPlan,envelope=V16_CAMERA_ENVELOPE,canvas=V16_CANVAS){
...
export function auditV16RenderedBounds(scenes:Array<{id:string;layout:V16Rect[];camera:V16CameraPlan;mascotRole?:string}>,envelope=V16_CAMERA_ENVELOPE,safeArea=V16_SAFE_AREA,canvas=V16_CANVAS){
...
export const V16_MOTION_BANDS={nearStaticFramePercent:[10,30],medianFrameChange:[1.2,3.5],continuousMovementExcludingCuts:[1.2,4],longestLowMotionSeconds:[0,2],majorTransitionPeakCount:[12,Number.POSITIVE_INFINITY]} as const;

export function evaluateV16MotionBands(motion:{nearStaticFramePercent:number;medianFrameChange:number;continuousMovementExcludingCuts:number;longestLowMotionSeconds:number;majorTransitionPeakCount:number},bands=V16_MOTION_BANDS){
