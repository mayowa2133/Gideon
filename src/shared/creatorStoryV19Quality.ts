export const V19_BANNED_PRIMARY_STRINGS = [
  "seeded fixture",
  "no model was called",
...
export interface V19SemanticEvent { frame: number; kind: "semantic" | "decorative"; description: string }
export interface V19Rect { id: string; kind: "mascot" | "caption" | "editorial" | "product" | "annotation" | "cta" | "social_ui"; left: number; top: number; right: number; bottom: number }
export interface V19CompositionFingerprint { frame: number; product: V19Rect | null; mascot: V19Rect | null; background: string; cameraScale: number; semanticState: string }

export function auditV19BannedStrings(primaryOcr: string, disclosureOcr: string) {
  const normalized = normalize(primaryOcr);
  const matches = V19_BANNED_PRIMARY_STRINGS.filter((value) => normalized.includes(normalize(value)));
...
export function auditV19StoryConsistency(input: {
...
export function auditV19Hook(spoken: string, headline?: { primary: string; accentItalic: string }) {
...
export function auditV19Cta(input: { text: string; spoken: string; keyword: string; action: string; brand: string; accountIdentity: string; destinationVerified: boolean; commentDeliveryVerified: boolean }) {
...
export function auditV19Captions(captions: Array<{ id: string; from: number; to: number; beatText: string; highlight?: string }>) {
...
export function auditV19NumeralAnchors(anchors: Array<{ graphic: string; spokenToken: string; sceneId: string }>, script: string, headlines: Record<string, { primary: string; accentItalic: string }>) {
...
export function auditV19SemanticMotion(events: V19SemanticEvent[], start: number, end: number, maximumGapFrames = 24) {
...
export function auditV19Transitions(transitions: Array<{type:"cut"|"slide"|"match_cut"|"object_wipe"|"dissolve";purpose:string}>, maximumDissolves = 2) {
...
export function auditV19Layout(rectangles: V19Rect[]) {
...
export function auditV19CompositionSimilarity(fingerprints: V19CompositionFingerprint[], maximumUnchangedFrames = 60) {
...
export function auditV19PhoneScale(input:{requiredTextCoverage:number;primaryFocalPoints:number;contactReadable:boolean;messageReadable:boolean;mascotFaceReadable:boolean;captionCollisionCount:number;evidencePixelsArePrimary:boolean}){
...
function sameFingerprint(left:V19CompositionFingerprint,right:V19CompositionFingerprint){return left.background===right.background&&left.semanticState===right.semanticState&&Math.abs(left.cameraScale-right.cameraScale)<.03&&sameRect(left.product,right.product)&&sameRect(left.mascot,right.mascot);}
function sameRect(left:V19Rect|null,right:V19Rect|null){if(!left||!right)return left===right;return Math.abs(left.left-right.left)<.03&&Math.abs(left.top-right.top)<.03&&Math.abs(left.right-right.right)<.03&&Math.abs(left.bottom-right.bottom)<.03;}
function intersects(left:V19Rect,right:V19Rect){return left.left<right.right&&left.right>right.left&&left.top<right.bottom&&left.bottom>right.top;}
function forbidden(left:V19Rect["kind"],right:V19Rect["kind"]){const pair=new Set([left,right]);return pair.has("mascot")&&(pair.has("caption")||pair.has("editorial")||pair.has("product")||pair.has("annotation")||pair.has("cta"))||pair.has("product")&&(pair.has("caption")||pair.has("annotation")||pair.has("social_ui"));}
...
export const V19_CANVAS={width:1080,height:1920} as const;
...
export const V19_SAFE_AREA={marginPx:24,topChromePx:96,bottomChromePx:112} as const;
...
export const V19_CAMERA_ENVELOPE={baseScaleMax:1.012,pushMax:.013,swayXMax:6,swayYMax:5,focusShiftMax:8} as const;
...
export const V19_TEXT_ZONES={captionTopBand:[70,200] as const,headlineTopMin:212} as const;
...
export const V19_RIG={width:660,height:940} as const;

export function mascotBoxForScene(scene:{layout:V19Rect[]}){
  const rect=scene.layout.find(({kind})=>kind==="mascot");
  if(!rect)return{x:0,y:0,scale:0};
  const left=rect.left*V19_CANVAS.width,top=rect.top*V19_CANVAS.height;
  const width=(rect.right-rect.left)*V19_CANVAS.width,height=(rect.bottom-rect.top)*V19_CANVAS.height;
  ...
  const scale=Math.min(width/V19_RIG.width,height/V19_RIG.height);
  return{x:left+(width-V19_RIG.width*scale)/2,y:top+(height-V19_RIG.height*scale),scale};
}

// Confirms the box the renderer will use stays inside the rect the manifest
// declares — the invariant that makes auditV19Layout's forbidden-pair collision
...
export const V19_RIG_BLEED={topPx:48} as const;
...
export type V19BackdropToken=keyof typeof V19_BACKDROPS;
...
export function auditV19BackdropCadence(scenes:Array<{id:string;backdrop:V19BackdropToken}>){
  const failures:string[]=[];
  scenes.forEach((scene,index)=>{
    if(index===0)return;
    const previous=scenes[index-1]!;
    if(scene.backdrop===previous.backdrop)failures.push(`${scene.id}:backdrop_repeats_previous`);
    else if(V19_BACKDROPS[scene.backdrop].tier===V19_BACKDROPS[previous.backdrop].tier)failures.push(`${scene.id}:tier_repeats_previous`);
  });
  return{passed:failures.length===0,failures};
}
...
export const V19_SHOT_BANDS={decodedShotCount:[13,20],meanShotSeconds:[1.7,2.7],medianShotSeconds:[1.4,2.4]} as const;

export function evaluateV19ShotBands(shots:{count:number;meanSeconds:number;medianSeconds:number}){
  const checks=[
    {metric:"decodedShotCount",value:shots.count,low:V19_SHOT_BANDS.decodedShotCount[0],high:V19_SHOT_BANDS.decodedShotCount[1]},
    {metric:"meanShotSeconds",value:shots.meanSeconds,low:V19_SHOT_BANDS.meanShotSeconds[0],high:V19_SHOT_BANDS.meanShotSeconds[1]},
    {metric:"medianShotSeconds",value:shots.medianSeconds,low:V19_SHOT_BANDS.medianShotSeconds[0],high:V19_SHOT_BANDS.medianShotSeconds[1]},
  ].map((check)=>({...check,passed:check.value>=check.low&&check.value<=check.high}));
  return{passed:checks.every(({passed})=>passed),checks};
}


export function auditV19MascotPlacement(scenes:Array<{id:string;layout:V19Rect[];mascotRole?:string}>){
  ...
  for(const scene of scenes){
    ...
    const bleedTop=box.y-V19_RIG_BLEED.topPx*box.scale;
    placements.push({sceneId:scene.id,box,bleedTop});
    if(box.scale<=0){failures.push(`${scene.id}:mascot_box_degenerate`);continue;}
    const right=box.x+V19_RIG.width*box.scale,bottom=box.y+V19_RIG.height*box.scale;
    const tolerance=1;
    if(box.x<rect.left*V19_CANVAS.width-tolerance)failures.push(`${scene.id}:mascot_left_of_declared_rect`);
    if(right>rect.right*V19_CANVAS.width+tolerance)failures.push(`${scene.id}:mascot_right_of_declared_rect`);
    if(box.y<rect.top*V19_CANVAS.height-tolerance)failures.push(`${scene.id}:mascot_above_declared_rect`);
    if(bottom>rect.bottom*V19_CANVAS.height+tolerance)failures.push(`${scene.id}:mascot_below_declared_rect`);
    ...
    for(const cta of scene.layout.filter(({kind})=>kind==="cta")){
      const overlapsY=bleedTop<cta.bottom*V19_CANVAS.height&&bottom>cta.top*V19_CANVAS.height;
      const overlapsX=box.x<cta.right*V19_CANVAS.width&&right>cta.left*V19_CANVAS.width;
...
export interface V19CameraPlan{scaleFrom:number;scaleTo:number;focus:{x:number;y:number}}
...
export function projectRectUnderCamera(rect:V19Rect,camera:V19CameraPlan,envelope=V19_CAMERA_ENVELOPE,canvas=V19_CANVAS){
...
export function auditV19RenderedBounds(scenes:Array<{id:string;layout:V19Rect[];camera:V19CameraPlan;mascotRole?:string}>,envelope=V19_CAMERA_ENVELOPE,safeArea=V19_SAFE_AREA,canvas=V19_CANVAS){
...
// Floors are deliberately NOT raised: V19 does not reach reference continuous
// motion, and lifting them would force decorative drift - the exact V10 failure
// these bands were introduced to stop.
export const V19_MOTION_BANDS={nearStaticFramePercent:[10,30],medianFrameChange:[1.2,8],continuousMovementExcludingCuts:[1.2,5.5],longestLowMotionSeconds:[0,4],majorTransitionPeakCount:[12,Number.POSITIVE_INFINITY]} as const;

export function evaluateV19MotionBands(motion:{nearStaticFramePercent:number;medianFrameChange:number;continuousMovementExcludingCuts:number;longestLowMotionSeconds:number;majorTransitionPeakCount:number},bands=V19_MOTION_BANDS){
