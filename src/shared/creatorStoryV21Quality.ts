export const V21_BANNED_PRIMARY_STRINGS = [
  "seeded fixture",
  "no model was called",
...
export interface V21SemanticEvent { frame: number; kind: "semantic" | "decorative"; description: string }
export interface V21Rect { id: string; kind: "mascot" | "caption" | "editorial" | "product" | "annotation" | "cta" | "social_ui"; left: number; top: number; right: number; bottom: number }
export interface V21CompositionFingerprint { frame: number; product: V21Rect | null; mascot: V21Rect | null; background: string; cameraScale: number; semanticState: string }

export function auditV21BannedStrings(primaryOcr: string, disclosureOcr: string) {
  const normalized = normalize(primaryOcr);
  const matches = V21_BANNED_PRIMARY_STRINGS.filter((value) => normalized.includes(normalize(value)));
...
export function auditV21StoryConsistency(input: {
...
export function auditV21Hook(spoken: string, headline?: { primary: string; accentItalic: string }) {
...
export function auditV21Cta(input: { text: string; spoken: string; keyword: string; action: string; brand: string; accountIdentity: string; destinationVerified: boolean; commentDeliveryVerified: boolean }) {
...
export function auditV21Captions(captions: Array<{ id: string; from: number; to: number; beatText: string; highlight?: string }>) {
...
export function auditV21NumeralAnchors(anchors: Array<{ graphic: string; spokenToken: string; sceneId: string }>, script: string, headlines: Record<string, { primary: string; accentItalic: string }>) {
...
export function auditV21SemanticMotion(events: V21SemanticEvent[], start: number, end: number, maximumGapFrames = 24) {
...
export function auditV21Transitions(transitions: Array<{type:"cut"|"slide"|"match_cut"|"object_wipe"|"dissolve";purpose:string}>, maximumDissolves = 2) {
...
export function auditV21Layout(rectangles: V21Rect[]) {
...
export function auditV21CompositionSimilarity(fingerprints: V21CompositionFingerprint[], maximumUnchangedFrames = 60) {
...
export function auditV21PhoneScale(input:{requiredTextCoverage:number;primaryFocalPoints:number;contactReadable:boolean;messageReadable:boolean;mascotFaceReadable:boolean;captionCollisionCount:number;evidencePixelsArePrimary:boolean}){
...
function sameFingerprint(left:V21CompositionFingerprint,right:V21CompositionFingerprint){return left.background===right.background&&left.semanticState===right.semanticState&&Math.abs(left.cameraScale-right.cameraScale)<.03&&sameRect(left.product,right.product)&&sameRect(left.mascot,right.mascot);}
function sameRect(left:V21Rect|null,right:V21Rect|null){if(!left||!right)return left===right;return Math.abs(left.left-right.left)<.03&&Math.abs(left.top-right.top)<.03&&Math.abs(left.right-right.right)<.03&&Math.abs(left.bottom-right.bottom)<.03;}
function intersects(left:V21Rect,right:V21Rect){return left.left<right.right&&left.right>right.left&&left.top<right.bottom&&left.bottom>right.top;}
function forbidden(left:V21Rect["kind"],right:V21Rect["kind"]){const pair=new Set([left,right]);return pair.has("mascot")&&(pair.has("caption")||pair.has("editorial")||pair.has("product")||pair.has("annotation")||pair.has("cta"))||pair.has("product")&&(pair.has("caption")||pair.has("annotation")||pair.has("social_ui"));}
...
export const V21_CANVAS={width:1080,height:1920} as const;
...
export const V21_SAFE_AREA={marginPx:24,topChromePx:96,bottomChromePx:112} as const;
...
//
// V21 sets both sway maxima to 0. Through V20 the camera translated the whole
...
// auditV21RenderedBounds still compute — with sway 0 their worst case simply
// tightens, which can only make the bounds check safer.
export const V21_CAMERA_ENVELOPE={baseScaleMax:1.012,pushMax:.013,swayXMax:0,swayYMax:0,focusShiftMax:8} as const;
...
export const V21_CAMERA_SCALE_RADIUS=960;
...
// assertion is the actual regression lock for V21: it needs no render, no
// threshold, and no calibration, and it cannot drift the way a tuned metric can.
export function v21CameraTransform(scene:{from:number;to:number;typography:string;camera:{scaleFrom:number;scaleTo:number;focus:{x:number;y:number}}},frame:number){
  ...
  const push=progress*Math.min(V21_CAMERA_ENVELOPE.pushMax,declaredTravel*.1);
  const calm=scene.typography==="product_annotation";
  const focusX=(scene.camera.focus.x-.5)*-V21_CAMERA_ENVELOPE.focusShiftMax,focusY=(scene.camera.focus.y-.5)*-V21_CAMERA_ENVELOPE.focusShiftMax;
  const heldPush=calm&&progress>.2&&progress<.8?.2*Math.min(V21_CAMERA_ENVELOPE.pushMax,declaredTravel*.1):push;
  const translateX=Math.round(focusX*progress),translateY=Math.round(focusY*progress);
  const scale=1+Math.round((V21_CAMERA_ENVELOPE.baseScaleMax+heldPush-1)*V21_CAMERA_SCALE_RADIUS)/V21_CAMERA_SCALE_RADIUS;
...
export const V21_TEXT_ZONES={captionTopBand:[70,200] as const,headlineTopMin:212} as const;
...
export const V21_RIG={width:660,height:940} as const;

export function mascotBoxForScene(scene:{layout:V21Rect[]}){
  const rect=scene.layout.find(({kind})=>kind==="mascot");
  if(!rect)return{x:0,y:0,scale:0};
  const left=rect.left*V21_CANVAS.width,top=rect.top*V21_CANVAS.height;
  const width=(rect.right-rect.left)*V21_CANVAS.width,height=(rect.bottom-rect.top)*V21_CANVAS.height;
  ...
  const scale=Math.min(width/V21_RIG.width,height/V21_RIG.height);
  return{x:left+(width-V21_RIG.width*scale)/2,y:top+(height-V21_RIG.height*scale),scale};
}

// Confirms the box the renderer will use stays inside the rect the manifest
// declares — the invariant that makes auditV21Layout's forbidden-pair collision
...
export const V21_RIG_BLEED={topPx:48} as const;
...
export type V21BackdropToken=keyof typeof V21_BACKDROPS;
...
export function auditV21BackdropCadence(scenes:Array<{id:string;backdrop:V21BackdropToken}>){
  const failures:string[]=[];
  scenes.forEach((scene,index)=>{
    if(index===0)return;
    const previous=scenes[index-1]!;
    if(scene.backdrop===previous.backdrop)failures.push(`${scene.id}:backdrop_repeats_previous`);
    else if(V21_BACKDROPS[scene.backdrop].tier===V21_BACKDROPS[previous.backdrop].tier)failures.push(`${scene.id}:tier_repeats_previous`);
  });
  return{passed:failures.length===0,failures};
}
...
export const V21_SHOT_BANDS={decodedShotCount:[13,20],meanShotSeconds:[1.7,2.7],medianShotSeconds:[1.4,2.4]} as const;

export function evaluateV21ShotBands(shots:{count:number;meanSeconds:number;medianSeconds:number}){
  const checks=[
    {metric:"decodedShotCount",value:shots.count,low:V21_SHOT_BANDS.decodedShotCount[0],high:V21_SHOT_BANDS.decodedShotCount[1]},
    {metric:"meanShotSeconds",value:shots.meanSeconds,low:V21_SHOT_BANDS.meanShotSeconds[0],high:V21_SHOT_BANDS.meanShotSeconds[1]},
    {metric:"medianShotSeconds",value:shots.medianSeconds,low:V21_SHOT_BANDS.medianShotSeconds[0],high:V21_SHOT_BANDS.medianShotSeconds[1]},
  ].map((check)=>({...check,passed:check.value>=check.low&&check.value<=check.high}));
  return{passed:checks.every(({passed})=>passed),checks};
}


export function auditV21MascotPlacement(scenes:Array<{id:string;layout:V21Rect[];mascotRole?:string}>){
  ...
  for(const scene of scenes){
    ...
    const bleedTop=box.y-V21_RIG_BLEED.topPx*box.scale;
    placements.push({sceneId:scene.id,box,bleedTop});
    if(box.scale<=0){failures.push(`${scene.id}:mascot_box_degenerate`);continue;}
    const right=box.x+V21_RIG.width*box.scale,bottom=box.y+V21_RIG.height*box.scale;
    const tolerance=1;
    if(box.x<rect.left*V21_CANVAS.width-tolerance)failures.push(`${scene.id}:mascot_left_of_declared_rect`);
    if(right>rect.right*V21_CANVAS.width+tolerance)failures.push(`${scene.id}:mascot_right_of_declared_rect`);
    if(box.y<rect.top*V21_CANVAS.height-tolerance)failures.push(`${scene.id}:mascot_above_declared_rect`);
    if(bottom>rect.bottom*V21_CANVAS.height+tolerance)failures.push(`${scene.id}:mascot_below_declared_rect`);
    ...
    for(const cta of scene.layout.filter(({kind})=>kind==="cta")){
      const overlapsY=bleedTop<cta.bottom*V21_CANVAS.height&&bottom>cta.top*V21_CANVAS.height;
      const overlapsX=box.x<cta.right*V21_CANVAS.width&&right>cta.left*V21_CANVAS.width;
...
export interface V21CameraPlan{scaleFrom:number;scaleTo:number;focus:{x:number;y:number}}
...
export function projectRectUnderCamera(rect:V21Rect,camera:V21CameraPlan,envelope=V21_CAMERA_ENVELOPE,canvas=V21_CANVAS){
...
export function auditV21RenderedBounds(scenes:Array<{id:string;layout:V21Rect[];camera:V21CameraPlan;mascotRole?:string}>,envelope=V21_CAMERA_ENVELOPE,safeArea=V21_SAFE_AREA,canvas=V21_CANVAS){
...
// Floors are deliberately NOT raised: V21 does not reach reference continuous
// motion, and lifting them would force decorative drift - the exact V10 failure
// these bands were introduced to stop.
export const V21_MOTION_BANDS={nearStaticFramePercent:[10,30],medianFrameChange:[1.2,8],continuousMovementExcludingCuts:[1.2,5.5],longestLowMotionSeconds:[0,4],majorTransitionPeakCount:[12,Number.POSITIVE_INFINITY]} as const;

export function evaluateV21MotionBands(motion:{nearStaticFramePercent:number;medianFrameChange:number;continuousMovementExcludingCuts:number;longestLowMotionSeconds:number;majorTransitionPeakCount:number},bands=V21_MOTION_BANDS){
