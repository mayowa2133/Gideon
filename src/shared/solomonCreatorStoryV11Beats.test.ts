...
import { compileSolomonCreatorStoryV11, SOLOMON_CREATOR_STORY_V11_BEATS } from "./solomonCreatorStoryV11Beats";

describe("Solomon Creator Story V11 beat compiler", () => {
  const compiled = compileSolomonCreatorStoryV11();

  it("compiles one source of truth for script, captions, headlines, and TTS beats", () => {
    expect(compiled.script).toBe(SOLOMON_CREATOR_STORY_V11_BEATS.filter(({ vo }) => vo).map(({ vo }) => vo).join(" "));
    expect(compiled.hook).toBe(SOLOMON_CREATOR_STORY_V11_BEATS[0]!.vo);
    expect(compiled.ttsBeats.map(({ text }) => text).join(" ")).toBe(compiled.script);
    for (const caption of compiled.captions) {
      const beat = SOLOMON_CREATOR_STORY_V11_BEATS.find(({ id }) => id === caption.id);
