...
import { compileSolomonCreatorStoryV12, SOLOMON_CREATOR_STORY_V12_BEATS } from "./solomonCreatorStoryV12Beats";

describe("Solomon Creator Story V12 beat compiler", () => {
  const compiled = compileSolomonCreatorStoryV12();

  it("compiles one source of truth for script, captions, headlines, and TTS beats", () => {
    expect(compiled.script).toBe(SOLOMON_CREATOR_STORY_V12_BEATS.filter(({ vo }) => vo).map(({ vo }) => vo).join(" "));
    expect(compiled.hook).toBe(SOLOMON_CREATOR_STORY_V12_BEATS[0]!.vo);
    expect(compiled.ttsBeats.map(({ text }) => text).join(" ")).toBe(compiled.script);
    for (const caption of compiled.captions) {
      const beat = SOLOMON_CREATOR_STORY_V12_BEATS.find(({ id }) => id === caption.id);
