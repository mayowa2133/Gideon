// Single source of truth for every V12 text surface. The VO script, TTS beats,
...
export interface V12CompiledCaption {
  id: string;
  from: number;
  to: number;
  text: string;
...
export interface V12BeatHeadline { primary: string; accentItalic: string }
export interface V12NumeralAnchor { graphic: string; spokenToken: string; sceneId: string }
export interface V12Beat {
  id: string;
  sceneId: string;
  vo: string;
  energy: "high" | "medium";
  chip?: Omit<V12CompiledCaption, "id" | "text">;
  headline?: V12BeatHeadline;
  anchors?: V12NumeralAnchor[];
}

export const SOLOMON_CREATOR_STORY_V12_BEATS: readonly V12Beat[] = [
  {
    id: "hook",
    sceneId: "hook",
...
export interface CompiledSolomonCreatorStoryV12 {
  script: string;
  hook: string;
  ctaSpoken: string;
  ctaDisplay: string;
  ...
  captions: V12CompiledCaption[];
  headlines: Record<string, V12BeatHeadline>;
  ttsBeats: Array<{ id: string; sceneId: string; text: string; energy: "high" | "medium" }>;
  numeralAnchors: V12NumeralAnchor[];
}

export function compileSolomonCreatorStoryV12(): CompiledSolomonCreatorStoryV12 {
  const beats = SOLOMON_CREATOR_STORY_V12_BEATS;
  ...
  const headlines: Record<string, V12BeatHeadline> = {};
