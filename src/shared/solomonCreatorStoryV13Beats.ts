// Single source of truth for every V13 text surface. The VO script, TTS beats,
...
export interface V13CompiledCaption {
  id: string;
  from: number;
  to: number;
  text: string;
...
export interface V13BeatHeadline { primary: string; accentItalic: string }
export interface V13NumeralAnchor { graphic: string; spokenToken: string; sceneId: string }
export interface V13Beat {
  id: string;
  sceneId: string;
  vo: string;
  energy: "high" | "medium";
  chip?: Omit<V13CompiledCaption, "id" | "text">;
  headline?: V13BeatHeadline;
  anchors?: V13NumeralAnchor[];
}

export const SOLOMON_CREATOR_STORY_V13_BEATS: readonly V13Beat[] = [
  {
    id: "hook",
    sceneId: "hook",
...
export interface CompiledSolomonCreatorStoryV13 {
  script: string;
  hook: string;
  ctaSpoken: string;
  ctaDisplay: string;
  ...
  captions: V13CompiledCaption[];
  headlines: Record<string, V13BeatHeadline>;
  ttsBeats: Array<{ id: string; sceneId: string; text: string; energy: "high" | "medium" }>;
  numeralAnchors: V13NumeralAnchor[];
}

export function compileSolomonCreatorStoryV13(): CompiledSolomonCreatorStoryV13 {
  const beats = SOLOMON_CREATOR_STORY_V13_BEATS;
  ...
  const headlines: Record<string, V13BeatHeadline> = {};
