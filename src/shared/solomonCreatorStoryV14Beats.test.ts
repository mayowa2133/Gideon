...
import { compileSolomonCreatorStoryV14, SOLOMON_CREATOR_STORY_V14_BEATS } from "./solomonCreatorStoryV14Beats";

describe("Solomon Creator Story V14 beat compiler", () => {
  const compiled = compileSolomonCreatorStoryV14();

  it("compiles one source of truth for script, captions, headlines, and TTS beats", () => {
    expect(compiled.script).toBe(SOLOMON_CREATOR_STORY_V14_BEATS.filter(({ vo }) => vo).map(({ vo }) => vo).join(" "));
    expect(compiled.hook).toBe(SOLOMON_CREATOR_STORY_V14_BEATS[0]!.vo);
    expect(compiled.ttsBeats.map(({ text }) => text).join(" ")).toBe(compiled.script);
    for (const caption of compiled.captions) {
      const beat = SOLOMON_CREATOR_STORY_V14_BEATS.find(({ id }) => id === caption.id);
