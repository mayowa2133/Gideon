// Single source of truth for every V14 text surface. The VO script, TTS beats,
...
export interface V14CompiledCaption {
  id: string;
  from: number;
  to: number;
  text: string;
...
export interface V14BeatHeadline { primary: string; accentItalic: string }
export interface V14NumeralAnchor { graphic: string; spokenToken: string; sceneId: string }
export interface V14Beat {
  id: string;
  sceneId: string;
  vo: string;
  energy: "high" | "medium";
  chip?: Omit<V14CompiledCaption, "id" | "text">;
  headline?: V14BeatHeadline;
  anchors?: V14NumeralAnchor[];
}

export const SOLOMON_CREATOR_STORY_V14_BEATS: readonly V14Beat[] = [
  {
    id: "hook",
    sceneId: "hook",
...
export interface CompiledSolomonCreatorStoryV14 {
  script: string;
  hook: string;
  ctaSpoken: string;
  ctaDisplay: string;
  ...
  captions: V14CompiledCaption[];
  headlines: Record<string, V14BeatHeadline>;
  ttsBeats: Array<{ id: string; sceneId: string; text: string; energy: "high" | "medium" }>;
  numeralAnchors: V14NumeralAnchor[];
}

export function compileSolomonCreatorStoryV14(): CompiledSolomonCreatorStoryV14 {
  const beats = SOLOMON_CREATOR_STORY_V14_BEATS;
  ...
  const headlines: Record<string, V14BeatHeadline> = {};
