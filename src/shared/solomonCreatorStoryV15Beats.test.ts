...
import { compileSolomonCreatorStoryV15, SOLOMON_CREATOR_STORY_V15_BEATS } from "./solomonCreatorStoryV15Beats";

describe("Solomon Creator Story V15 beat compiler", () => {
  const compiled = compileSolomonCreatorStoryV15();

  it("compiles one source of truth for script, captions, headlines, and TTS beats", () => {
    expect(compiled.script).toBe(SOLOMON_CREATOR_STORY_V15_BEATS.filter(({ vo }) => vo).map(({ vo }) => vo).join(" "));
    expect(compiled.hook).toBe(SOLOMON_CREATOR_STORY_V15_BEATS[0]!.vo);
    expect(compiled.ttsBeats.map(({ text }) => text).join(" ")).toBe(compiled.script);
    for (const caption of compiled.captions) {
      const beat = SOLOMON_CREATOR_STORY_V15_BEATS.find(({ id }) => id === caption.id);
