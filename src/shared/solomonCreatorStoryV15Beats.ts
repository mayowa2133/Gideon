// Single source of truth for every V15 text surface. The VO script, TTS beats,
...
export interface V15CompiledCaption {
  id: string;
  from: number;
  to: number;
  text: string;
...
export interface V15BeatHeadline { primary: string; accentItalic: string }
export interface V15NumeralAnchor { graphic: string; spokenToken: string; sceneId: string }
export interface V15Beat {
  id: string;
  sceneId: string;
  vo: string;
  energy: "high" | "medium";
  chip?: Omit<V15CompiledCaption, "id" | "text">;
  headline?: V15BeatHeadline;
  anchors?: V15NumeralAnchor[];
}

export const SOLOMON_CREATOR_STORY_V15_BEATS: readonly V15Beat[] = [
  {
    id: "hook",
    sceneId: "hook",
...
export interface CompiledSolomonCreatorStoryV15 {
  script: string;
  hook: string;
  ctaSpoken: string;
  ctaDisplay: string;
  ...
  captions: V15CompiledCaption[];
  headlines: Record<string, V15BeatHeadline>;
  ttsBeats: Array<{ id: string; sceneId: string; text: string; energy: "high" | "medium" }>;
  numeralAnchors: V15NumeralAnchor[];
}

export function compileSolomonCreatorStoryV15(): CompiledSolomonCreatorStoryV15 {
  const beats = SOLOMON_CREATOR_STORY_V15_BEATS;
  ...
  const headlines: Record<string, V15BeatHeadline> = {};
