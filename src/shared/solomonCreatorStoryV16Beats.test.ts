...
import { compileSolomonCreatorStoryV16, SOLOMON_CREATOR_STORY_V16_BEATS } from "./solomonCreatorStoryV16Beats";

describe("Solomon Creator Story V16 beat compiler", () => {
  const compiled = compileSolomonCreatorStoryV16();

  it("compiles one source of truth for script, captions, headlines, and TTS beats", () => {
    expect(compiled.script).toBe(SOLOMON_CREATOR_STORY_V16_BEATS.filter(({ vo }) => vo).map(({ vo }) => vo).join(" "));
    expect(compiled.hook).toBe(SOLOMON_CREATOR_STORY_V16_BEATS[0]!.vo);
    expect(compiled.ttsBeats.map(({ text }) => text).join(" ")).toBe(compiled.script);
    for (const caption of compiled.captions) {
      const beat = SOLOMON_CREATOR_STORY_V16_BEATS.find(({ id }) => id === caption.id);
