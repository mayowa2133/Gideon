// Single source of truth for every V16 text surface. The VO script, TTS beats,
...
export interface V16CompiledCaption {
  id: string;
  from: number;
  to: number;
  text: string;
...
export interface V16BeatHeadline { primary: string; accentItalic: string }
export interface V16NumeralAnchor { graphic: string; spokenToken: string; sceneId: string }
export interface V16Beat {
  id: string;
  sceneId: string;
  vo: string;
  energy: "high" | "medium";
  chip?: Omit<V16CompiledCaption, "id" | "text">;
  headline?: V16BeatHeadline;
  anchors?: V16NumeralAnchor[];
}

export const SOLOMON_CREATOR_STORY_V16_BEATS: readonly V16Beat[] = [
  {
    id: "hook",
    sceneId: "hook",
...
export interface CompiledSolomonCreatorStoryV16 {
  script: string;
  hook: string;
  ctaSpoken: string;
  ctaDisplay: string;
  ...
  captions: V16CompiledCaption[];
  headlines: Record<string, V16BeatHeadline>;
  ttsBeats: Array<{ id: string; sceneId: string; text: string; energy: "high" | "medium" }>;
  numeralAnchors: V16NumeralAnchor[];
}

export function compileSolomonCreatorStoryV16(): CompiledSolomonCreatorStoryV16 {
  const beats = SOLOMON_CREATOR_STORY_V16_BEATS;
  ...
  const headlines: Record<string, V16BeatHeadline> = {};
