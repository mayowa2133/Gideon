...
import { compileSolomonCreatorStoryV17, SOLOMON_CREATOR_STORY_V17_BEATS } from "./solomonCreatorStoryV17Beats";

describe("Solomon Creator Story V17 beat compiler", () => {
  const compiled = compileSolomonCreatorStoryV17();

  it("compiles one source of truth for script, captions, headlines, and TTS beats", () => {
    expect(compiled.script).toBe(SOLOMON_CREATOR_STORY_V17_BEATS.filter(({ vo }) => vo).map(({ vo }) => vo).join(" "));
    expect(compiled.hook).toBe(SOLOMON_CREATOR_STORY_V17_BEATS[0]!.vo);
    expect(compiled.ttsBeats.map(({ text }) => text).join(" ")).toBe(compiled.script);
    for (const caption of compiled.captions) {
      const beat = SOLOMON_CREATOR_STORY_V17_BEATS.find(({ id }) => id === caption.id);
