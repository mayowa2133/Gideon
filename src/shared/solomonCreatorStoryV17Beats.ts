// Single source of truth for every V17 text surface. The VO script, TTS beats,
...
export interface V17CompiledCaption {
  id: string;
  from: number;
  to: number;
  text: string;
...
export interface V17BeatHeadline { primary: string; accentItalic: string }
export interface V17NumeralAnchor { graphic: string; spokenToken: string; sceneId: string }
export interface V17Beat {
  id: string;
  sceneId: string;
  vo: string;
  energy: "high" | "medium";
  chip?: Omit<V17CompiledCaption, "id" | "text">;
  headline?: V17BeatHeadline;
  anchors?: V17NumeralAnchor[];
}

export const SOLOMON_CREATOR_STORY_V17_BEATS: readonly V17Beat[] = [
  {
    id: "hook",
    sceneId: "hook",
...
export interface CompiledSolomonCreatorStoryV17 {
  script: string;
  hook: string;
  ctaSpoken: string;
  ctaDisplay: string;
  ...
  captions: V17CompiledCaption[];
  headlines: Record<string, V17BeatHeadline>;
  ttsBeats: Array<{ id: string; sceneId: string; text: string; energy: "high" | "medium" }>;
  numeralAnchors: V17NumeralAnchor[];
}

export function compileSolomonCreatorStoryV17(): CompiledSolomonCreatorStoryV17 {
  const beats = SOLOMON_CREATOR_STORY_V17_BEATS;
  ...
  const headlines: Record<string, V17BeatHeadline> = {};
