// Single source of truth for every V18 text surface. The VO script, TTS beats,
...
export interface V18CompiledCaption {
  id: string;
  from: number;
  to: number;
  text: string;
...
export interface V18BeatHeadline { primary: string; accentItalic: string }
export interface V18NumeralAnchor { graphic: string; spokenToken: string; sceneId: string }
export interface V18Beat {
  id: string;
  sceneId: string;
  vo: string;
  energy: "high" | "medium";
  chip?: Omit<V18CompiledCaption, "id" | "text">;
  headline?: V18BeatHeadline;
  anchors?: V18NumeralAnchor[];
}

export const SOLOMON_CREATOR_STORY_V18_BEATS: readonly V18Beat[] = [
  {
    id: "hook",
    sceneId: "hook",
...
export interface CompiledSolomonCreatorStoryV18 {
  script: string;
  hook: string;
  ctaSpoken: string;
  ctaDisplay: string;
  ...
  captions: V18CompiledCaption[];
  headlines: Record<string, V18BeatHeadline>;
  ttsBeats: Array<{ id: string; sceneId: string; text: string; energy: "high" | "medium" }>;
  numeralAnchors: V18NumeralAnchor[];
}

export function compileSolomonCreatorStoryV18(): CompiledSolomonCreatorStoryV18 {
  const beats = SOLOMON_CREATOR_STORY_V18_BEATS;
  ...
  const headlines: Record<string, V18BeatHeadline> = {};
