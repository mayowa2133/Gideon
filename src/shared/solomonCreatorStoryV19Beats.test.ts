...
import { compileSolomonCreatorStoryV19, SOLOMON_CREATOR_STORY_V19_BEATS } from "./solomonCreatorStoryV19Beats";

describe("Solomon Creator Story V19 beat compiler", () => {
  const compiled = compileSolomonCreatorStoryV19();

  it("compiles one source of truth for script, captions, headlines, and TTS beats", () => {
    expect(compiled.script).toBe(SOLOMON_CREATOR_STORY_V19_BEATS.filter(({ vo }) => vo).map(({ vo }) => vo).join(" "));
    expect(compiled.hook).toBe(SOLOMON_CREATOR_STORY_V19_BEATS[0]!.vo);
    expect(compiled.ttsBeats.map(({ text }) => text).join(" ")).toBe(compiled.script);
    for (const caption of compiled.captions) {
      const beat = SOLOMON_CREATOR_STORY_V19_BEATS.find(({ id }) => id === caption.id);
...
describe("V19 kinetic word groups", () => {
  const compiled = compileSolomonCreatorStoryV19();
