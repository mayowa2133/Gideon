...
import { compileSolomonCreatorStoryV20, SOLOMON_CREATOR_STORY_V20_BEATS } from "./solomonCreatorStoryV20Beats";

describe("Solomon Creator Story V20 beat compiler", () => {
  const compiled = compileSolomonCreatorStoryV20();

  it("compiles one source of truth for script, captions, headlines, and TTS beats", () => {
    expect(compiled.script).toBe(SOLOMON_CREATOR_STORY_V20_BEATS.filter(({ vo }) => vo).map(({ vo }) => vo).join(" "));
    expect(compiled.hook).toBe(SOLOMON_CREATOR_STORY_V20_BEATS[0]!.vo);
    expect(compiled.ttsBeats.map(({ text }) => text).join(" ")).toBe(compiled.script);
    for (const caption of compiled.captions) {
      const beat = SOLOMON_CREATOR_STORY_V20_BEATS.find(({ id }) => id === caption.id);
...
describe("V20 kinetic word groups", () => {
  const compiled = compileSolomonCreatorStoryV20();
