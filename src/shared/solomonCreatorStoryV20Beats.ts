// Single source of truth for every V20 text surface. The VO script, TTS beats,
...
export interface V20CompiledCaption {
  id: string;
  from: number;
  to: number;
  text: string;
  ...
  wordGroups: V20WordGroup[];
}
export interface V20WordGroup { text: string; from: number; to: number; emphasis: boolean }
export interface V20BeatHeadline { primary: string; accentItalic: string }
export interface V20NumeralAnchor { graphic: string; spokenToken: string; sceneId: string }
export interface V20Beat {
  id: string;
  sceneId: string;
  vo: string;
  energy: "high" | "medium";
  chip?: Omit<V20CompiledCaption, "id" | "text" | "wordGroups">;
  headline?: V20BeatHeadline;
  anchors?: V20NumeralAnchor[];
}

export const SOLOMON_CREATOR_STORY_V20_BEATS: readonly V20Beat[] = [
  {
    id: "hook",
    sceneId: "hook",
...
export interface CompiledSolomonCreatorStoryV20 {
  script: string;
  hook: string;
  ctaSpoken: string;
  ctaDisplay: string;
  ...
  captions: V20CompiledCaption[];
  headlines: Record<string, V20BeatHeadline>;
  ttsBeats: Array<{ id: string; sceneId: string; text: string; energy: "high" | "medium" }>;
  numeralAnchors: V20NumeralAnchor[];
}
...
export function buildWordGroups(source: string, from: number, to: number, highlight?: string): V20WordGroup[] {
...
export function compileSolomonCreatorStoryV20(): CompiledSolomonCreatorStoryV20 {
  const beats = SOLOMON_CREATOR_STORY_V20_BEATS;
  ...
  const headlines: Record<string, V20BeatHeadline> = {};
