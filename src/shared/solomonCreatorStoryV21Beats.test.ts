...
import { compileSolomonCreatorStoryV21, SOLOMON_CREATOR_STORY_V21_BEATS } from "./solomonCreatorStoryV21Beats";

describe("Solomon Creator Story V21 beat compiler", () => {
  const compiled = compileSolomonCreatorStoryV21();

  it("compiles one source of truth for script, captions, headlines, and TTS beats", () => {
    expect(compiled.script).toBe(SOLOMON_CREATOR_STORY_V21_BEATS.filter(({ vo }) => vo).map(({ vo }) => vo).join(" "));
    expect(compiled.hook).toBe(SOLOMON_CREATOR_STORY_V21_BEATS[0]!.vo);
    expect(compiled.ttsBeats.map(({ text }) => text).join(" ")).toBe(compiled.script);
    for (const caption of compiled.captions) {
      const beat = SOLOMON_CREATOR_STORY_V21_BEATS.find(({ id }) => id === caption.id);
...
describe("V21 kinetic word groups", () => {
  const compiled = compileSolomonCreatorStoryV21();
