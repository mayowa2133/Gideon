// Single source of truth for every V21 text surface. The VO script, TTS beats,
...
export interface V21CompiledCaption {
  id: string;
  from: number;
  to: number;
  text: string;
  ...
  wordGroups: V21WordGroup[];
}
export interface V21WordGroup { text: string; from: number; to: number; emphasis: boolean }
export interface V21BeatHeadline { primary: string; accentItalic: string }
export interface V21NumeralAnchor { graphic: string; spokenToken: string; sceneId: string }
export interface V21Beat {
  id: string;
  sceneId: string;
  vo: string;
  energy: "high" | "medium";
  chip?: Omit<V21CompiledCaption, "id" | "text" | "wordGroups">;
  headline?: V21BeatHeadline;
  anchors?: V21NumeralAnchor[];
}

export const SOLOMON_CREATOR_STORY_V21_BEATS: readonly V21Beat[] = [
  {
    id: "hook",
    sceneId: "hook",
...
export interface CompiledSolomonCreatorStoryV21 {
  script: string;
  hook: string;
  ctaSpoken: string;
  ctaDisplay: string;
  ...
  captions: V21CompiledCaption[];
  headlines: Record<string, V21BeatHeadline>;
  ttsBeats: Array<{ id: string; sceneId: string; text: string; energy: "high" | "medium" }>;
  numeralAnchors: V21NumeralAnchor[];
}
...
export function buildWordGroups(source: string, from: number, to: number, highlight?: string): V21WordGroup[] {
...
export function compileSolomonCreatorStoryV21(): CompiledSolomonCreatorStoryV21 {
  const beats = SOLOMON_CREATOR_STORY_V21_BEATS;
  ...
  const headlines: Record<string, V21BeatHeadline> = {};
