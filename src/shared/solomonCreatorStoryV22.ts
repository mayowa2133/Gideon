...
export function createSolomonCreatorStoryV22Manifest(inputs:V22SourceInput[],parentMasterSha256:string):SolomonCreatorStoryV22Manifest{
  ...
  const scenes:V22Scene[]=[
    ...
    scene("friction",141,173,narr("friction"),"friction",[],[],mascot("friction",32,"host","concerned","concerned_curve","stop_palm","open_palm","camera","crowding-tabs","emotion",[{atFrame:6,face:"surprised",holdFrames:8}]),"cut","Tabs crowd and recoil the presenter","editorial_peak",true,layouts("host"),{recipe:"recoil",scaleFrom:1,scaleTo:1.06,focus:{x:.5,y:.45},semanticTarget:"mascot"},"oxblood"),
    ...
    scene("collapse",270,311,"","collapse",["opportunity"],[],mascot("collapse",41,"absent","focused","small_open","presentation_palm","approval","product","opportunity-card","transition"),"object_wipe","Five fragments collapse into opportunity","editorial_peak",true,layouts("detail"),{recipe:"fragments_to_object",scaleFrom:.9,scaleTo:1.25,focus:{x:.5,y:.48},semanticTarget:"opportunity"},"slate"),
    scene("role",311,379,narr("role"),"role",["opportunity","contact"],["role"],mascot("role",68,"host","focused","small_open","true_point_left","presentation_palm","product","role-company","attention"),"cut","Opportunity object carries into evidence","product_annotation",false,layouts("split_close"),{recipe:"object_follow",scaleFrom:1.42,scaleTo:1.7,focus:{x:.25,y:.3},semanticTarget:"Product Engineer at Northstar Labs"},"paper"),
    scene("reason",379,449,narr("reason"),"reasons",["contact"],["contact","relevance"],mascot("reason",70,"host","focused","small_open","presentation_palm","true_point_right","product","avery-proof","interpretation"),"cut","Contact and proof become one object","product_annotation",false,layouts("split"),{recipe:"proof_attach",scaleFrom:1.55,scaleTo:1.92,focus:{x:.25,y:.7},semanticTarget:"Avery evidence"},"oxblood"),
    ...
    scene("grounded",690,786,narr("grounded"),"grounded",["outreach_blank","outreach_complete"],["draft"],mascot("grounded",96,"host","skeptical","confident_smile","stop_palm","approval","product","sentence-compare","interpretation",[{atFrame:12,face:"surprised",holdFrames:7}]),"cut","Generic sentence morphs into grounded sentence","editorial_peak",false,layouts("split"),{recipe:"text_morph",scaleFrom:1.7,scaleTo:2.08,focus:{x:.61,y:.47},semanticTarget:"inserted context"},"oxblood"),
    ...
    scene("result",995,1027,"","result",["tracker_after","contact","outreach_complete"],["status","contact","relevance","draft"],mascot("result",32,"host","happy","wide_open","celebration","approval","product","connected-result","payoff_reaction",[{atFrame:3,face:"surprised",holdFrames:9}]),"cut","Result holds while mascot celebrates after proof","editorial_peak",false,layouts("split"),{recipe:"result_reframe",scaleFrom:1.08,scaleTo:1.28,focus:{x:.5,y:.42},semanticTarget:"one connected next step"},"blush"),
    ...
    scene("sting",1129,1155,"","sting",[],[],mascot("sting",26,"hero_close","wink","closed_smile","wave","rest_mitt","camera","solomon-mark","cta"),"cut","Bookmark pulse resolves into Solomon mark","cta",false,layouts("presenter_close"),{recipe:"pulse_to_mark",scaleFrom:1,scaleTo:1.06,focus:{x:.5,y:.5},semanticTarget:"Solomon"},"ink")
  ];
...
export function auditSolomonCreatorStoryV22(manifest:SolomonCreatorStoryV22Manifest){
  ...
  const backdropLuma=auditV22BackdropLuma();
...
// Deterministic per-scene posture. A string hash rather than a random seed so a
// scene keeps its stance across renders, and spread over a real range rather
// than the two values the old length-parity trick gave.
function postureHash(sceneId:string,salt:number){let hash=2166136261^salt;for(let index=0;index<sceneId.length;index+=1){hash^=sceneId.charCodeAt(index);hash=Math.imul(hash,16777619);}return((hash>>>0)%1000)/1000;}
function spread(sceneId:string,salt:number,low:number,high:number){return Number((low+postureHash(sceneId,salt)*(high-low)).toFixed(3));}
function mascot(sceneId:string,duration:number,role:V22MascotPerformance["role"],face:V22Face,mouthBias:V22Mouth,leftGesture:V22Gesture,rightGesture:V22Gesture,target:"camera"|"product"|"caption"|"cta",elementId:string,purpose:V22MascotPerformance["narrativePurpose"],faceAccents?:V22MascotPerformance["faceAccents"]):V22MascotPerformance{const absent=role==="absent";return v22MascotPerformanceSchema.parse({sceneId,role,narrativePurpose:purpose,face,mouthBias,faceAccents,gazePath:absent?[{frame:0,target:"camera",x:.5,y:.4}]:[{frame:0,target:"camera",x:.5,y:.4},{frame:3,target,x:target==="cta"?.72:target==="product"?.68:.5,y:target==="cta"?.68:.42}],head:{turn:target==="product"?spread(sceneId,11,.28,.55):target==="cta"?.3:spread(sceneId,17,-.26,.26),tilt:spread(sceneId,23,-.5,.5),beats:[4,Math.max(8,Math.floor(duration*.55))]},torso:{lean:(faceAccents??[]).some(({face:accent})=>accent==="surprised")?.58:face==="concerned"?-.38:spread(sceneId,41,.12,.46),rotate:spread(sceneId,31,-.34,.34),recoil:face==="concerned"?.55:.12},left:{gesture:leftGesture,timing:{start:5,peak:Math.min(duration-8,Math.max(11,Math.floor(duration*.32))),recover:Math.min(duration-2,Math.max(18,Math.floor(duration*.74))),wristRotation:14}},right:{gesture:rightGesture,timing:{start:8,peak:Math.min(duration-5,Math.max(16,Math.floor(duration*.48))),recover:Math.min(duration-1,Math.max(22,Math.floor(duration*.86))),wristRotation:-18}},blinkFrames:[Math.max(10,Math.floor(duration*.43)),Math.max(14,Math.floor(duration*.43)+2)],audioFrames:Array.from({length:duration},(_,frame)=>({frame,rms:frame%7===0?.16:.42,speaking:!absent,onset:frame%17===3?.7:.06,phraseBoundary:frame%29===0})),interactionTarget:absent?undefined:{elementId,x:target==="cta"?.72:.68,y:target==="cta"?.68:.42,action:`${leftGesture}/${rightGesture}`}});}
function layouts(mode:"hero"|"host"|"left"|"right"|"product"|"cta"|"split"|"split_close"|"detail"|"presenter_close"):V22Rect[]{const caption:V22Rect={id:"caption",kind:"caption",left:.05,top:.04,right:.95,bottom:.16};
...
// Framing variety. Every scene but the hook and the CTA held the same shot --
// product above, presenter below -- which is what makes the film read as one
// long slide rather than an edit. Two more framings, solved the same way:
//
//   detail            product fills the frame, presenter absent. The cut-away.
//     product .17-.92 -> 326-1766, and no mascot rect so nothing can collide.
//   presenter_close   presenter fills the frame, no product. The sign-off.
//     mascot .30-.975 -> width-limited fit at 1.309, rig 642-1872, antenna top
//     594, clear of the caption band that ends at 307.
if(mode==="detail")return[{id:"product",kind:"product",left:.04,top:.17,right:.96,bottom:.92},caption];
if(mode==="presenter_close")return[{id:"mascot",kind:"mascot",left:.10,top:.30,right:.90,bottom:.975},caption];
if(mode==="split")return[{id:"product",kind:"product",left:.04,top:.17,right:.96,bottom:.54},{id:"mascot",kind:"mascot",left:.23,top:.56,right:.77,bottom:.975},caption];
  // `split_close` is `split` with the presenter given the room the proof bands
  // gave back. Ten of eighteen scenes ran `split`, which is why the film reads as
  // one composition repeated: the presenter is the same size in the same place in
  // more than half the shots, where the references cut between wide and close.
  //
  // Only scenes whose content actually shrank can take it, and the binding
  // constraint is not the card -- it is the proof labels, which sat at the content
  // floor. Solved for `role`, whose band is 157px where its card was 590:
  //
  //   caption ends    .16  * 1920 =  307.2
  //   band  400 -> 557 (drawn)               labels 590 -> 700 (moved up from 905)
  //   product ends    .38  * 1920 =  729.6   (29.6px below the labels)
  //   antenna top                 =  735.2   (5.6px clear, at this scale 32.8px
  //                                           of bleed above a .40 box)
  //   mascot box top  .40  * 1920 =  768.0
  //   rig bottom      .975 * 1920 = 1872.0
  //
  // Fit is min(.54*1080/660, .575*1920/940) = .884, width-limited as `split` is,
  // giving 23.4% of the frame against `split`'s 25.1% at a taller box -- the gain
  // is that it appears on a scene that had 18.9% before, not that the box is
  // bigger than split's.
  if(mode==="split_close")return[{id:"product",kind:"product",left:.04,top:.17,right:.96,bottom:.38},{id:"mascot",kind:"mascot",left:.23,top:.40,right:.77,bottom:.975},caption];
