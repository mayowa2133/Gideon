...
export const SOLOMON_CREATOR_STORY_V22_DURATION_FRAMES=1080 as const;
...
export function createSolomonCreatorStoryV22Manifest(inputs:V22SourceInput[],parentMasterSha256:string):SolomonCreatorStoryV22Manifest{
  ...
  const scenes:V22Scene[]=[
    scene("hook",0,24,narr("hook"),"hook",[],[],mascot("hook",24,"hero_close","happy","wide_open","wave","open_palm","camera","hook-status","emotion",[{atFrame:2,face:"surprised",holdFrames:9}]),"cut","Face-first greeting with a brief surprise accent","spoken",false,layouts("hero"),{recipe:"camera_push",scaleFrom:1.14,scaleTo:1.28,focus:{x:.5,y:.34},semanticTarget:"eyes"},"clay"),
    scene("status",24,54,"","status",["tracker_before","tracker_after"],["status"],mascot("status",30,"host","happy","small_open","presentation_palm","true_point_right","product","interview-control","attention"),"cut","Applied → click → Interviewing","product_annotation",false,layouts("split"),{recipe:"control_to_result",scaleFrom:1.7,scaleTo:2.05,focus:{x:.76,y:.62},semanticTarget:"Interviewing"},"paper"),
    scene("contact",54,102,"","contact",["contact"],["contact"],mascot("contact",48,"host","friendly","small_open","open_palm","true_point_right","product","avery-card","attention"),"slide","Opportunity indicator becomes contact card","product_annotation",false,layouts("split"),{recipe:"card_extract",scaleFrom:1.5,scaleTo:1.85,focus:{x:.28,y:.67},semanticTarget:"Avery Chen"},"slate"),
    scene("reasons",102,132,"","reasons",["contact"],["relevance"],mascot("reasons",30,"host","focused","neutral_dash","true_point_left","presentation_palm","product","reasons","interpretation"),"cut","Three evidence cards expand from product","product_annotation",false,layouts("split"),{recipe:"evidence_steps",scaleFrom:1.65,scaleTo:1.95,focus:{x:.27,y:.76},semanticTarget:"relevance proof"},"blush"),
    scene("friction",132,162,narr("friction"),"friction",[],[],mascot("friction",30,"host","concerned","concerned_curve","stop_palm","open_palm","camera","crowding-tabs","emotion"),"cut","Tabs crowd and recoil the presenter","editorial_peak",true,layouts("host"),{recipe:"recoil",scaleFrom:1,scaleTo:1.06,focus:{x:.5,y:.45},semanticTarget:"mascot"},"espresso"),
    scene("five",162,252,narr("five"),"five_surfaces",["opportunity","contact","outreach_blank"],[],mascot("five",90,"absent","skeptical","neutral_dash","thinking_hand","stop_palm","product","five-surfaces","interpretation"),"object_wipe","Five realistic tools copy one role","editorial_peak",true,layouts("product"),{recipe:"copy_chain",scaleFrom:1.05,scaleTo:1.32,focus:{x:.48,y:.47},semanticTarget:"5× context"},"blush"),
    scene("collapse",252,291,"","collapse",["opportunity"],[],mascot("collapse",39,"host","focused","small_open","presentation_palm","approval","product","opportunity-card","transition"),"object_wipe","Five fragments collapse into opportunity","editorial_peak",true,layouts("split"),{recipe:"fragments_to_object",scaleFrom:.9,scaleTo:1.25,focus:{x:.5,y:.48},semanticTarget:"opportunity"},"slate"),
    scene("role",291,354,narr("role"),"role",["opportunity","contact"],["role"],mascot("role",63,"host","focused","small_open","true_point_left","presentation_palm","product","role-company","attention"),"cut","Opportunity object carries into evidence","product_annotation",false,layouts("split"),{recipe:"object_follow",scaleFrom:1.42,scaleTo:1.7,focus:{x:.25,y:.3},semanticTarget:"Product Engineer at Northstar Labs"},"paper"),
    scene("reason",354,420,narr("reason"),"reasons",["contact"],["contact","relevance"],mascot("reason",66,"host","focused","small_open","presentation_palm","true_point_right","product","avery-proof","interpretation"),"cut","Contact and proof become one object","product_annotation",false,layouts("split"),{recipe:"proof_attach",scaleFrom:1.55,scaleTo:1.92,focus:{x:.25,y:.7},semanticTarget:"Avery evidence"},"espresso"),
    scene("signature",420,475,narr("signature"),"signature",["opportunity","contact","outreach_blank","outreach_complete"],["role","contact"],mascot("signature",55,"host","happy","confident_smile","presentation_palm","true_point_left","product","context-flow","transition"),"object_wipe","JOB → PERSON opens the chain","editorial_peak",true,layouts("split"),{recipe:"semantic_follow",scaleFrom:1,scaleTo:1.12,focus:{x:.52,y:.48},semanticTarget:"job to person"},"clay",420),
    scene("signature_proof",475,585,"","signature",["contact","outreach_blank","outreach_complete"],["relevance","draft"],mascot("signature_proof",110,"host","focused","confident_smile","presentation_palm","true_point_left","product","context-flow","transition"),"cut","Proof joins the chain and resolves into the message","editorial_peak",true,layouts("split"),{recipe:"semantic_follow",scaleFrom:1.12,scaleTo:1.30,focus:{x:.52,y:.48},semanticTarget:"evidence into message"},"slate",420),
    ...
    scene("grounded",645,735,narr("grounded"),"grounded",["outreach_blank","outreach_complete"],["draft"],mascot("grounded",90,"host","skeptical","confident_smile","stop_palm","approval","product","sentence-compare","interpretation"),"cut","Generic sentence morphs into grounded sentence","editorial_peak",false,layouts("split"),{recipe:"text_morph",scaleFrom:1.7,scaleTo:2.08,focus:{x:.61,y:.47},semanticTarget:"inserted context"},"espresso"),
    scene("control",735,825,narr("control"),"control",["outreach_complete"],["control"],mascot("control",90,"host","direct_cta","neutral_dash","stop_palm","true_point_right","product","save-edit","interpretation"),"cut","Edit one word, click Save Edit, leave send untouched","product_annotation",false,layouts("split"),{recipe:"edit_then_hold",scaleFrom:1.7,scaleTo:2.0,focus:{x:.62,y:.66},semanticTarget:"Save Edit and Cancel"},"paper"),
    ...
    scene("result",930,960,"","result",["tracker_after","contact","outreach_complete"],["status","contact","relevance","draft"],mascot("result",30,"host","happy","wide_open","celebration","approval","product","connected-result","payoff_reaction"),"cut","Result holds while mascot celebrates after proof","editorial_peak",false,layouts("split"),{recipe:"result_reframe",scaleFrom:1.08,scaleTo:1.28,focus:{x:.5,y:.42},semanticTarget:"one connected next step"},"blush"),
    ...
    scene("sting",1056,1080,"","sting",[],[],mascot("sting",24,"host","wink","closed_smile","wave","rest_mitt","camera","solomon-mark","cta"),"cut","Bookmark pulse resolves into Solomon mark","cta",false,layouts("split"),{recipe:"pulse_to_mark",scaleFrom:1,scaleTo:1.06,focus:{x:.5,y:.5},semanticTarget:"Solomon"},"ink")
  ];
...
export function assertSolomonCreatorStoryV22Manifest(manifest:SolomonCreatorStoryV22Manifest){
  ...
  if(manifest.scenes.at(-1)?.to!==1080)throw new Error("V22 timeline must end at frame 1080.");
...
function events(from:number,to:number,id:string):V22SemanticEvent[]{const result=[];for(let frame=from;frame<to;frame+=18)result.push({frame,kind:"semantic" as const,description:`${id} semantic state ${result.length+1}`});return result;}
