...
export const V15_MASCOT_GEOMETRY_VERSION="solomon-baby-schema-v15.1" as const;
export const V15_MASCOT_MATERIAL_VERSION="solomon-soft-gloss-performance-v2" as const;

export const v15FaceSchema=z.enum(["friendly","surprised","focused","happy","concerned","direct_cta","skeptical","wink"]);
export const v15MouthSchema=z.enum(["closed_smile","small_open","wide_open","surprised_circle","concerned_curve","neutral_dash","thinking_wave","confident_smile"]);
export const v15GestureSchema=z.enum(["rest_mitt","true_point_left","true_point_right","point_down","open_palm","presentation_palm","stop_palm","approval","thinking_hand","bookmark_hold","bookmark_tap","wave","celebration"]);

export const v15LimbTimingSchema=z.object({start:z.number().int().nonnegative(),peak:z.number().int().positive(),recover:z.number().int().positive(),wristRotation:z.number().min(-90).max(90)}).superRefine((value,context)=>{if(!(value.start<value.peak&&value.peak<value.recover))context.addIssue({code:"custom",message:"Limb timing must progress start → peak → recover."});});

export const v15MascotPerformanceSchema=z.object({
  ...
  left:z.object({gesture:v15GestureSchema,timing:v15LimbTimingSchema}),
  right:z.object({gesture:v15GestureSchema,timing:v15LimbTimingSchema}),
  blinkFrames:z.array(z.number().int().nonnegative()),
  faceAccents:z.array(z.object({atFrame:z.number().int().nonnegative(),face:v15FaceSchema,holdFrames:z.number().int().min(1).max(9)})).max(2).optional(),
...
export function auditSolomonMascotV15(geometry:typeof SOLOMON_MASCOT_V15_GEOMETRY=SOLOMON_MASCOT_V15_GEOMETRY){
...
export const V15_FACE_PATH_IDS:Record<V15Face,string>={friendly:"arch-soft",surprised:"round-large",focused:"bar-narrow",happy:"crescent-up",concerned:"inner-raised",direct_cta:"bright-symmetric",skeptical:"asymmetric-narrow",wink:"single-closed"};
export const V15_GESTURE_SILHOUETTE_IDS:Record<V15Gesture,string>={rest_mitt:"rest",true_point_left:"index-left",true_point_right:"index-right",point_down:"index-down",open_palm:"open-five",presentation_palm:"palm-up",stop_palm:"stop-five",approval:"thumb-approval",thinking_hand:"chin-touch",bookmark_hold:"bookmark-grip",bookmark_tap:"bookmark-index",wave:"wave-open",celebration:"arms-up"};

export function auditMascotPerformanceV15(plans:V15MascotPerformance[]){
  const visible=plans.filter(({role})=>role!=="absent");
  const faces=new Set(visible.flatMap((plan)=>[V15_FACE_PATH_IDS[plan.face],...(plan.faceAccents??[]).map(({face})=>V15_FACE_PATH_IDS[face])]));
  const gestures=new Set(visible.flatMap(({left,right})=>[V15_GESTURE_SILHOUETTE_IDS[left.gesture],V15_GESTURE_SILHOUETTE_IDS[right.gesture]]).filter((value)=>value!=="rest"));
...
export function sampleMascotAudioV15(frames:V15MascotPerformance["audioFrames"],frame:number){return frames.reduce((best,current)=>Math.abs(current.frame-frame)<Math.abs(best.frame-frame)?current:best,frames[0]!);}
