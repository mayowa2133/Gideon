...
export const V22_MASCOT_GEOMETRY_VERSION="solomon-baby-schema-v22.1" as const;
export const V22_MASCOT_MATERIAL_VERSION="solomon-soft-gloss-performance-v2" as const;

export const v22FaceSchema=z.enum(["friendly","surprised","focused","happy","concerned","direct_cta","skeptical","wink"]);
export const v22MouthSchema=z.enum(["closed_smile","small_open","wide_open","surprised_circle","concerned_curve","neutral_dash","thinking_wave","confident_smile"]);
export const v22GestureSchema=z.enum(["rest_mitt","true_point_left","true_point_right","point_down","open_palm","presentation_palm","stop_palm","approval","thinking_hand","bookmark_hold","bookmark_tap","wave","celebration"]);

export const v22LimbTimingSchema=z.object({start:z.number().int().nonnegative(),peak:z.number().int().positive(),recover:z.number().int().positive(),wristRotation:z.number().min(-90).max(90)}).superRefine((value,context)=>{if(!(value.start<value.peak&&value.peak<value.recover))context.addIssue({code:"custom",message:"Limb timing must progress start → peak → recover."});});

export const v22MascotPerformanceSchema=z.object({
  ...
  left:z.object({gesture:v22GestureSchema,timing:v22LimbTimingSchema}),
  right:z.object({gesture:v22GestureSchema,timing:v22LimbTimingSchema}),
  blinkFrames:z.array(z.number().int().nonnegative()),
  faceAccents:z.array(z.object({atFrame:z.number().int().nonnegative(),face:v22FaceSchema,holdFrames:z.number().int().min(1).max(9)})).max(2).optional(),
...
export function auditSolomonMascotV22(geometry:typeof SOLOMON_MASCOT_V22_GEOMETRY=SOLOMON_MASCOT_V22_GEOMETRY){
...
export const V22_FACE_PATH_IDS:Record<V22Face,string>={friendly:"arch-soft",surprised:"round-large",focused:"bar-narrow",happy:"crescent-up",concerned:"inner-raised",direct_cta:"bright-symmetric",skeptical:"asymmetric-narrow",wink:"single-closed"};
export const V22_GESTURE_SILHOUETTE_IDS:Record<V22Gesture,string>={rest_mitt:"rest",true_point_left:"index-left",true_point_right:"index-right",point_down:"index-down",open_palm:"open-five",presentation_palm:"palm-up",stop_palm:"stop-five",approval:"thumb-approval",thinking_hand:"chin-touch",bookmark_hold:"bookmark-grip",bookmark_tap:"bookmark-index",wave:"wave-open",celebration:"arms-up"};

export function auditMascotPerformanceV22(plans:V22MascotPerformance[]){
  const visible=plans.filter(({role})=>role!=="absent");
  const faces=new Set(visible.flatMap((plan)=>[V22_FACE_PATH_IDS[plan.face],...(plan.faceAccents??[]).map(({face})=>V22_FACE_PATH_IDS[face])]));
  const gestures=new Set(visible.flatMap(({left,right})=>[V22_GESTURE_SILHOUETTE_IDS[left.gesture],V22_GESTURE_SILHOUETTE_IDS[right.gesture]]).filter((value)=>value!=="rest"));
...
export function sampleMascotAudioV22(frames:V22MascotPerformance["audioFrames"],frame:number){return frames.reduce((best,current)=>Math.abs(current.frame-frame)<Math.abs(best.frame-frame)?current:best,frames[0]!);}
...
// frequency is what reads as busy. The first V22 render inherited V20's raw
...
export const V22_IDLE_FLOAT={amplitudeX:1.2,amplitudeY:1.5,speedX:.17,speedY:.13} as const;

export function mascotIdleSeed(value:string){return [...value].reduce((sum,char)=>((sum*31)+char.charCodeAt(0))%997,17);}
export function unevenV22(seed:number,frame:number,speed:number){return Math.sin(frame*speed+seed*.13)*.58+Math.sin(frame*speed*.43+seed*.29)*.28+Math.sin(frame*speed*.19+seed*.41)*.14;}

export function mascotIdleFloat(sceneId:string,frame:number){
  const seed=mascotIdleSeed(sceneId);
  return{
    x:Math.round(unevenV22(seed,frame,V22_IDLE_FLOAT.speedX)*V22_IDLE_FLOAT.amplitudeX),
    y:Math.round(unevenV22(seed+17,frame,V22_IDLE_FLOAT.speedY)*V22_IDLE_FLOAT.amplitudeY),
  };
}
